...
- Page content is MDX next to the code: `<name>.{en,ru}.mdx` (overview), `examples.<name>.{en,ru}.mdx` (examples tab), `docs/guides/*.{en,ru}.mdx` and `docs/data-grid/**`. Every page exists in both languages — update both. `build:docs-content` compiles the pages with `tools/docs-pages` into Angular components under `dist/docs-pages`, which the app imports as `@koobiq/docs-pages` and `docsPageResolver` picks per route, so the pages are prerendered with their live examples. It also regenerates the SEO descriptions and runs `docs:api-gen` (`tools/api-gen`), which writes the public API of every entry point as typed data under `dist/docs-pages-api` (`@koobiq/docs-pages-api`) — per entry its signature as the source declares it and the members worth explaining, the JSDoc compiled to HTML — for the one component of the API tab, `DocsApiPage`, to render, and writes the manifest `check-api-docs` and `llms-full.txt` read to `dist/docs-content/api-manifest`.
