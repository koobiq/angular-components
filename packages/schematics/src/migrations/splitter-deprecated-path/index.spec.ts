...
const SCHEMATIC_NAME = 'splitter-deprecated-path';
...
describe(SCHEMATIC_NAME, () => {
    ...
    it('rewrites a bare @koobiq/components/splitter import to /deprecated', async () => {
        const [first] = projects.keys();
        const { ts } = paths(projects.get(first)!);

        appTree.overwrite(
            ts,
            "import { KbqSplitterModule } from '@koobiq/components/splitter';\n" + 'const x: any = KbqSplitterModule;\n'
        );

        const updated = (await run(first)).readText(ts);

        expect(updated).toContain("from '@koobiq/components/splitter/deprecated'");
        expect(updated).not.toMatch(/from '@koobiq\/components\/splitter';/);
    });
    ...
    it('does not touch an unrelated sibling package whose name merely starts with "splitter"', async () => {
        const [first] = projects.keys();
        const { ts } = paths(projects.get(first)!);
        const original = "import { Whatever } from '@koobiq/components/splitter-x';\nconst x: any = Whatever;\n";
    ...
    describe('ng update entry point', () => {
        it('applies the fix when invoked without options', async () => {
            ...
            expect(result.readText(ts)).toContain("from '@koobiq/components/splitter/deprecated'");
        });
    });
