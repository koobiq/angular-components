...
describe(SCHEMATIC_NAME, () => {
    ...
    it('does not mistake the sidepanel design token for the removed class', async () => {
        ...
        // `--kbq-sidepanel-overlay-background` is a token the library still ships. Reading it says nothing
        // about the class that went away, and the package's own theme file is written exactly like this.
        appTree.overwrite(ts, "const styles = ['.dim { background: var(--kbq-sidepanel-overlay-background) }'];\n");

        await run(first);

        expect(messages.join('\n')).not.toContain('no longer added to the overlay host element');
    });
