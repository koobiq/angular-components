 ...
 *
 * The lookbehind keeps the `--kbq-sidepanel-*` design tokens out: a custom property always carries the
 * leading `--`, and a stylesheet that merely reads one is not a consumer of anything this migration
 * changed. A selector or an element — `.kbq-sidepanel-overlay`, `<kbq-sidepanel-header>` — is preceded
 * by something else and still matches.
 */
...
export const warnPatterns: WarnPattern[] = [
    ...
    {
        anchor: SIDEPANEL_TYPE,
        // Bounded on both sides so the `--kbq-sidepanel-overlay-background` token does not read as the class:
        // the token is preceded by `-` and continues with `-background`, the class does neither.
