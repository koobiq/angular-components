...
/** A receiver whose static type is a autocomplete, valid within `[start, end]` of the source. */
interface Receiver {
 ...
 * resolution): method/function params, class fields (incl. `@ViewChild(KbqAutocomplete) x: KbqAutocomplete` and constructor
...
function classifyAccess(node: ts.PropertyAccessExpression, sourceFile: ts.SourceFile, edits: Edit[]): void {
    ...
    // Write target: `x.member = RHS`. Every KbqAutocomplete signal member is `input()` (read-only), so there is no
...
/** Collects edits for every read/write of a value-safe signal member on a known autocomplete receiver. */
...
/** Collects the members read on a autocomplete receiver that no consumer can keep reading as-is. */
...
/** Pass A — rewrite value-safe programmatic reads of autocomplete signal members in TypeScript code. */
...
/** Collects template reference variable names bound to a `<kbq-autocomplete>` element. */
class AutocompleteRefCollector implements Visitor {
    readonly refs = new Set<string>();

    visitElement(element: any): void {
        if (element.name === AUTOCOMPLETE_ELEMENT) {
...
/** Pass B (core) — parse a template, discover autocomplete refs, rewrite their value-safe signal reads. */
async function migrateTemplate(template: string): Promise<{ content: string; changed: boolean }> {
    if (!template.includes(AUTOCOMPLETE_ELEMENT)) return { content: template, changed: false };

    const parsed = await parseTemplate(template);

    if (!parsed.tree) return { content: template, changed: false };

    const collector = new AutocompleteRefCollector();
...
/** Pass B (inline) — rewrite autocomplete ref reads inside inline component templates. */
...
/**
 * A `.ts` file is a autocomplete consumer if it names any of the exported symbols, imports the package, or renders
 * the element in an inline template — a component that only imports `KbqAutocompleteModule` names no type.
 */
...
export default function autocompleteSignals(options: Schema): Rule {
    return async (tree: Tree, context: SchematicContext) => {
        ...
        for (const filePath of htmlPaths) {
            ...
            if (!original || !original.includes(`<${AUTOCOMPLETE_ELEMENT}`)) continue;
