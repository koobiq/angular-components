...
| `formField.cleaner = …` / `hint` / `passwordHints` / `prefix` / `suffix` / `passwordToggle` | Read-only signal content queries now. Project the content instead — code that assigned a query result (usually a test faking it) has to render the real component.         |
