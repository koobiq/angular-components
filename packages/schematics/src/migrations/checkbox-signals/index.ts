...
function referencesCheckbox(content: string): boolean {
    return (
        /\bKbqCheckbox\w*\b/.test(content) ||
        content.includes(CHECKBOX_PACKAGE) ||
        content.includes(`<${CHECKBOX_ELEMENT}`)
    );
}
...
export default function checkboxSignals(options: Schema): Rule {
    return async (tree: Tree, context: SchematicContext) => {
        ...
        for (const filePath of tsPaths) {
            ...
            if (!original || !referencesCheckbox(original)) continue;
