...
/** A receiver whose static type is a loaderOverlay, valid within `[start, end]` of the source. */
interface Receiver {
    /** Source text of the receiver expression, e.g. `loaderOverlay` or `this.loaderOverlay`. */
    text: string;
    start: number;
    end: number;
}
 ...
 * resolution): method/function params, class fields (incl. `@ViewChild(KbqLoaderOverlay) x: KbqLoaderOverlay` and constructor
...
function classifyAccess(node: ts.PropertyAccessExpression, sourceFile: ts.SourceFile, edits: Edit[]): void {
    ...
    // Write target: `x.member = RHS`. Every KbqLoaderOverlay signal member is `input()` (read-only), so there is no
...
/** Collects edits for every read/write of a value-safe signal member on a known loaderOverlay receiver. */
...
/** Collects the members read on a loaderOverlay receiver that no consumer can keep reading as-is. */
...
/** Pass A — rewrite value-safe programmatic reads of loaderOverlay signal members in TypeScript code. */
function migrateTsExpressions(content: string, fileName: string): string {
    ...
    const receivers = collectReceivers(sourceFile, OVERLAY_TYPE);
...
/** Collects template reference variable names bound to a `<kbq-loader-overlay>` element. */
class LoaderOverlayRefCollector implements Visitor {
    readonly refs = new Set<string>();

    visitElement(element: any): void {
        if (element.name === OVERLAY_ELEMENT) {
...
/** Pass B (core) — parse a template, discover loaderOverlay refs, rewrite their value-safe signal reads. */
async function migrateTemplate(template: string): Promise<{ content: string; changed: boolean }> {
    if (!template.includes(OVERLAY_ELEMENT)) return { content: template, changed: false };

    const parsed = await parseTemplate(template);

    if (!parsed.tree) return { content: template, changed: false };

    const collector = new LoaderOverlayRefCollector();
...
/** Pass B (inline) — rewrite loaderOverlay ref reads inside inline component templates. */
...
/**
 * A `.ts` file is a loaderOverlay consumer if it names any of the exported symbols, imports the package, or renders
 * the element in an inline template — a component that only imports `KbqLoaderOverlayModule` names no type.
 */
...
export default function loaderOverlaySignals(options: Schema): Rule {
    return async (tree: Tree, context: SchematicContext) => {
        ...
        // Nothing here uses the loaderOverlay, so the summary would only be noise.
