 ...
 *
 * Attribute values are matched as opaque quoted strings, so a `>` inside an attribute expression
 * (e.g. `[class.a]="x > y"`) does not end the match early and strand a later `[innerHTML]` binding
 * on the same tag outside it. The third alternative excludes both quote characters so that only one
 * branch can consume a quote: an unterminated tag would otherwise backtrack exponentially.
 */
...
export const SUMMARY = [
    '  The rewrite is textual and only sees a literal [innerHTML]= inside a literal <kbq-flag …> ' +
        'opening tag. An innerHTML write made from TypeScript against a flag element, and an ' +
        'innerHTML-named input that a wrapper component of yours forwards to a flag, are both ' +
        'invisible to it and have to be moved by hand.',
