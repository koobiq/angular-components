...
/** A receiver whose static type is a progress spinner, valid within `[start, end]` of the source. */
interface Receiver {
 ...
 * resolution): method/function params, class fields (incl. `@ViewChild(KbqProgressSpinner) x: KbqProgressSpinner` and constructor
...
function classifyAccess(node: ts.PropertyAccessExpression, sourceFile: ts.SourceFile, edits: Edit[]): void {
    ...
    // Already migrated: `x.size()` (call) or `x.size.set(...)` — leave alone (idempotent).
    ...
    // Write target: `x.size = RHS`. Every KbqProgressSpinner signal member is `input()` (read-only), so there is no
...
/** Collects edits for every read/write of a value-safe signal member on a known spinner receiver. */
...
/** Pass A — rewrite value-safe programmatic reads of spinner signal members in TypeScript code. */
function migrateTsExpressions(content: string, fileName: string): string {
    ...
    const receivers = collectReceivers(sourceFile, SPINNER_TYPE);
...
function warnReceiverMembers(context: SchematicContext, filePath: string, content: string): void {
    ...
    if (protectedAccess.size > 0) {
        logMessage(context.logger, [
            `${LABEL} ${filePath}`,
            `  These KbqProgressSpinner members are now \`protected\` and can't be read from outside the ` +
                `component: ${[...protectedAccess].join(', ')}. They are derived from the \`value\` and \`size\` ` +
                `inputs — compute what you need from those instead.`
        ]);
    }
}

/** Collects template reference variable names bound to a `<kbq-progress-spinner>` element. */
class SpinnerRefCollector implements Visitor {
    readonly refs = new Set<string>();

    visitElement(element: any): void {
        if (element.name === SPINNER_ELEMENT) {
...
/** Pass B (core) — parse a template, discover spinner refs, rewrite their value-safe signal reads. */
async function migrateTemplate(template: string): Promise<{ content: string; changed: boolean }> {
    if (!template.includes(SPINNER_ELEMENT)) return { content: template, changed: false };

    const parsed = await parseTemplate(template);

    if (!parsed.tree) return { content: template, changed: false };

    const collector = new SpinnerRefCollector();
...
/**
 * A `.ts` file is a spinner consumer if it names any of the exported symbols, imports the package, or renders
 * the element in an inline template — a component that only imports `KbqProgressSpinnerModule` names no type.
 */
