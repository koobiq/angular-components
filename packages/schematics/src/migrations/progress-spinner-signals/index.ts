...
const LABEL = '[progress-spinner-signals]';
const TS_EXT = '.ts';
const HTML_EXT = '.html';
...
/** A receiver whose static type is a progress spinner, valid within `scope`. */
interface Receiver {
    /** Source text of the receiver expression, e.g. `spinner` or `this.spinner`. */
    text: string;
...
/** Nodes that rebind `this`, so `this.spinner` inside them is a different object. Arrows don't. */
...
/** Local names `SPINNER_TYPE` is bound to in this file, including aliased imports. */
function localTypeNames(sourceFile: ts.SourceFile): string[] {
    const names = new Set<string>([SPINNER_TYPE]);

    const visit = (node: ts.Node): void => {
        if (ts.isImportSpecifier(node) && (node.propertyName?.text ?? node.name.text) === SPINNER_TYPE) {
...
/**
 * The spinner-ness of an initializer, for the shapes a modern Angular consumer writes:
 * `inject(KbqProgressSpinner)`, `viewChild(KbqProgressSpinner)`, `viewChild.required(…)`,
 * `contentChild(…)`. Returns whether the declaration is a spinner and whether it is a signal query.
 */
function initializerTypeOf(
    initializer: ts.Expression | undefined,
    typeNames: string[]
): { spinner: boolean; signalQuery: boolean } {
    const none = { spinner: false, signalQuery: false };
    ...
    return { spinner: true, signalQuery: name !== 'inject' };
}

/**
 * Collects the spinner receivers: method/function params, class fields (incl.
 * `@ViewChild(KbqProgressSpinner) x: KbqProgressSpinner`, constructor parameter-properties and the
 * `inject()` / `viewChild()` initializer forms) and typed locals. Annotations that resolve are recorded in
...
function collectReceivers(sourceFile: ts.SourceFile, typeNames: string[], resolved?: Set<ts.Node>): Receiver[] {
    ...
    const visit = (node: ts.Node): void => {
        if (ts.isParameter(node) && ts.isIdentifier(node.name) && isTypeReference(node.type, typeNames)) {
            ...
            const { spinner, signalQuery } = initializerTypeOf(node.initializer, typeNames);

            if (owner && (annotated || spinner)) {
...
function resolveReceiver(
    ...
    // `this . spinner` and `this /* x */ . spinner` spell the same receiver as `this.spinner`.
    ...
    return receivers.find((receiver) => {
        ...
        // For `this.spinner`, a nested `function` or class changes what `this` is; an arrow does not.
        // For a bare `spinner`, a nested redeclaration of the same name shadows the receiver.
...
function classifyAccess(node: ts.PropertyAccessExpression, sourceFile: ts.SourceFile, edits: Edit[]): void {
    ...
    // Already migrated: `x.size()` (call) or `x.size.set(...)` and the rest of the signal API.
    ...
    // Write target: `x.size = RHS` and every compound form (`+=`, `||=`, …). Every spinner signal member is
    // `input()` (read-only), so the write is left untouched and becomes a compile error the consumer fixes
    // by hand. Appending `()` to any of them would produce unparseable TypeScript instead.
    ...
    // Read (incl. optional chain `x?.size`): append `()`.
    edits.push({ start: node.getEnd(), end: node.getEnd(), text: '()' });
}

/** Collects edits for every read of a signal member on a known spinner receiver. */
function collectAccessEdits(sourceFile: ts.SourceFile, receivers: Receiver[], bindings: Binding[]): Edit[] {
    ...
    const visit = (node: ts.Node): void => {
        if (
            ...
            // A signal query holds the instance behind a call of its own, so the read is `query().size()`.
            // Appending one `()` to `this.query.size` would be wrong in both halves; leave it to the warning.
            if (receiver && !receiver.signalQuery) classifyAccess(node, sourceFile, edits);
        }
...
interface ReceiverWarnings {
    ...
    /** Lines naming the spinner in a position no receiver could be scoped to. */
    unresolved: number[];
}

/**
 * 1-based lines where the spinner is named in a position `collectReceivers` cannot resolve - a union, an
 * array, a `QueryList<…>`, a cast, a return type - plus the reads the access pass structurally cannot
 * reach: `spinner['size']` and `const { size } = spinner`.
 */
...
/** Collects the members read on a spinner receiver that no consumer can keep reading as-is. */
...
/** Pass A — rewrite programmatic reads of spinner signal members in TypeScript code. */
function migrateTsExpressions(content: string, fileName: string): string {
    ...
    const receivers = collectReceivers(sourceFile, localTypeNames(sourceFile));
...
const protectedMessage = (members: Iterable<string>) =>
    `These KbqProgressSpinner members are \`protected\` now and can't be read from outside the component: ` +
    `${[...members].join(', ')}. ${PROTECTED_HINT}`;
...
/** A spinner reference variable, valid only within the embedded view that declares it. */
 ...
 * `<kbq-progress-spinner>`, the source ranges that actually hold Angular expressions, and every other name
 * the template introduces (so a `@for` variable sharing a ref's name is not rewritten).
 */
class TemplateScanner implements Visitor {
    readonly spinnerRefs: TemplateRef[] = [];
    ...
    visitElement(element: any): void {
        const isSpinner = element.name === SPINNER_ELEMENT;
        ...
        for (const attr of element.attrs ?? []) {
            ...
            if (reference !== undefined) {
                // `#s="cdkOverlayOrigin"` names a directive on the element, not the spinner itself.
                const exportsSpinner = !attr.value || attr.value === 'kbqProgressSpinner';

                if (isSpinner && exportsSpinner) this.spinnerRefs.push({ name: reference, ...this.view });
    ...
    visitBlock(block: any): void {
        for (const parameter of block.parameters ?? []) {
            ...
            // `@for (spinner of rows; track spinner)` and `@if (x; as y)` introduce names of their own.
...
/**
 * Matches `<ref>.<member>` inside a template expression. The lookbehind rejects `state.spinner.size`, where
...
/** Protected members read through a spinner reference variable, which no template can keep reading. */
...
interface TemplateResult {
    ...
    manual: Set<string>;
    /** The template renders the spinner but could not be parsed, so nothing in it was inspected. */
    unparseable: boolean;
}
...
/** Pass B (core) — parse a template, discover spinner refs, rewrite their signal reads. */
async function migrateTemplate(template: string): Promise<TemplateResult> {
    if (!template.includes(SPINNER_ELEMENT)) return untouched(template);
    ...
    const refs = scanner.spinnerRefs.filter((ref) => !scanner.otherNames.has(ref.name));
...
/** Pass B (inline) — rewrite spinner ref reads inside inline component templates. */
async function migrateInlineTemplates(
    ...
    if (!content.includes(SPINNER_ELEMENT)) return { content, manual, unparseable: false };
...
/** Reports the protected members a template reads through a spinner ref. */
...
/**
 * A `.ts` file is a spinner consumer if it names the exported symbol, imports the package, or renders the
 * element in an inline template — a component that only imports `KbqProgressSpinnerModule` names no type.
 */
function referencesSpinner(content: string): boolean {
    return (
        /\bKbqProgressSpinner\w*\b/.test(content) ||
        content.includes(SPINNER_PACKAGE) ||
        content.includes(`<${SPINNER_ELEMENT}`)
    );
}
...
export default function progressSpinnerSignals(options: Schema): Rule {
    return async (tree: Tree, context: SchematicContext) => {
        ...
        for (const filePath of tsPaths) {
            ...
            if (!original || !referencesSpinner(original)) continue;
        ...
        for (const filePath of htmlPaths) {
            ...
            // Counted on "renders the spinner", matching the `.ts` loop: a project whose only usage is a
            // plain `[value]` binding in an external template still needs the summary.
            if (!original || !original.includes(`<${SPINNER_ELEMENT}`)) continue;
        ...
        // Nothing here uses the spinner, so the summary would only be noise.
