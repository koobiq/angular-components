...
const KNOWN_MEMBERS: ReadonlySet<string> = new Set(
    TARGETS.flatMap((t) => [...t.signalMembers, ...t.reportedMembers, ...t.readOnlyMembers])
);
...
/** Nodes that rebind `this`, so `this.timepicker` inside them is a different object. Arrows don't. */
 ...
 * `inject(KbqTimepicker)`, `viewChild(KbqTimepicker)`, `viewChild.required(…)`, `contentChild(…)`.
 */
function initializerTypeOf(
    ...
    // `viewChild.required(...)` is `Signal<KbqTimepicker>`; the bare form adds `| undefined`.
 ...
 * `@ViewChild(KbqTimepicker) x: KbqTimepicker`, constructor parameter-properties and the `inject()` / `viewChild()`
...
/** The target one of the wrappers around a receiver asserts it to be: `(x as KbqTimepicker)`. */
...
/**
 * A receiver synthesized where the expression itself holds the instance: `(x as KbqTimepicker).member`, whose
...
function resolveReceiver(
    ...
    // `this.timepicker()?.format` is how a signal query is read in practice: the call already unwraps the signal,
    ...
    // `this . timepicker` and `this /* x */ . timepicker` spell the same receiver as `this.timepicker`.
    ...
    return receivers.find((receiver) => {
        ...
        // For `this.timepicker`, a nested `function` or class changes what `this` is; an arrow does not.
        // For a bare `timepicker`, a nested redeclaration of the same name shadows the receiver.
...
function accessKind(node: ts.PropertyAccessExpression): AccessKind {
    ...
    // Already migrated: `x.format()` (call) or the signal API on it.
    ...
    if (ts.isBinaryExpression(parent) && parent.left === node && ASSIGNMENT_OPERATORS.has(parent.operatorToken.kind))
        return 'write';
    // Only the increment operators count: a `PrefixUnaryExpression` is also how `!x.format` is spelled.
...
interface TsFindings {
    ...
    /** Members read whose value changed, so the call has to be added by hand. */
    reported: Set<string>;
...
function collectAccesses(
    ...
    const visit = (node: ts.Node): void => {
        if (ts.isPropertyAccessExpression(node) && ts.isIdentifier(node.name) && KNOWN_MEMBERS.has(node.name.text)) {
            ...
            // The member sets are per class: `KbqTimepicker` has a `format` of its own - an array - that did not
            ...
            const reportedMembers: readonly string[] = receiver?.target.reportedMembers ?? [];
            const readOnlyMembers: readonly string[] = receiver?.target.readOnlyMembers ?? [];

            if (
                receiver &&
                (signalMembers.includes(member) || reportedMembers.includes(member) || readOnlyMembers.includes(member))
            ) {
    ...
    return { edits, writes, reported, signalQueryReads };
}

/**
 * 1-based lines where a target is named in a position `collectReceivers` cannot resolve, plus the reads the
 * access pass structurally cannot reach: `timepicker['format']` and `const { format } = timepicker`.
 */
function collectUnresolvedMentions(
    ...
    const visit = (node: ts.Node): void => {
        if (
            ...
            // `x as KbqTimepicker` is the typing of that one access, which `resolveReceiver` already honours.
 ...
 * `KbqTimepicker` sits - is the element itself, so it matches nothing.
 */
...
/**
 * Matches `<ref>.<member>` inside a template expression. The lookbehind rejects `form.tp.format`, where
...
/** Rewrites `ref.format` reads to calls, only inside the given expression ranges. */
...
/** Members whose value changed, read through a reference variable, which the rewrite leaves to a human. */
function collectRefReportedMembers(template: string, refs: TemplateRef[], expressions: Range[]): Set<string> {
    ...
    for (const { start, end } of expressions) {
        ...
        for (const ref of refs) {
            if (start < ref.start || end > ref.end || ref.target.reportedMembers.length === 0) continue;

            for (const match of source.matchAll(memberAccessPattern(ref.name, ref.target.reportedMembers))) {
                found.add(match[2]);
            }
        }
    }
...
interface TemplateResult {
    content: string;
    changed: boolean;
    /** Members whose value changed, read through a reference variable. */
    reported: Set<string>;
    /** The template names the timepicker but could not be parsed, so nothing in it was inspected. */
    unparseable: boolean;
}
...
/** Whether a template can hold a timepicker reference: the directive attribute and its `exportAs` share this. */
const mentionsTimepicker = (template: string): boolean => template.includes('kbqTimepicker');

/** Pass B (core) — parse a template, discover timepicker refs, rewrite their signal reads. */
async function migrateTemplate(template: string): Promise<TemplateResult> {
    if (!mentionsTimepicker(template)) return untouched(template);
    ...
    return {
        ...
        reported: collectRefReportedMembers(template, refs, scanner.expressions),
        unparseable: false
    };
}

/** Pass B (inline) — rewrite timepicker ref reads inside inline component templates. */
async function migrateInlineTemplates(
    content: string,
    fileName: string
): Promise<{ content: string; reported: Set<string>; unparseable: boolean }> {
    const reported = new Set<string>();

    // Parsing the file to find inline templates is the expensive half, and most consumers have none.
    if (!mentionsTimepicker(content)) return { content, reported, unparseable: false };
    ...
    for (const { start, end } of ranges) {
        ...
        for (const member of outcome.reported) reported.add(member);
    ...
    return { content: result, reported, unparseable };
}
...
/**
 * A `.ts` file is a timepicker consumer if it names any of the exported symbols, imports the package, or reaches
 * the timepicker through its `exportAs` in an inline template.
 */
function referencesTimepicker(content: string): boolean {
    return /\bKbqTimepicker\w*\b/.test(content) || content.includes(TIMEPICKER_PACKAGE) || mentionsTimepicker(content);
}
...
export default function timepickerSignals(options: Schema): Rule {
    return async (tree: Tree, context: SchematicContext) => {
        ...
        for (const filePath of tsPaths) {
            ...
            if (inline.reported.size > 0) report(filePath, reportedMessage(inline.reported));
        ...
        for (const filePath of htmlPaths) {
            ...
            if (!original || !mentionsTimepicker(original)) continue;

            consumers++;

            const outcome = await migrateTemplate(original);

            if (outcome.reported.size > 0) report(filePath, reportedMessage(outcome.reported));
