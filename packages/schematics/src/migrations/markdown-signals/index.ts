...
/** A receiver whose static type is a markdown, valid within `[start, end]` of the source. */
interface Receiver {
 ...
 * resolution): method/function params, class fields (incl. `@ViewChild(KbqMarkdown) x: KbqMarkdown` and constructor
...
function classifyAccess(node: ts.PropertyAccessExpression, sourceFile: ts.SourceFile, edits: Edit[]): void {
    ...
    // Write target: `x.member = RHS`. Every KbqMarkdown signal member is `input()` (read-only), so there is no
...
/** Collects edits for every read/write of a value-safe signal member on a known markdown receiver. */
...
class MarkdownRefCollector implements Visitor {
    readonly refs = new Set<string>();

    visitElement(element: any): void {
        if (element.name === MARKDOWN_ELEMENT) {
...
/** Pass B (core) — parse a template, discover markdown refs, rewrite their value-safe signal reads. */
async function migrateTemplate(template: string): Promise<{ content: string; changed: boolean }> {
    if (!template.includes(MARKDOWN_ELEMENT)) return { content: template, changed: false };

    const parsed = await parseTemplate(template);

    if (!parsed.tree) return { content: template, changed: false };

    const collector = new MarkdownRefCollector();
...
/**
 * A `.ts` file is a markdown consumer if it names any of the exported symbols, imports the package, or renders
 * the element in an inline template — a component that only imports `KbqMarkdownModule` names no type.
 */
...
export default function markdownSignals(options: Schema): Rule {
    return async (tree: Tree, context: SchematicContext) => {
        ...
        // Nothing here uses the markdown, so the summary would only be noise.
