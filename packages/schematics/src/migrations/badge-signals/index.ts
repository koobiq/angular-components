...
/** A receiver whose static type is a badge, valid within `[start, end]` of the source. */
interface Receiver {
...
function classifyAccess(node: ts.PropertyAccessExpression, sourceFile: ts.SourceFile, edits: Edit[]): void {
    ...
    // Write target: `x.compact = RHS`. Every KbqBadge signal member is `input()` (read-only), so there is no
...
function collectReceiverWarnings(sourceFile: ts.SourceFile): ReceiverWarnings {
    ...
    const badgeReceivers = collectReceivers(sourceFile, BADGE_TYPE);
    ...
    const visit = (node: ts.Node): void => {
        if (ts.isPropertyAccessExpression(node) && ts.isIdentifier(node.name)) {
            ...
            if (inReceiverScope(node, sourceFile, badgeReceivers)) {
            ...
            if (STYLER_PRIVATE_MEMBERS.includes(name) && inReceiverScope(node, sourceFile, stylerReceivers)) {
    ...
    return { valueChanged, protectedAccess, stylerAccess };
}
...
function warnReceiverMembers(context: SchematicContext, filePath: string, content: string): void {
    ...
    const { valueChanged, protectedAccess, stylerAccess } = collectReceiverWarnings(sourceFile);
...
/** Collects template reference variable names bound to a `<kbq-badge>` element. */
class BadgeRefCollector implements Visitor {
    readonly refs = new Set<string>();

    visitElement(element: any): void {
        if (element.name === BADGE_ELEMENT) {
...
async function migrateTemplate(template: string): Promise<{ content: string; changed: boolean }> {
    if (!template.includes(BADGE_ELEMENT)) return { content: template, changed: false };

    const parsed = await parseTemplate(template);

    if (!parsed.tree) return { content: template, changed: false };

    const collector = new BadgeRefCollector();
