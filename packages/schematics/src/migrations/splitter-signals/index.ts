...
/** A receiver whose static type is a splitter, valid within `[start, end]` of the source. */
interface Receiver {
    /** Source text of the receiver expression, e.g. `splitter` or `this.splitter`. */
    text: string;
    start: number;
    end: number;
}
 ...
 * resolution): method/function params, class fields (incl. `@ViewChild(KbqSplitterComponent) x: KbqSplitterComponent` and constructor
...
function classifyAccess(node: ts.PropertyAccessExpression, sourceFile: ts.SourceFile, edits: Edit[]): void {
    ...
    // Write target: `x.member = RHS`. Every KbqSplitterComponent signal member is `input()` (read-only), so there is no
...
/** Collects edits for every read/write of a value-safe signal member on a known splitter receiver. */
...
/** Collects the members read on a splitter receiver that no consumer can keep reading as-is. */
...
/** Pass A — rewrite value-safe programmatic reads of splitter signal members in TypeScript code. */
function migrateTsExpressions(content: string, fileName: string): string {
    ...
    const receivers = RECEIVER_TYPES.flatMap((type) => collectReceivers(sourceFile, type));
...
function warnReceiverMembers(context: SchematicContext, filePath: string, content: string): void {
    ...
    if (protectedAccess.size > 0) {
        logMessage(context.logger, [
            `${LABEL} ${filePath}`,
            `  These KbqSplitterComponent members are now \`protected\` and can't be read from outside the ` +
                `component: ${[...protectedAccess].join(', ')}. ${PROTECTED_HINT}`
        ]);
    }
}

/** Collects template reference variable names bound to a `<kbq-splitter>` element. */
class SplitterComponentRefCollector implements Visitor {
    readonly refs = new Set<string>();

    visitElement(element: any): void {
        if (element.name === SPLITTER_ELEMENT) {
...
/** Pass B (core) — parse a template, discover splitter refs, rewrite their value-safe signal reads. */
async function migrateTemplate(template: string): Promise<{ content: string; changed: boolean }> {
    if (!template.includes(SPLITTER_ELEMENT)) return { content: template, changed: false };

    const parsed = await parseTemplate(template);

    if (!parsed.tree) return { content: template, changed: false };

    const collector = new SplitterComponentRefCollector();
...
/** Pass B (inline) — rewrite splitter ref reads inside inline component templates. */
...
/**
 * A `.ts` file is a splitter consumer if it names any of the exported symbols, imports the package, or renders
 * the element in an inline template — a component that only imports `KbqSplitterComponentModule` names no type.
 */
function referencesSplitterComponent(content: string): boolean {
    return (
        /\bKbqSplitterComponent\w*\b/.test(content) ||
        content.includes(SPLITTER_PACKAGE) ||
        content.includes(`<${SPLITTER_ELEMENT}`)
    );
}
...
export default function splitterSignals(options: Schema): Rule {
    return async (tree: Tree, context: SchematicContext) => {
        ...
        for (const filePath of tsPaths) {
            ...
            if (!original || !referencesSplitterComponent(original)) continue;
        ...
        // Nothing here uses the splitter, so the summary would only be noise.
