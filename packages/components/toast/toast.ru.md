...
Кнопка «Закрыть» по умолчанию — нативный `<button>`, доступное имя которого берётся из локали, поэтому до неё можно дойти по <span class="docs-hot-key-button">Tab</span> и нажать её по <span class="docs-hot-key-button">Enter</span> или <span class="docs-hot-key-button">Space</span>. Шаблон, переданный в `closeButton`, полностью заменяет её — оставьте настоящую кнопку и задайте ей доступное имя.
...
Последовательность повторяет порядок чтения тоста: сначала сообщение, затем его действия и в конце кнопка «Закрыть».
...
Тост объявляется в момент появления: стили **error** и **warning** вставляются с `role="alert"` и прерывают текущее объявление, остальные используют `role="status"` и дожидаются паузы. Оба варианта несут `aria-atomic="true"`, поэтому сообщение читается целиком. Тост, показанный через `showTemplate`, получает `role="status"` на первом элементе шаблона, если тот не объявил свою роль сам.

Автоматическое скрытие приостанавливают только ховер и фокус, и только у тоста, показанного через `show`: у шаблонного тоста из `showTemplate` таких слушателей нет, и он продолжает отсчёт под курсором. Пользователь скринридера, читающий виртуальным курсором, не двигает ни то ни другое, поэтому тост с действием или сообщением об ошибке следует показывать с `duration: 0` и закрывать явно.
...
Обе анимации пропускаются, если операционная система просит уменьшить движение (`prefers-reduced-motion: reduce`): тогда тост появляется и исчезает мгновенно.
