...
Если вычислять `[localeOverrides]` от активной локали, свои надписи продолжают следовать за `setLocale()`. Чтобы поменять их во всём приложении, зарегистрируйте их как данные локали через `KBQ_LOCALE_DATA` в корневых провайдерах — там их читает `KbqLocaleService`.
