...
Группа вкладок запоминает, какая вкладка была выбрана, и восстанавливает состояние после перезагрузки страницы. Включено по умолчанию — используйте `[useStateSaving]="false"` для отключения у компонента.
