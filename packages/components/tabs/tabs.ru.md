...
Группа вкладок запоминает, какая вкладка была выбрана, и восстанавливает её при следующей отрисовке. Включено по умолчанию — передайте `[useStateSaving]="false"` для группы, выбором в которой распоряжается приложение.

<!-- example(tabs-state-saving) -->

Пока привязаны `selectedIndex` или `activeTab`, не сохраняется ничего: выбор принадлежит тому, кто управляет привязкой.

Задавайте вкладкам `tabId`. Только идентификатор переживает перестановку вкладок — без него выбор откатывается на позицию, о чём в режиме разработки выводится предупреждение.

`kbq-tab-nav-bar` не сохраняет состояние никогда: активную ссылку определяет роутер, а восстанавливать имеет смысл URL. Удалить сохранённое: `clearSavedState()`.
