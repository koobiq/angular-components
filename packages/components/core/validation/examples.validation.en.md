...
Keep the validator on the group — it stays entirely yours, the library constrains neither its logic nor the
shape of the error it produces. What the library adds is the display half:
`ShowOnCrossFieldErrorStateMatcher` shows a group-level error on the controls that error concerns.

It needs one thing from you: a way to map an error to its controls.
...
The callback is handed one error of a group at a time and returns the names of the controls it applies to, or
`null` for errors it does not know. Names are `AbstractControl.get` paths, so `'passwords.confirm'`
works, as do `FormArray` indices — and the rule may sit on any ancestor group, not only the immediate parent.

One behaviour matters more than the rest: an error is revealed only once **every** control it concerns is
touched, or the form is submitted. Revealing it earlier means marking a pair red while the user is still
typing the second value, when it cannot yet be anything but "wrong".

That is the rule for the whole design system, so it is not configurable. A form that genuinely needs another
one — a range whose far end is prefilled and never visited, where waiting for it would hide the error until
submit — overrides it:
