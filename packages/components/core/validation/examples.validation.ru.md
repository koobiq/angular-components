...
здесь — всегда собственный `FormControl` поля: матчер никогда не видит группу.
...
Это не баг. Это разделение ответственности: валидаторы решают, **что** невалидно, а матчер решает, **когда и
где** это показать.
...
Одного экземпляра матчера хватает на любое число правил на группе — в примере выше на одной группе два
правила, и оба показывает один и тот же матчер.
