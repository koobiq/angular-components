...
Если дату не удалось разобрать или она отсутствует, pipe выводит пустую строку. Для диапазонов это относится к случаю, когда обе границы пустые; pipe продолжительности, кроме того, выводит пустую строку, если начало позже конца. Если нужно узнать об ошибке, а не скрыть её, вызывайте методы `DateFormatter` напрямую — они бросают исключение.
