...
| [Темизация](/ru/main/theming)                      | `ThemeService`, зарегистрированные варианты оформления и CSS-переменные                                                                            |
...
Локализация компонентов работает, когда для `KBQ_LOCALE_SERVICE` предоставлен `KbqLocaleService`; активная локаль задается через `KBQ_LOCALE_ID`.
...
`ThemeService` хранит зарегистрированные варианты оформления и тот, что применен сейчас. `KbqDefaultThemes` — встроенная пара, светлая и темная; `setThemes(items)` заменяет список своим.

Переключение — `setTheme(theme)` или индекс в `themes`, активный вариант читается через `getTheme()`. `current` это `BehaviorSubject`, поэтому подписчик сразу получает текущий вариант, а затем каждое переключение.

При переключении на `document.body` меняется `className` варианта — именно на него опираются CSS-переменные дизайн-системы.
...
| `MultipleMode`                                   | Режим множественного выбора для списков и деревьев                                         |
