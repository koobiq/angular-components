...
Читайте один раз при инициализации, пишите при каждом изменении состояния, удаляйте методом `clear()`.
...
**Ключ задаёт потребитель.** `stateSavingKey` обязателен на практике. `fallbackKey` нужен лишь для того, чтобы отсутствие ключа деградировало, а не падало: автоматически сгенерированный идентификатор зависит от порядка создания компонентов, а он меняется при ленивой загрузке, условном рендере и перестановке. Компонент, существующий в единственном экземпляре на приложение, может задокументировать литеральный ключ; всё, что рендерится на странице многократно, обязано его принимать.

**Храните идентификаторы, а не позиции.** Сохраняйте идентификатор выбранной вкладки, а не её индекс. Индекс переживает перезагрузку, но не переживает перестановку — и тогда молча восстановится не то, вместо того чтобы не восстановиться ничего.
