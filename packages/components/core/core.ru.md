...
| `kbqNumberFormattersLocaleConfigurationProvider(…)`  | Переопределяет правила форматирования чисел                                                                     |
...
| `KBQ_NUMBER_FORMATTERS_LOCALE_CONFIGURATION` | Правила форматирования чисел: десятичные разделители и сокращения округления                                                              |
...
- **Ключ выводится из документа, если он не задан.** `KBQ_STATE_SAVING_KEY_RESOLVER` строит его из цепочки имён тегов до `<body>`, обрезанной первым встреченным `id`, который становится якорем, — поэтому перестройка разметки ниже якоря сдвигает ключ и оставляет прежнюю запись сиротой. Если хост в момент чтения не находится в документе, ключа нет вовсе, и в режиме разработки об этом сообщается. Смена `stateSavingKey` в рантайме переводит компонент на запись под новым ключом: директива сообщает об изменении, а компонент восстанавливается из неё.
- **Храните идентификаторы, а не позиции.** Индекс переживает перезагрузку, но не переживает перестановку — и тогда молча восстановится не то, вместо того чтобы не восстановиться ничего.
- **Только JSON-сериализуемые данные.** Нельзя писать экземпляры компонентов, `TemplateRef`, функции и объекты дат, созданные адаптером; сохраняйте проекцию — идентификатор вместо объекта, ISO-строку вместо `DateTime`.
- **Нормализуйте при чтении.** `getState()` возвращает `unknown` намеренно: web storage общий на origin и правится пользователем, поэтому `normalize` обязан отбрасывать всё, что не соответствует ожидаемой форме. Здесь же мигрируются данные, записанные прошлой версией.
...
- **Не сохраняйте из оверлея.** У компонентов, создаваемых императивно в CDK-оверлее, нет стабильного ключа, поэтому их состояние принадлежит владеющему ими компоненту.
- **Хранилище читается синхронно.** Компонент читает один раз при инициализации и ждать не может, поэтому хранилище, чей `getState` возвращает `Promise`, ничего не восстановит, о чём предупреждают в режиме разработки. Чтобы состояние жило на сервере, загрузите его до отрисовки приложения (`provideAppInitializer`) и отдавайте из памяти.
- **Два компонента не должны делить ключ.** Ключ адресует запись целиком, а не пространство имён, поэтому компоненты с общим ключом затирают друг друга; столкнуться может только явный `stateSavingKey`, и в режиме разработки об этом предупреждают.
- **`KBQ_STATE_SAVING_ENABLED` решает до первого чтения.** `KbqStateSavingService.setEnabled()` — переключатель времени выполнения, но компонент читает при инициализации, поэтому решать лучше заранее токеном: `false` выключает сохранение во всём приложении, а отдельные компоненты возвращаются в него через `[useStateSaving]="true"`.

Хранилища на основе веб-хранилища пишут под префиксом `kbq.state.`, поэтому запись не может столкнуться с записью приложения, и отмечают каждую запись временем сохранения. Запись, которую `KBQ_STATE_SAVING_TTL` (по умолчанию 90 дней) не читали и не перезаписывали, удаляется при следующем создании хранилища — именно это не даёт накапливаться осиротевшим ключам; чтение обновляет отметку времени. Чтобы состояние жило только в пределах вкладки браузера, предоставьте `KbqSessionStorageStateStore`:

```ts
providers: [{ provide: KBQ_STATE_STORE, useExisting: KbqSessionStorageStateStore }];
```

Своё хранилище — например, серверное — реализует интерфейс `KbqStateStore` и предоставляется через тот же токен; указанное в `providers` самого компонента, оно заменяет хранилище только для него, а не для всего приложения. Если это одно из браузерных хранилищ, наследуйтесь от `KbqWebStorageStateStore`: он уже защищён от SSR, недоступного хранилища и нечитаемых данных.
