...
| `KBQ_STATE_STORE`                         | Куда компоненты сохраняют состояние между перезагрузками. Готовые реализации: `KbqLocalStorageStateStore`, `KbqSessionStorageStateStore`  |
| `KBQ_STATE_SAVING_KEY_RESOLVER`           | Как компонент выводит ключ хранения, если ключ не задан. По умолчанию `kbqStructuralStateSavingKey`                                       |
| `KBQ_STATE_SAVING_TTL`                    | Сколько запись в веб-хранилище живёт без чтения и перезаписи. По умолчанию 90 дней                                                        |
...
Компонент может сохранять своё состояние между перезагрузками через `KBQ_STATE_STORE`. Хранилище — это обычная связка «ключ — JSON», а `kbqStateSaving()` — обвязка над ним, которая берёт на себя то, в чём легче всего ошибиться: под каким ключом писать, когда запись разрешена и как превратить недоверенные данные обратно в состояние.

Объявите два инпута на компоненте, а контроллер создайте после них — инициализаторы полей выполняются по порядку:
...
Читайте один раз при инициализации, пишите при каждом изменении состояния, удаляйте методом `clear()`. Записи до этого первого `read()` подавляются: иначе инпут, меняющий состояние во время обновления родительского шаблона, перезаписал бы сохранённое значение раньше, чем компонент его увидит.
...
Правила ниже выведены из того, какие состояния реально держат компоненты. Если их нарушить, состояние восстановится не в тот компонент или не восстановится вовсе.

**Ключ выводится из документа, если он не задан.** `KBQ_STATE_SAVING_KEY_RESOLVER` строит его из цепочки имён тегов до `<body>`, обрезанной первым встреченным `id`, который становится якорем, — поэтому компонент сохраняет состояние без всякой настройки, а закрепить ключ автор может как через `stateSavingKey`, так и через `id`. Перестройка разметки ниже якоря сдвигает ключ и оставляет прежнюю запись сиротой, так что компоненту, состояние которого важно пережить редизайн, всё равно стоит задокументировать `stateSavingKey`. Если хост в момент чтения не находится в документе, ключа нет вовсе: ничего не сохраняется, и в dev-режиме об этом сообщается.

**Храните идентификаторы, а не позиции.** Сохраняйте идентификатор выбранной вкладки, а не её индекс. Индекс переживает перезагрузку, но не переживает перестановку — и тогда молча восстановится не то, вместо того чтобы не восстановиться ничего. Если идентификатор должен был задать потребитель, но не задал, остаётся только позиция — опишите такой запасной вариант в документации самого компонента, как это сделано у аккордеона для секции без `[value]`.

**Только JSON-сериализуемые данные.** Нельзя писать экземпляры компонентов, `TemplateRef`, функции и объекты дат, созданные адаптером: `JSON.stringify` либо бросит, либо тихо превратит их в то, что не прочитается обратно. Сохраняйте проекцию: идентификатор вместо объекта, ISO-строку вместо `DateTime`. Это строже, чем `structuredClone`, который `Date` как раз переживает.

**Нормализуйте при чтении.** `getState()` возвращает `unknown` намеренно. Web storage общий на origin и правится пользователем, поэтому `normalize` обязан отбрасывать всё, что не соответствует ожидаемой форме — приведение типа без проверки превращает отредактированную вручную запись в падение при восстановлении. Здесь же мигрируются данные, записанные прошлой версией, чтобы обновление не сбрасывало молча то, что было у пользователей.

**Пишите снимок целиком, а не изменение.** Полный снимок сам вычищает исчезнувшие значения. Инкрементальная запись оставляет их накапливаться и восстанавливать себя снова и снова.

**Восстанавливайте внутри `applying()`.** Восстановленное состояние применяется через собственные сеттеры компонента, а они пишут по ходу дела — без этой защиты состояние тут же запишется обратно.

**Приоритет должен быть явным.** Контролируемый инпут важнее сохранённого состояния, сохранённое состояние важнее значения по умолчанию.

**Не сохраняйте из оверлея.** У компонентов, создаваемых императивно в CDK-оверлее — сайдпанели, модальные окна, выпадающие меню, поповеры — нет стабильного ключа. Их состояние должен сохранять владеющий ими компонент.

Хранилища на основе веб-хранилища пишут под префиксом `kbq.state.`, поэтому запись не может столкнуться с записью приложения, и отмечают каждую запись временем сохранения. Запись, которую `KBQ_STATE_SAVING_TTL` (по умолчанию 90 дней) не читали и не перезаписывали, удаляется при следующем создании хранилища — именно это не даёт накапливаться ключам, осиротевшим после перестройки разметки. Чтение обновляет отметку времени, поэтому состояние, которое открывают, но не меняют, не истекает под активным пользователем.
