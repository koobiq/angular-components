...
| `KBQ_STATE_SAVING_ENABLED`                | Значение по умолчанию для `useStateSaving`. Передайте `false`, чтобы выключить сохранение во всём приложении                              |
...
Компонент сохраняет своё состояние между перезагрузками, подключая директиву `KbqStateSaving`. Директива берёт на себя то, в чём легче всего ошибиться, — под каким ключом писать, когда запись разрешена и как превратить недоверенные данные обратно в состояние, — а пишет через `KBQ_STATE_STORE`, обычную связку «ключ — JSON».

Подключайте её через `hostDirectives`, пробрасывая оба инпута, чтобы потребитель мог настроить сохранение прямо на компоненте, и получайте её через `inject()`, чтобы управлять. Больше ничего объявлять не нужно: директива не требует конфигурации.
...
Для собственного компонента это работает так же, как для нашего, — директива является строительным блоком, а не внутренней деталью.

Читайте один раз при инициализации, пишите при каждом изменении состояния, удаляйте через `clear()`. Записи до первого `read()` подавляются, поэтому привязка инпута, меняющая состояние во время обновления родительского шаблона, не может затереть сохранённое до того, как компонент его увидит.
...
Директива не решает, **что** сохранять и **когда** читать, — это остаётся за компонентом, единственным, кто знает своё состояние. По той же причине функция нормализации передаётся аргументом в `read()`.
...
**Не сохраняйте из оверлея.** У компонентов, создаваемых императивно в CDK-оверлее — сайдпанели, модальные окна, выпадающие меню, поповеры — нет стабильного ключа. Их состояние должен сохранять владеющий ими компонент. А вот был ли оверлей открыт — это состояние владельца, а не оверлея: сайдпанели, которой задали `stateSavingKey`, столько сохраняет `KbqSidepanelService`.
...
**Хранилище читается синхронно.** Компонент читает один раз при инициализации и ждать не может, поэтому хранилище, чей `getState` возвращает `Promise`, ничего не восстановит: нагрузка дойдёт до `normalize` неразрешённой и будет отвергнута, а запись при этом продолжит работать. В режиме разработки об этом предупреждают. Чтобы состояние жило на сервере, загрузите его до отрисовки приложения — `provideAppInitializer` — и отдавайте из памяти.

**Два компонента не должны делить ключ.** Ключ адресует запись целиком, а не пространство имён: компоненты с общим ключом затирают друг друга, а восстановит тот, кто прочитал первым, — то, что записал второй. Столкнуться может только явный `stateSavingKey`: выведенный ключ описывает положение в документе. В режиме разработки о занятом ключе предупреждают.

**`KBQ_STATE_SAVING_ENABLED` решает до первого чтения.** `KbqStateSavingService.setEnabled()` — переключатель времени выполнения, но компонент читает при инициализации, поэтому настройка, приехавшая позже, подействует со следующей отрисовки. Токен позволяет решить заранее: `false` выключает сохранение для всего приложения, а отдельные компоненты возвращаются в него через `[useStateSaving]="true"` — именно это обычно нужно тестовому приложению или странице с примерами.

#### Просмотр и управление сохранённым

Общий для приложения обзор даёт `KbqStateSavingService`. Он объявлен в корневом инжекторе, поэтому
`inject(KbqStateSavingService)` в любом месте возвращает один и тот же экземпляр.

Данные он берёт из двух источников, потому что ни один не покрывает другой. Реестр живых компонентов
говорит, кто сохраняет, под каким ключом и что держит сейчас, — в том числе компонент, которому через
`providers` подставили собственный `KBQ_STATE_STORE`, невидимый для корневого хранилища. Хранилище
говорит, что лежит на самом деле, включая записи, которые не принадлежат ни одному живому компоненту:
компонент, вокруг которого поменяли разметку, начинает писать под новым ключом, а о старой записи знает
только хранилище.

```ts
const stateSaving = inject(KbqStateSavingService);

stateSaving.components(); // кто сохраняет, под каким ключом и что держит
stateSaving.keys(); // все ключи хранилища
stateSaving.orphans(); // ключи без живого владельца

stateSaving.remove(key); // одна запись — и в корневом хранилище, и в собственном хранилище компонента
stateSaving.clearOrphans(); // только то, что больше никому не принадлежит
stateSaving.clear(); // всё

stateSaving.setEnabled(false); // выключить сохранение во всём приложении
```

Всё отдаётся снимками, а не сигналами. Компоненты регистрируются во время создания своего хоста, то есть
во время change detection, и записанный тогда сигнал будет прочитан уже проверенным представлением
(`NG0100`). Подпишитесь на `changes` — он срабатывает при регистрации, сохранении, очистке
и уничтожении компонента, при записи и удалении через сервис и когда хранилище сообщает о записи из
другой вкладки — и берите новый снимок в ответ.

`keys()` и `orphans()` опираются на необязательный `KbqStateStore.keys()`: хранилище, которое не умеет
перечислять записи, его не реализует, и тогда сервис показывает только живые компоненты.

`setEnabled(false)` — выключатель под настройку вида «не запоминать состояние интерфейса». Он забирает
сохранение сразу у всех компонентов, при этом собственный `useStateSaving` компонента всё равно должен
быть включён. Выключатель останавливает чтение и запись, но не удаляет уже сохранённое и не сворачивает
уже восстановленное: для первого есть `clear()`, для второго — перезагрузка. Очистку собственного
состояния компонента он намеренно не блокирует: удаление запомненного исполняет его смысл, а не
противоречит ему.

`write()` нужен для импорта настроек. Восстановление одноразовое и происходит при инициализации, поэтому
запись под ключом живого компонента дойдёт до него только после перезагрузки.
