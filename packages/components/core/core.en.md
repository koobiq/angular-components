...
| [Theming](/en/main/theming)                        | `ThemeService`, registered appearance variants and CSS variables                                                                                                     |
...
Component localization needs `KbqLocaleService` provided for `KBQ_LOCALE_SERVICE`; `KBQ_LOCALE_ID` selects the active locale.
...
`ThemeService` holds the registered appearance variants and the one currently applied. `KbqDefaultThemes` is the built-in pair, light and dark; `setThemes(items)` replaces the list with your own.

Switch with `setTheme(theme)` — or by index into `themes` — and read the active variant with `getTheme()`. `current` is a `BehaviorSubject`, so a subscriber is handed the active variant immediately and then every switch.

Switching moves the variant's `className` on `document.body`, which is what the design-system CSS variables key on.
...
| `MultipleMode`                                   | Multiple-selection mode for lists and trees                                                |
