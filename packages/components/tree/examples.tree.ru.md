...
В результат поиска входят совпадения и все их родители:

- дерево сжимается до совпадений и пути к ним: видны найденные элементы и цепочка их родителей
- путь до совпадения раскрыт, а вложенные в него элементы скрыты;
- совпавшие ветки можно развернуть и свернуть, чтобы просмотреть содержимое;
- совпавший фрагмент выделяется.
...
По клику на элемент разворачивается первый уровень дочерних узлов, выбрать можно только листовые элементы. Возможность выбора отключается атрибутом `selectable` у `kbq-tree-option`: при `[selectable]="false"` опция остается доступной для фокуса, навигации и разворачивания, но не выбирается.
