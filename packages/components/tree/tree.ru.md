...
Режим задаётся через `multiple`: `checkbox`, `keyboard` либо `single` / `false` / отсутствие атрибута для
одиночного выбора. Это обычный input, поэтому `[multiple]` можно связать и менять в любой момент. Если было
