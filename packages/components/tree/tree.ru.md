...
Дерево запоминает, какие узлы были раскрыты, и восстанавливает состояние после перезагрузки страницы. Включено по умолчанию — используйте `[useStateSaving]="false"` для отключения у компонента.
