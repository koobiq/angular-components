...
сообщает уровень вложенности элемента, его позицию среди соседей и то, раскрыт ли он, выбран ли и
отмечен ли.

- Дерево — это `role="tree"`, каждая строка — `role="treeitem"`.
- Табуляция останавливается на самом дереве, а элемент, на который переходят стрелки, сообщается через
  `aria-activedescendant` и не получает настоящий фокус.
- У раскрывающихся строк есть `aria-expanded`, у каждой строки — `aria-level`, `aria-posinset` и
  `aria-setsize`. Закрытая ветка в эти счётчики не входит: набор описывает только то, что на экране.
...
Строки переиспользуются между обновлениями данных, а ключом служит значение, которое возвращает
`treeControl.getValue`. Чтобы задать другой ключ, передайте функцию `trackBy`.
