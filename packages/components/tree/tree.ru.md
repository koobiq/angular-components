...
Дерево запоминает, какие узлы были раскрыты, и восстанавливает их при следующей отрисовке. Включено по умолчанию — передайте `[useStateSaving]="false"` для дерева, начальным состоянием которого распоряжается приложение.

<!-- example(tree-state-saving) -->

Выделение не сохраняется: оно принадлежит форме, к которой привязано дерево.

Раскрытие сохраняется по значению, которое возвращает `getValue` для узла, поэтому значение должно быть строкой, стабильной между перезагрузками и уникальной в пределах дерева. У дерева на `NestedTreeControl` метода `getValue` нет, и оно не сохраняет ничего — как и дерево, отрисованное внутри оверлея, в том числе в `kbq-tree-select`.

Раскрытие, выполненное самим приложением — `treeControl.expandAll()` или прямая запись в `expansionModel`, — не сохраняется, пока не вызвать `saveState()`. Удалить сохранённое: `clearSavedState()`.
