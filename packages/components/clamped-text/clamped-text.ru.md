...
Входной параметр `isCollapsed` позволяет управлять состоянием компонента из родительского контекста — например, из параметров маршрута. Параметр `isCollapsedChange` позволяет синхронизировать состояние обратно, образуя двустороннее связывание.
