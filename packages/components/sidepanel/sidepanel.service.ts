...
export class KbqSidepanelService implements OnDestroy {
    ...
    /**
     * Only the top sidepanel of a position stack carries the indent, however deep the stack is — the strip
     * is what reveals the panel underneath, and one per level is the visual noise the docs rule out.
     *
     * The flag belongs to the stack rather than to the panel that had one when it opened, so it is
     * recomputed on both ends: opening demotes the panel that was on top, and closing a *lower* sidepanel
     * would otherwise leave the top one exposing a strip of a panel that is no longer there.
     */
    private updateIndents(config: KbqSidepanelConfig) {
        const stack = this.getOpenedSidepanelsWithSamePosition(config);

        stack.forEach((sidepanelRef, index) =>
            sidepanelRef.containerInstance.setWithIndent(stack.length > 1 && index === stack.length - 1)
        );
    }
