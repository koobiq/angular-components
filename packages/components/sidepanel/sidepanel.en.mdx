...
The sidepanel closes on a left mouse button click only: the right and middle buttons open the browser's context menu or trigger its own actions and leave the panel alone. The [modal dialog](/en/components/modal) behaves the same way.

<Example id="sidepanel-overview" />
...
<Example id="sidepanel-disable-close" />
...
<Example id="sidepanel-normal-mode" />
...
<Example id="sidepanel-overlayed" />
...
<Example id="sidepanel-state-saving" />
