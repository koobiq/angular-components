...
Сайдпанель может запомнить, была ли она открыта, и вернуться после перезагрузки страницы. Чтобы это включить, передайте `stateSavingKey` в `KbqSidepanelConfig`; без ключа не сохраняется ничего.

Сайдпанель не восстановит себя сама: она существует, только пока открыта. `KbqSidepanelService` хранит флаг, а открывает панель заново приложение:
...
Записывается закрытие панели по её собственному поводу — крестиком, кликом по подложке, по Escape или через `KbqSidepanelRef.close()`. `closeAll()` и уничтожение сервиса флаг не трогают, поэтому открытая панель возвращается. Давайте каждой панели свой ключ, а забыть сохранённое — `clearSavedState(key)`.

Подумайте, что повторное открытие сделает со страницей: модальная сайдпанель вернётся вместе с подложкой и удержанием фокуса поверх страницы, которую читатель ещё не видел.
