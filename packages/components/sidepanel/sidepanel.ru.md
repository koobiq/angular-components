...
Сайдпанель может запомнить, была ли она открыта, и вернуться при следующем визите. Чтобы это включить, передайте `stateSavingKey` в `KbqSidepanelConfig`; без ключа не сохраняется ничего.
