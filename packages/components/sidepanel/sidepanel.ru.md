...
При открытой модальной сайдпанели фокус перемещается только по элементам внутри окна, включая крестик и кнопки в футере, и не выходит за его пределы. Немодальная сайдпанель оставляет страницу за собой интерактивной и поэтому фокус не удерживает; если ловушка фокуса нужна и в немодальном режиме, задайте `trapFocus: true` в `KbqSidepanelConfig`.
...
| <div style="min-width: 100px;">Клавиша</div>                                               | Действие                                                                                                                                      | Встроено |
| ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| <span class="docs-hot-key-button">Esc</span>                                               | Закрыть сайдпанель:<br>1. Если в сайдпанели не было изменения данных<br>2. Если у элемента в фокусе нет собственного обработчика этой клавиши | Да       |
| <span class="docs-hot-key-button">Ctrl</span> + <span class="docs-hot-key-button">↵</span> | Выполняется основное действие сайдпанели                                                                                                      | Нет      |
| <span class="docs-hot-key-button">↵</span>                                                 | Отправка формы по нажатию Enter в любом текстовом инпуте                                                                                      | Нет      |

Компонент сам обрабатывает только <span class="docs-hot-key-button">Esc</span>. Остальные две строки — рекомендация для содержимого сайдпанели: повесьте эти сочетания на свою форму или кнопки в футере.

#### Ограничения верстки

Открытая сайдпанель — это `position: fixed` с постоянным `transform` (именно он и обеспечивает выезд), поэтому она становится содержащим блоком для всех потомков с `position: fixed`. Все, что внутри панели ожидает привязки к границам вьюпорта, привяжется к самой панели. Для декоративных элементов внутри сайдпанели используйте `position: absolute` или `position: sticky`, а настоящие оверлеи выводите через контейнер оверлеев CDK.
