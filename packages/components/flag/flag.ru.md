...
**Проекция `<img>`.** Самый безопасный вариант: ничего не приходится обходить, а обрезку делает сам браузер.

```html
<kbq-flag decorative><img src="…/DE.svg" alt="" /></kbq-flag>
```

**Проекция инлайнового `<svg>`**, написанного прямо в шаблоне. Angular компилирует пространство имён SVG, поэтому ничего не вырезается и обход санитайзера не нужен.
...
**Входной параметр `svg`** — для флагов, которые приходят строкой: именно так их отдаёт `country-flag-icons`.
...
Обход здесь обязателен: тега `svg` нет в списке разрешённых элементов санитайзера Angular, поэтому обычная строка вырезается целиком и флаг не отображается. Отсюда и правило вокруг него.

**Через `bypassSecurityTrustHtml` можно передавать только разметку, известную на этапе сборки** — константу из пакета флагов или из собственного набора проекта. Разметку, пришедшую по сети, обходить нельзя: проверьте код страны и возьмите флаг из локального набора (см. «Если в пакете нет нужного флага»). Обход значения, на которое влияет запрос, превращает флаг в точку хранимого XSS.
...
Это основной формат — используйте его в большинстве сценариев: инлайн с текстом, списки, селекты. Флаги с другим соотношением (например, 4:3) поддерживаются через переопределение CSS-переменной `--kbq-flag-aspect-ratio`. Переменная объявлена с нулевой специфичностью, поэтому её достаточно переопределить обычным классом — без `!important` и дополнительных селекторов.

<!-- example(flag-custom-ratio) -->

Каким бы ни было соотношение сторон блока, флаг обрезается по нему — одинаково и для проецируемого `<img>`, и для инлайнового `<svg>`. Берите источник, близкий к нужному соотношению, чтобы обрезка была минимальной.
...
Флаг без `label` по умолчанию скрыт от экранных дикторов, поэтому недопустимый случай никогда не озвучивается как безымянная графика. Это подстраховка, а не замена разметке выше: по умолчанию скрывается и имя, заданное самим изображением (атрибут `alt` или `<title>` внутри SVG), — поэтому повторяйте его в `label`, если флаг несёт смысл.
