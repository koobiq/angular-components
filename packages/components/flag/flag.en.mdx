...
**A projected `<img>`.** The recommended form — nothing is bypassed, and the browser crops the image itself.
...
**The `svg` input.** Use it for flags that arrive as a string — this is what `country-flag-icons` exports.
...
The bypass is not optional here: `<svg>` is not on Angular's sanitizer allowlist, so a plain string is stripped down to nothing and the flag disappears.
...
This is the primary format — use it in most scenarios: inline with text, lists, selects. A flag with a different ratio (e.g. 4:3) takes the `aspectRatio` input, which accepts any CSS `aspect-ratio` value:
...
The input wins over the ratio the shape implies, so `shape="square"` together with an `aspectRatio` is no longer square. It also wins over the `--kbq-flag-aspect-ratio` CSS variable, which is still the way to set one ratio for a whole group of flags at once: the variable is declared at zero specificity, so a plain class rule redefines it — no `!important`, no extra selectors.
