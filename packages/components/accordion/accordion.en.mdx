...
<Example id="accordion-header" />

##### Inside the Content Area

This area can contain any type of content.

<Example id="accordion-content" />
...
<Example id="accordion-interactive-elements" />
...
<Example id="accordion-state-saving" />
...
A bound `[value]` wins over the persisted state, which wins over `defaultValue`; a section's own `[expanded]` applies only when none of them is set. `clearSavedState()` removes what is stored.
...
<Example id="accordion-in-section" />

#### Inside a Panel

<Example id="accordion-in-panel" />
