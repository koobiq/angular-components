...
export class KbqSearchExpandable implements ControlValueAccessor, AfterViewInit, OnDestroy {
    ...
    // Declared before `button` on purpose: view queries resolve in declaration order, and the
    // `button.changes` subscriber below reads `tooltip` — if it were declared after `button`, it
    // would still hold the previous (undefined) result at that point.
