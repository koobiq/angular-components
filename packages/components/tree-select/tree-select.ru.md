...
Аргумент `compareViewValues` у `FlatTreeControl` — подключаемый компаратор. Передайте вместо него `createSearchPredicate` из `@koobiq/components/core`, чтобы искать по нескольким словам. Поведение по умолчанию не меняется — это опциональное подключение. Подробнее — в гайде [Умный поиск](/ru/main/search-smart).
