...
Когда выбраны все, контрол может показывать особую подпись, а не буквально перечислять выбранные узлы. Спроецируйте `kbq-select-trigger`, пока `allOptionsSelected` дерева равно `true`, — без него tree-select вернётся к триггеру по умолчанию.
