...
Спроецированный `kbq-cleaner` сбрасывает выбор. Отключённые узлы не сбрасываются — по одному пользователь их
тоже снять не может. Когда больше ничего не выбрано, контрол скрывается. Чтобы задать, что именно очищать,
передайте `clearPredicate`.

Узел в свёрнутой ветке считается отключённым только через `isDisabled` у tree control, а не через атрибут
`[disabled]` у опции.
