...
По умолчанию поле растёт с каждой выбранной опцией, поэтому длинный выбор делает его высотой в несколько
строк. Свойство `multilineMaxRows` останавливает рост на заданном числе строк с тегами, а остальные теги
прокручиваются внутри поля — заодно выпадающий список остаётся рядом с полем, а не далеко под последней строкой.

Если панель не помещается ни под таким полем, ни над ним, а само поле выросло выше панели, панель прижимается
к первой строке поля и рисуется поверх остальных — там же, где её открыл бы однострочный селект. Первая
строка, шеврон и клинер остаются видимыми и кликабельными.
