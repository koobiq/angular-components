...
Кнопка сброса очищает все, кроме неактивных узлов. Эту логику можно изменить параметром `clearPredicate`: с `[clearPredicate]="clearEverything"` и полем `clearEverything = () => true` очищаются и неактивные.
...
Контрол объявлен как `role="combobox"`: он сообщает `aria-expanded`, указывает через `aria-controls` на открытый выпадающий список и отражает свою валидность и обязательность в `aria-invalid`/`aria-required`. При открытии списка фокус уходит внутрь него — на узел под клавиатурой либо в строку поиска, если она задана, — поэтому, пока список открыт, скринридер читает панель, а не сам контрол.

У комбобокса должно быть имя, и подпись `kbq-form-field` его не даёт: `<label for>` именует только нативные контролы. Без собственного имени контрол использует `placeholder`. Предпочитайте `aria-labelledby` со ссылкой на уже видимый текст, а `aria-label` задавайте, когда заголовка на странице нет:
