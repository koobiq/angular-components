...
Без поискового запроса чекбокс охватывает всё дерево, включая свёрнутые ветки: у него три состояния — снят, промежуточный и выбран, — а клик по промежуточному состоянию выбирает оставшиеся узлы, а не снимает выделение. Строка участвует в клавиатурной навигации как первый элемент дерева, и каждое действие по ней — клик или сочетание клавиш ниже — порождает событие `onSelectAll`. Подпись берётся из локали (`select.selectAll`).

<!-- example(tree-select-select-all) -->

При активном поисковом запросе чекбокс охватывает только узлы на экране: найденные и те родители, которые остались видимыми ради них.

<!-- example(tree-select-select-all-search) -->

Неактивные (`disabled`) узлы игнорируются: они не выбираются и не снимаются, а состояние чекбокса отражает только те узлы, которые пользователь действительно может переключить.
