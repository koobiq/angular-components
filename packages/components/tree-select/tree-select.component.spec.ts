...
describe('KbqTreeSelect', () => {
    ...
    describe('with search', () => {
        ...
        // A search field returns DOM focus to the input (ActiveDescendant), so the select scrolls the active
        // option in manually via `scrollTop` instead of leaning on the native scroll-into-view side effect of
        // `HTMLElement.focus()`, which Safari does not perform for an option that immediately loses focus back
        // to the input. jsdom reports zero geometry, so the option/container rects are stubbed to drive the
        // calculation. Each case would fail against the old `focus()`-only implementation.
        describe('scrolling the active option into view', () => {
            ...
            const activeOptionElement = () => (select as any).tree().keyManager.activeItem.getHostElement();
            ...
            beforeEach(fakeAsync(() => {
                ...
                searchInput = overlayContainerElement.querySelector('.search-input')!;

                Object.defineProperty(container, 'offsetHeight', { configurable: true, value: 256 });
                stubRect(container, 0, 256);

                select.tree()!.keyManager.setFirstItemActive();
                fixture.detectChanges();
            }));
