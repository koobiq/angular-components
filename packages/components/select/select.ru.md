...
Строки, которые селект выводит сам, — подпись «выбрать все», счётчик скрытых значений, доступные имена кнопки очистки и кнопок удаления тегов — следуют за активной локалью. Чтобы переопределить их у одного экземпляра, привяжите `localeConfiguration`; все ключи, которые вы не указали, продолжают следовать за локалью. Остальные способы переопределения описаны в [руководстве по локализации](/ru/guides/localization).
