...
Если значений много, атрибут `selectAll` добавляет над списком мастер-чекбокс, который одним действием выбирает все значения либо снимает выделение со всех. Работает только при множественном выборе.
...
Каждое действие по строке — клик или сочетание клавиш ниже — порождает одно событие `selectionChange` на всю пачку, после которого следует `onSelectAll`.
...
При поиске мастер-чекбокс действует только на результаты, попавшие под запрос, — скрытые фильтром опции не затрагиваются, а состояние чекбокса отражает только то, что видно на экране.

<!-- example(select-select-all-search) -->

Чекбокс игнорирует неактивные (`disabled`) опции: он их не выбирает и не снимает, а его собственное состояние отражает только те опции, которые пользователь действительно может переключить.
