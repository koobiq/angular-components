...
Если в качестве значений опций используются объекты, задайте `displayWith`, чтобы триггер (и теги в режиме мультивыбора) отображали человекочитаемую подпись выбранного значения даже тогда, когда DOM опции был переиспользован виртуальным скроллом или ещё ни разу не рендерился (например, при программной установке начального значения вне первоначального viewport).
