...
Отключённые опции не сбрасываются — по одной пользователь их тоже снять не может. Когда больше ничего не выбрано, контрол сброса скрывается. Чтобы задать, что именно очищать, передайте `clearPredicate`.

<!-- example(select-cleaner-with-disabled) -->

### Множественный выбор

В каком бы порядке ни выбирали значения, форма получает их в порядке панели — в том, в котором объявлены опции. Чтобы задать другой порядок, передайте `sortComparator`.
...
Строки, которые селект выводит сам, — подпись «выбрать все», счётчик скрытых значений, доступные имена кнопки очистки и кнопок удаления тегов — следуют за активной локалью. Чтобы переопределить их у одного экземпляра, привяжите `[localeOverrides]`; все ключи, которые вы не указали, продолжают следовать за локалью. Остальные способы переопределения описаны в [руководстве по локализации](/ru/guides/localization).

<!-- example(select-locale-configuration) -->

### Доступность

Селект объявляется как `combobox`, которому принадлежит `listbox`. `aria-expanded` следует за состоянием панели, а `aria-controls` указывает на список опций, пока он открыт. При открытии панели фокус переходит внутрь неё: на опцию, на которой стоят стрелки, — так она сама себя и озвучивает, — либо на поле поиска, если оно спроецировано.
...
Если нет ни того, ни другого, именем становится `placeholder` — сам по себе он отрисовывается обычным текстом и имени не даёт.
...
Пока в панели вместо опций показано что-то другое — состояние загрузки, ошибки, пустого списка или пустого результата поиска, — список опций помечен `aria-busy`, поэтому он не озвучивается как список опций, в котором ничего нет.

В режиме мультивыбора у каждого тега есть свой элемент удаления: `button`, срабатывающий по `Enter` и `Space`. Его имя собирается из a11y-локали (`remove`) и текста самого значения, поэтому озвучивается «Удалить Страна», а не просто «Удалить». Место в порядке табуляции получают только теги, которые триггер действительно показывает: те, за которые отвечает счётчик «+N», обрезаны и не видны, и остановка табуляции увела бы фокус на невидимое. Свой шаблон `#kbqSelectTagContent` заменяет эту разметку целиком — воспроизведите элемент удаления, если оставляете удаление тегов.
