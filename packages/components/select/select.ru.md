...
Если значений много, атрибут `selectAll` добавляет над списком мастер-чекбокс, который одним действием выбирает все значения либо снимает выделение со всех. При поиске мастер-чекбокс выбирает только результаты, попавшие под запрос. Работает только при множественном выборе.
...
Чекбокс игнорирует неактивные (`disabled`) опции: он их не выбирает и не снимает, а его собственное состояние отражает только те опции, которые пользователь действительно может переключить. Подпись берётся из локали (`select.selectAll`).

Каждое действие по строке порождает одно событие `selectionChange` на всю пачку, после которого следует `onSelectAll`.
