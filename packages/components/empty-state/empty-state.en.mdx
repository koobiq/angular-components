...
Each slot is matched by its own `ng-content` selector, and the component has no catch-all slot. A conditional slot therefore has to sit at the root of its `@if` block, where the selector can still see it:
...
Wrapping one control-flow block in another — an `@if (hasIcon())` whose only child is an `@if (wrapped()) … @else …` picking between two shapes of the illustration — puts a block, not an element, at the root. Nothing matches the selector, and the illustration is dropped from the DOM silently: no error, no warning, and `kbq-empty-state_has-icon` still on the host, because the directive is instantiated even though its element is never projected.

Give each shape its own `@if` at the same level (`@if (hasIcon() && wrapped())`, `@if (hasIcon() && !wrapped())`) instead of nesting them.
