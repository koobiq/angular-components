...
The tooltip can open next to the text caret in an editable field. Its position updates as the user types, moves the caret or scrolls the field. When text is selected, the tooltip anchors to the selection.

<Example id="tooltip-relative-to-caret" />

Set `kbqRelativeToCaret="true"` to enable this mode. Use `kbqRelativeToCaretVertical` to control the tooltip's vertical position: `field` places it above or below the whole field, while `line` places it next to the line containing the caret. By default, `auto` uses `field` for a single-line `input` and `line` for a `textarea` and `contenteditable`. Horizontally, the tooltip follows the caret in both modes. All tooltip placements remain available.

If the browser cannot determine the caret's position, the tooltip anchors to the field. This may happen with `password`, `email` and `number` inputs, right-to-left text, or centered and right-aligned text. When the tooltip is attached to a container, it uses the first editable element inside it.
