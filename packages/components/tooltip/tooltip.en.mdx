...
The field is the element the tooltip is attached to when that element is an `input`, a `textarea` or a `contenteditable`; otherwise it is the first such element inside. Fields that render something other than their value (`password` and the like) and right-to-left text fall back to the field itself.
