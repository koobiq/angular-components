...
У опции пайпов `select` и `multiselect` может быть вторая строка — добавьте `caption` к значению. Это место для технического идентификатора за понятным человеку именем.
...
Подпись видна только в выпадающем списке: в триггере, его тултипе и при клавиатурном наборе выводится одно лишь `name`, но в доступное имя опции подпись входит. Поиск ищет по ней наравне с именем — по тем же правилам [умного поиска](/ru/other/search-smart), что и сама панель. Если в шаблоне пайпа задан `valueTemplate`, `caption` игнорируется: шаблон владеет опцией целиком.

По умолчанию обе строки обрезаются. Задайте `multilineOptions: true` в шаблоне пайпа, чтобы они переносились; триггер пайпа при этом всегда остаётся однострочным. Пока имя не дорастёт до предельной ширины панели, оба режима выглядят одинаково — панель расширяется раньше, чем что-то обрежется.
