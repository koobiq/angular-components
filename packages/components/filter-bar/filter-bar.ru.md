...
Значение пайпа поиска — просто строка, которую панель фильтров возвращает через `filterChange`. Отфильтруйте свои данные этой строкой с помощью `createSearchPredicate` из `@koobiq/components/core`. Собственное сопоставление опций у пайпов `Select`/`MultiSelect` при этом не меняется. Подробнее — в гайде [Умный поиск](/ru/main/search-smart).
