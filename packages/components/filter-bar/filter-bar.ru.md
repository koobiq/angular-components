...
«Выбрать все» такие значения полностью игнорирует: их нельзя ни выбрать, ни развыбрать этим действием, а общий чекбокс отражает только те опции, которые пользователь реально может переключать. При этом под `selectedAllEqualsSelectedNothing` полный выбор по-прежнему коммитится как пустое `value` — неснимаемые значения там подразумеваются, а не перечисляются.
