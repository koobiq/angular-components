...
Счёт «восемь опций» верен для однострочных строк. Опция с подписью выше 32px, а при `multilineOptions` выше может стать любая, — поэтому в то же ограничение их поместится меньше.

<!-- example(filter-bar-panel-max-height) -->

### Подпись в опциях

У опции пайпов `select` и `multiselect` может быть вторая строка: добавьте `caption` к значению в `values`. Это место для технического идентификатора за понятным человеку именем — того самого, по которому фильтр в итоге применяется.

```ts
values: [
    { name: 'Тип угрозы', id: 'threat', value: 'threat', caption: 'category.generic' }
];
```

Подпись показывается только в выпадающем списке. В триггере пайпа и в его тултипе по-прежнему выводится одно лишь `name` — как и при клавиатурном наборе с переходом к опции, — но в доступное имя опции подпись входит, поэтому скринридер читает обе строки. Поиск по пайпу ищет и по подписи, и по имени — по тем же правилам [умного поиска](/ru/other/search-smart), что и сама панель: без учёта регистра и диакритики, слова запроса объединяются по И, а `"фраза в кавычках"` ищется целиком. Если в шаблоне пайпа задан `valueTemplate`, `caption` игнорируется полностью: шаблон владеет опцией целиком — подпись не рисуется и не участвует в поиске.

По умолчанию каждая из строк остаётся однострочной и обрезается. Задайте `multilineOptions: true` в шаблоне пайпа, чтобы имя и подпись переносились на столько строк, сколько нужно:
...
Опция растёт вместе с содержимым, поэтому список с переносами упирается в `panelMaxHeight` на меньшем числе строк. Триггера пайпа это не касается — он всегда остаётся однострочным; параметр не имеет отношения к `multiline` у селекта, который раскладывает выбранные значения в триггере по строкам. Остальные типы пайпов параметр игнорируют.

Ширина панели считается по содержимому, поэтому длинное имя сперва просто расширяет её, а не переносится и не обрезается, — вплоть до токена `--kbq-panel-size-width-max` (по умолчанию 640px). `multilineOptions` меняет то, что происходит дальше: без него каждая строка обрезается по этой границе многоточием, с ним — переносится, и опция становится выше. То есть до тех пор, пока имена не дорастут до предельной ширины, оба режима выглядят одинаково — а как раз с этого момента перенос и начинает быть нужен.
...
`filter-bar` запоминает, какой фильтр выбран и какие изменения в него внесли, и восстанавливает состояние после перезагрузки страницы. Включено по умолчанию — используйте `[useStateSaving]="false"` для отключения у компонента.

Это не то же самое, о чём раздел выше. Там пользователь сохраняет именованный фильтр, и хранит его приложение; здесь компонент хранит состояние на устройстве.

<!-- example(filter-bar-state-saving) -->

Восстановление пишет через модель `filter`, поэтому перекрывает значение, поданное привязкой `[filter]` на инициализации, и поднимает `filterChange` до того, как пользователь что-либо трогал. Выигрывает только изменение, сделанное после этого.

Проецируйте пайпы из того значения, которое отдаёт компонент: привяжите `[(filter)]` или присвойте то, что приходит в `(filterChange)`. Восстановление заменяет объект фильтра и каждый его пайп — ровно так же, как это делает выбор фильтра в `<kbq-filters>`, — поэтому пайп, спроецированный из собственного массива, это уже не тот пайп, что лежит в `filter`: его правка не доходит до сохранённого состояния, а кнопка удаления перестаёт срабатывать.

Значения возвращаются новыми объектами — ровно для этого и нужен `compareWith`, см. [Типы фильтров](#типы-фильтров). Фильтр опознаётся по `name`, поэтому тот, которого больше нет в `filters`, не восстанавливается. Пайп собирается заново из фильтра или из `pipeTemplates`, поэтому пайп, который приложение собрало само, минуя оба источника, не восстанавливается — положите его в `pipeTemplates`, если он должен возвращаться.

`clearSavedState()` удаляет сохранённое, `hasSavedState` говорит, есть ли оно. Ни то ни другое не связано с `saveFilterState()` / `restoreFilterState()`, которые снимают копию фильтра в памяти в пределах одной сессии.
