...
Значения возвращаются новыми объектами — ровно для этого и нужен `compareWith`, см. [Типы фильтров](#типы-фильтров). Фильтр опознаётся по `name`, поэтому тот, которого больше нет в `filters`, не восстанавливается.
