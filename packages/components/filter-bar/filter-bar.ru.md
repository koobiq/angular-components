...
`filter-bar` запоминает, какой фильтр выбран и какие изменения в него внесли, и восстанавливает то и другое при следующей отрисовке. Включено по умолчанию — передайте `[useStateSaving]="false"` для компонента, фильтром которого целиком распоряжается приложение.

Это не то же самое, о чём раздел выше. Там пользователь сохраняет именованный фильтр, и хранит его приложение; здесь компонент запоминает на этом устройстве, какой из них был в работе.
