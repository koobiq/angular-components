...
В выпадающем списке пайпов `select` и `multiselect` можно показать дополнительную подпись опции. Для этого добавьте к значению опции свойство `caption`, например с техническим идентификатором.

По умолчанию текст названия и подписи обрезается. `multilineOptions: true` включает перенос.
