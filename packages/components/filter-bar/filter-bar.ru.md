...
Опция с подписью и любая опция при `multilineOptions` выше 32px — их помещается меньше.

<!-- example(filter-bar-panel-max-height) -->

### Подпись в опциях

Добавьте к значению опции пайпа `select` или `multiselect` свойство `caption`, чтобы показать в выпадающем списке вторую строку — например, технический идентификатор.

По умолчанию обе строки обрезаются; `multilineOptions: true` включает перенос.
