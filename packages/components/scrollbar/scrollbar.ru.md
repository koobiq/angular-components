...
В режимах `hover` и `always` полоса рисуется только для оси, которую пользователь может прокрутить: у оси с вычисленным `overflow` `hidden`, `clip` или `visible` полосы нет, сколько бы содержимое ни выходило за границы.
