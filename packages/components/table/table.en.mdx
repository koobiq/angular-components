...
The header is painted with `--kbq-table-sticky-header-background`. It takes the page background by default; on a card, a modal or a sidepanel, point it at that surface — `--kbq-table-sticky-header-background: var(--kbq-background-card)`.
