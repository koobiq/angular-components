...
| `--kbq-form-field-states-autofill-placeholder`  | плейсхолдер — обычно скрыт, так как значение уже подставлено. Он появится, если очистить значение программно (например, через `<kbq-cleaner>`), пока для браузера поле остаётся полем с автозаполнением, поэтому он должен читаться не хуже обычного |
