...
@Component({
    ...
    host: {
        ...
        '[class.kbq-code-block_hide-line-numbers]': 'hideLineNumbers',
...
export class KbqCodeBlock implements AfterViewInit {
    ...
    @Input({ transform: booleanAttribute }) hideLineNumbers = false;
    ...
    @Input({ transform: booleanAttribute }) softWrap: boolean = true;
    ...
    protected downloadCode(): void {
        ...
        const fileName = this.hasExtensionInFilename(file) ? file.filename : `${file.filename}.txt`;
        link.setAttribute('download', fileName);
        link.click();
    }

    /** Whether the filename has an extension. */
    private hasExtensionInFilename({ filename }: KbqCodeBlockFile): boolean {
