...
Заголовка, который назвал бы диалог для скринридера, здесь нет, поэтому передавайте `kbqAriaLabel` — иначе диалог будет объявлен без имени.

<!-- example(modal-overview) -->

#### Диалог с заголовком и подписью

Используйте подпись, чтобы добавить контекст под заголовком диалога — например, кратко описать действие или его область применения. Подпись ограничена двумя строками: если текст длиннее, он обрезается многоточием.
...
Все эти способы закрывают диалог одинаково — и открытый через `KbqModalService`, и объявленный как `<kbq-modal [(kbqVisible)]>`. Сначала выполняются `kbqOnOk` и `kbqOnCancel`: привязанные как выходы, `(kbqOnCancel)`, они лишь уведомляют, и диалог закрывается; переданные функцией, которая возвращает `false` (или Promise с `false`), они оставляют его открытым.
...
- Фокус устанавливается на первый элемент диалога, доступный по Tab, если нет особых предписаний. У диалога с шапкой это крестик, поэтому нужный элемент помечайте атрибутом `autofocus` или `cdkFocusInitial` — оба важнее умолчания
...
- Если внутри модального окна много контента, есть прокрутка, то установка фокуса на первый доступный элемент может автоматически промотать содержимое окна и убрать его начальные части из поля зрения. Для этого случая задайте `kbqAutoFocus: 'dialog'`: фокус встанет на сам диалог — он доступен с клавиатуры и не прокручивает тело. Значение `'first-heading'` ставит фокус на заголовок, а `false` не трогает фокус вовсе, и тогда переносить его внутрь диалога должен вызывающий код

Диалог, внутри которого нет ни одного фокусируемого элемента, всё равно забирает фокус у триггера позади себя: фокус получает сам диалог.
...
| ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <span class="docs-hot-key-button">Ctrl</span> + <span class="docs-hot-key-button">↵</span> | Выполняется основное действие диалога                                                                                                                                  |
| <span class="docs-hot-key-button">Esc</span>                                               | Закрывает диалог так же, как кнопка «Отмена»: сначала выполняется `kbqOnCancel`, и он может отменить закрытие. Чтобы отключить клавишу, задайте `kbqCloseByESC: false` |
...
Если модальное окно открывается через `KbqModalService.open` с `kbqComponent`, для верстки диалога используйте директивы `kbq-modal-title`, `kbq-modal-body` и `kbq-modal-footer`. Чтобы добавить подзаголовок, разместите `kbq-modal-caption` внутри `kbq-modal-title` — он выводится в шапке диалога, под заголовком. Эти директивы сохраняют стандартную раскладку и тени при прокрутке: длинное тело диалога отбрасывает тень на шапку и футер точно так же, как в модалке, созданной через `KbqModalService.create`.

Заголовок и подзаголовок дают диалогу имя и описание — через `aria-labelledby` и `aria-describedby`, — поэтому сверстанный вручную диалог читается так же, как созданный через `KbqModalService.create`.

Если элемент управления должен стоять рядом с заголовком, а не в футере — кнопка настроек, счётчик, ссылка на документацию, — пометьте его атрибутом `kbqModalTitleActions` и разместите внутри `kbq-modal-title`. Он выводится рядом с заголовком, вне ограничения в две строки, которое действует на заголовок и подзаголовок, поэтому длинный заголовок не вытеснит его за пределы видимой области:
