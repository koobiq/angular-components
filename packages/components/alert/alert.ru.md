...
Проецируйте одну иконку статуса. Слот отрисовывает все попавшие в него `[kbq-icon]` и `[kbq-icon-item]`, но автоматически окрашивает только одну из них — `kbq-icon` имеет приоритет над `kbq-icon-item`, — поэтому любая следующая иконка сохранит тот цвет, который ей задали. При проекции обеих разновидностей в режиме разработки ещё и выводится предупреждение.
