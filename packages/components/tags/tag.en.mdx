...
<Example id="tag-fill-and-style" />

### Disabled State

<Example id="tag-disabled" />
...
The removal option is configured using the `removable` attribute (enabled by default). A tag that cannot be removed, because `removable` is off or the tag is disabled, does not show its remove icon.

<Example id="tag-removable" />
...
<Example id="tag-editable-with-validation" />
