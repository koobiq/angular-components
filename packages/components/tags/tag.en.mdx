...
The removal option is configured using the `removable` attribute (enabled by default). When removal is disabled, hide the remove icon as well: a cross that does nothing misleads the user.
