...
Спроецированный `kbq-cleaner` сбрасывает теги. Отключённые он не предлагает и скрывается, когда больше ничего не осталось. Удаление остаётся за вашим обработчиком — уберите теги, которые список отдал в `clearTargets`. Чтобы изменить отбор, передайте `clearPredicate`.
