...
Кнопка сброса удаляет все теги, кроме неактивных, и собственный обработчик ей не нужен: удаляет она через то же событие `removed`, что и иконка удаления внутри тега. Отбор можно изменить параметром `clearPredicate`: с `[clearPredicate]="() => true"` удаляются и неактивные. У списка с `removable="false"` кнопки сброса нет.
