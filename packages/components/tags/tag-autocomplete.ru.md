...
Опция удаления настраивается при помощи атрибута `removable` (по умолчанию включена). У неактивного тега иконка удаления не показывается, а у списка с `removable="false"` нет и кнопки сброса.

<!-- example(tag-autocomplete-removable) -->

Кнопке сброса собственный обработчик не нужен: она удаляет теги через то же событие `removed`, что и иконка удаления внутри тега. Какие теги она удаляет, задаёт параметр `clearPredicate`: с `[clearPredicate]="clearEverything"` и `clearEverything = () => true` очищаются и неактивные.
