...
- Клик по кнопке сброса — она удаляет все теги, кроме неактивных.
- Программное удаление через [API](/ru/components/tag/api) компонента.

Опция удаления настраивается при помощи атрибута `removable` (по умолчанию включена). У неактивного тега иконка удаления не показывается.

<!-- example(tag-autocomplete-removable) -->

### Сброс тегов

Кнопка сброса удаляет все теги, кроме неактивных, и собственный обработчик ей не нужен: удаляет она через то же событие `removed`, что и иконка удаления внутри тега. Отбор можно изменить параметром `clearPredicate`: с `[clearPredicate]="clearEverything"` и полем `clearEverything = () => true` удаляются и неактивные. У списка с `removable="false"` кнопки сброса нет.
...
Валидаторы вешаются на `<kbq-tag-list>`: контрол формы для всего списка — это именно `kbq-tag-list`, а поле ввода служит только для набора текста.

Отдельный `[formControl]` или `[ngModel]` на `<input kbqTagInputFor>` при этом остаётся нужен: `kbqAutocomplete` работает через него, и без него ломаются фильтрация меню и отслеживание набранного текста. Библиотека просто не учитывает валидаторы, навешанные на этот контрол.

Значение контрола — массив тегов, так что одним набором валидаторов проверяются и количество тегов, и содержимое каждого из них. Валидация одинаково распространяется на теги, выбранные из меню автодополнения, набранные вручную и вставленные из буфера обмена.

Валидаторы не запрещают добавить тег: тег добавляется, а поле переходит в состояние ошибки. Если некорректное значение не должно попадать в список, отфильтруйте его в обработчиках `(kbqTagInputTokenEnd)` и `(optionSelected)`.

Момент показа ошибки задаётся через [ErrorStateMatcher](/ru/other/validation) и по умолчанию наступает, когда контрол невалиден и при этом получил `touched` либо форма была отправлена.
