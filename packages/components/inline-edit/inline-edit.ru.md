...
Для одиночного селекта в режиме редактирования можно показывать только выпадающее меню без рамки поля ввода, а опцию «Не указано» внутри списка использовать вместо кнопки очистки.

<!-- example(inline-edit-select) -->

#### Многострочный селект

Если текст опции не помещается на одну строку, дайте триггеру и опциям переноситься вместо обрезания, а размер панели подстройте под ширину поля.
