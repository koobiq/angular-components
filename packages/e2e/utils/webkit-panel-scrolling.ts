...
/** What one pop-up panel has to name for the shared WebKit scrolling suite to drive it. */
export interface E2eWebkitPanelScrollingConfig {
    /** Component whose panel is under test, used in the suite title. */
    name: string;
    /** Route the panel lives on. */
    route: string;
    /** Opens the panel on that route. */
    open: (page: Page) => Promise<void>;
    /** The scrolling element inside the panel. */
    port: string;
    /** One row inside the scrollport. */
    item: string;
    /** The row the keyboard has moved to. */
    activeItem: string;
}

/**
 * WebKit-only guard for panel scrolling (DS-3299), shared by every component that opens one.
 *
...
export function e2eDescribeWebkitPanelScrolling(config: E2eWebkitPanelScrollingConfig): void {
    ...
    test.describe(`${name} panel scrolling`, () => {
        ...
        test.beforeEach(async ({ page }) => {
            ...
            await expect(getPort(page).locator(item).first()).toBeVisible();
        });
        ...
        test('does not move the list when the pointer lands on a partially clipped row', async ({ page }) => {
            const port = getPort(page);
            ...
            const offsets = await port.evaluate(async (element, sel) => {
                ...
                const portBox = element.getBoundingClientRect();
                const clipped = [...element.querySelectorAll<HTMLElement>(sel)].find((row) => {
                    const box = row.getBoundingClientRect();

                    return box.top < portBox.top && box.bottom > portBox.top;
                });
        ...
        test('brings the active row into view on keyboard navigation without scrolling the page', async ({ page }) => {
            const port = getPort(page);
            const itemCount = await port.locator(item).count();
            ...
            expect(await e2eIsFullyInView(port, activeItem)).toBe(true);
