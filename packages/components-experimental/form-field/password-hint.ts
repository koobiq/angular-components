...
import { KBQ_FORM_FIELD_REF, KbqFormFieldRef } from '@koobiq/components/core';
...
export class KbqPasswordHint extends KbqHint implements AfterContentInit {
    ...
    constructor(
        ...
        @Optional() @Inject(forwardRef(() => KBQ_FORM_FIELD_REF)) private formField: KbqFormFieldRef
    ) {
        super(elementRef);
    }
    ...
    ngAfterContentInit(): void {
        ...
        setTimeout(() => {
            (this.formField.control as KbqInputPassword).stateChanges.subscribe(this.checkValue);

            (this.formField.control as KbqInputPassword).checkRule.subscribe(() => {
