...
export class RelativeDateFormatterExample {
    ...
    private populateRelativeShort(locale: string) {
        ...
        short.beforeYesterdayNotCurrentYear = this.dateFormatter.relativeShortDate(
            now.minus({ years: 1, days: 2 })
        );
        short.beforeYesterdayCurrentYear = this.dateFormatter.relativeShortDate(now.minus({ days: 2 }));
        short.beforeYesterdayCurrentYearSeconds = this.dateFormatter.relativeShortDateTime(
        ...
        short.yesterdaySeconds = this.dateFormatter.relativeShortDateTime(
            now.minus({ days: 1 }), {seconds: true}
        );
        short.yesterdayMilliseconds = this.dateFormatter.relativeShortDateTime(
            now.minus({ days: 1 }), {milliseconds: true}
        );

        short.today = this.dateFormatter.relativeShortDate(now.minus({ hours: 1 }));
        short.todaySeconds = this.dateFormatter.relativeShortDateTime(
            now.minus({ hours: 1 }), {seconds: true}
        );
        short.todayMilliseconds = this.dateFormatter.relativeShortDateTime(
            now.minus({ hours: 1 }), {milliseconds: true}
        );

        short.tomorrow = this.dateFormatter.relativeShortDate(now.plus({ days: 1, hours: 1 }));
        short.tomorrowSeconds = this.dateFormatter.relativeShortDateTime(
            now.plus({ days: 1, hours: 1 }), {seconds: true}
        );
        short.tomorrowMilliseconds = this.dateFormatter.relativeShortDateTime(
        ...
        short.afterTomorrowCurrentYearSeconds = this.dateFormatter.relativeShortDateTime(
            now.plus({ days: 2 }), {seconds: true}
        );
        short.afterTomorrowCurrentYearMilliseconds = this.dateFormatter.relativeShortDateTime(
            now.plus({ days: 2 }), {milliseconds: true}
        );

        short.afterTomorrowNotCurrentYear = this.dateFormatter.relativeShortDate(
    ...
    private populateRelativeLong(locale: string) {
        ...
        long.beforeYesterdayNotCurrentYear = this.dateFormatter.relativeLongDate(
            now.minus({ years: 1, days: 2 })
        );
        long.beforeYesterdayCurrentYear = this.dateFormatter.relativeLongDate(now.minus({ days: 2 }));
        long.beforeYesterdayCurrentYearSeconds = this.dateFormatter.relativeLongDateTime(now.minus({ days: 2 }), {seconds: true});
        long.beforeYesterdayCurrentYearMilliseconds = this.dateFormatter.relativeLongDateTime(
            now.minus({ days: 2 }), {milliseconds: true}
        );
        ...
        long.yesterdayMilliseconds = this.dateFormatter.relativeLongDateTime(now.minus({ days: 1 }), {milliseconds: true});
        ...
        long.todayMilliseconds = this.dateFormatter.relativeLongDateTime(now.minus({ hours: 1 }), {milliseconds: true});

        long.tomorrow = this.dateFormatter.relativeLongDate(now.plus({ days: 1, hours: 1 }));
        long.tomorrowSeconds = this.dateFormatter.relativeLongDateTime(now.plus({ days: 1, hours: 1 }), {seconds: true});
        long.tomorrowMilliseconds = this.dateFormatter.relativeLongDateTime(now.plus({ days: 1, hours: 1 }), {milliseconds: true});

        long.afterTomorrowCurrentYear = this.dateFormatter.relativeLongDate(now.plus({ days: 2 }));
        long.afterTomorrowCurrentYearSeconds = this.dateFormatter.relativeLongDateTime(now.plus({ days: 2 }), {seconds: true});
        long.afterTomorrowCurrentYearMilliseconds = this.dateFormatter.relativeLongDateTime(
            now.plus({ days: 2 }), {milliseconds: true}
        );

        long.afterTomorrowNotCurrentYear = this.dateFormatter.relativeLongDate(
            now.plus({ years: 1, days: 2 })
        );
    }
}
