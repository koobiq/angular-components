    ...
    template: `
        <kbq-form-field>
            <kbq-tree-select
                ...
                (closed)="input.focus()"
            >
                <div kbq-select-matcher style="width: 100%" [useDefaultHandlers]="false">
                    <input
                        #input="kbqInput"
                        autocomplete="off"
                        kbqInput
                        type="text"
                        [(ngModel)]="selected"
                        (click)="handleClick(input, select)"
                        (keydown)="handleKeydown($event, select)"
                    />
                </div>
...
export class TreeSelectCustomMatcherWithInputExample {
    selected = 'docs';
    ...
    onSelectionChange($event: KbqTreeSelectChange) {
        ...
        console.log(`onSelectionChange: ${option.value}`);

        this.selected = option.value;
    }

    handleClick(input: KbqInput, treeSelect: KbqTreeSelect) {
        treeSelect.toggle();
    ...
    handleKeydown(event, treeSelect: KbqTreeSelect) {
        ...
        if (!treeSelect.disabled) {
            if (treeSelect.panelOpen) {
                ...
                treeSelect.triggerKeydownHandler(event);
            }
        }
    }
