import { ChangeDetectionStrategy, Component } from '@angular/core';
import {
    AbstractControl,
    FormControl,
    ReactiveFormsModule,
    ValidationErrors,
    ValidatorFn,
    Validators
} from '@angular/forms';
import { kbqDisableLegacyValidationDirectiveProvider } from '@koobiq/components/core';
import { KbqFormFieldModule } from '@koobiq/components/form-field';
import { KbqIconModule } from '@koobiq/components/icon';
import { KbqInputModule } from '@koobiq/components/input';
import { MaskitoDirective } from '@maskito/angular';
import { MaskitoOptions, MaskitoPlugin, maskitoUpdateElement } from '@maskito/core';
import {
    maskitoCaretGuard,
    maskitoEventHandler,
    maskitoNumberOptionsGenerator,
    maskitoTimeOptionsGenerator
} from '@maskito/kit';

/**
 * @title Input with mask
 */
@Component({
    selector: 'input-with-mask-example',
    providers: [kbqDisableLegacyValidationDirectiveProvider()],
    imports: [
        ReactiveFormsModule,
        MaskitoDirective,
        KbqFormFieldModule,
        KbqInputModule,
        KbqIconModule
    ],
    template: `
        <kbq-form-field>
            <kbq-label>Amount</kbq-label>
            <input kbqInput placeholder="1,000,000 $" [maskito]="currencyMask" [formControl]="currencyControl" />
            <kbq-hint>Format: 100,000.00$</kbq-hint>
            <kbq-hint>Use ↑/↓ to increase or decrease the value</kbq-hint>
            <kbq-cleaner />
        </kbq-form-field>

        <kbq-form-field>
            <kbq-label>Percent</kbq-label>
            <input kbqInput placeholder="0.00%" [maskito]="percentMask" [formControl]="percentControl" />
            <kbq-hint>From 0 to 100 %</kbq-hint>
            <kbq-hint>Use ↑/↓ to increase or decrease the value</kbq-hint>
            <kbq-cleaner />
        </kbq-form-field>

        <kbq-form-field>
            <kbq-label>Time</kbq-label>
            <i kbq-icon="kbq-clock_16" kbqPrefix></i>
            <input kbqInput placeholder="HH:MM:SS" [maskito]="timeMask" [formControl]="timeControl" />
            <kbq-hint>Format: HH:MM:SS</kbq-hint>
            <kbq-hint>Use ↑/↓ to increase or decrease the value</kbq-hint>
            <kbq-cleaner />
        </kbq-form-field>

        <kbq-form-field>
            <kbq-label>Phone</kbq-label>
            <input kbqInput placeholder="+7 (999) 123-45-67" [maskito]="phoneMask" [formControl]="phoneControl" />
            <kbq-hint>Format: +7 (999) 123-45-67</kbq-hint>
            <kbq-error>Invalid phone number</kbq-error>
            <kbq-cleaner />
        </kbq-form-field>

        <kbq-form-field>
            <kbq-label>IP address</kbq-label>
            <input kbqInput placeholder="192.168.0.1" [maskito]="ipAddressMask" [formControl]="ipAddressControl" />
            <kbq-hint>Format: 192.168.0.1</kbq-hint>
            <kbq-error>Invalid IP address</kbq-error>
            <kbq-cleaner />
        </kbq-form-field>

        <kbq-form-field>
            <kbq-label>MAC address</kbq-label>
            <input
                kbqInput
                placeholder="AA:BB:CC:DD:EE:FF"
                [maskito]="macAddressMask"
                [formControl]="macAddressControl"
            />
            <kbq-hint>Format: AA:BB:CC:DD:EE:FF</kbq-hint>
            <kbq-error>Invalid MAC address</kbq-error>
            <kbq-cleaner />
        </kbq-form-field>

        <kbq-form-field>
            <kbq-label>IPv6 address</kbq-label>
            <input
                kbqInput
                placeholder="2001:0db8:85a3:0000:0000:8a2e:0370:7334"
                [maskito]="ipv6AddressMask"
                [formControl]="ipv6AddressControl"
            />
            <kbq-hint>Format: 2001:0db8:85a3:0000:0000:8a2e:0370:7334</kbq-hint>
            <kbq-error>Invalid IPv6 address</kbq-error>
            <kbq-cleaner />
        </kbq-form-field>

        <kbq-form-field>
            <kbq-label>Port</kbq-label>
            <input kbqInput placeholder="8080" [maskito]="portMask" [formControl]="portControl" />
            <kbq-hint>Range: 0-65535</kbq-hint>
            <kbq-error>Invalid port number</kbq-error>
            <kbq-cleaner />
        </kbq-form-field>

        <kbq-form-field>
            <kbq-label>License key</kbq-label>
            <input kbqInput placeholder="XXXX-XXXX-XXXX-XXXX" [maskito]="licenseMask" [formControl]="licenseControl" />
            <kbq-hint>Format: XXXX-XXXX-XXXX-XXXX</kbq-hint>
            <kbq-error>Invalid license key</kbq-error>
            <kbq-cleaner />
        </kbq-form-field>
    `,
    styles: `
        :host {
            display: flex;
            flex-direction: column;
            gap: var(--kbq-size-s);
            align-items: center;
        }

        .kbq-form-field {
            width: 350px;
        }
    `,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class InputWithMaskExample {
    readonly currencyControl = new FormControl('123456.9');
    readonly percentControl = new FormControl('99.98');
    readonly timeControl = new FormControl('11:04:59');
    readonly ipAddressControl = new FormControl('192.168.0.1', [this.ipAddressValidator()]);
    readonly macAddressControl = new FormControl('AA:BB:CC:DD:EE:FF', [
        Validators.pattern(/^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$/)
    ]);
    readonly ipv6AddressControl = new FormControl('2001:0db8:85a3:0000:0000:8a2e:0370:7334', [
        Validators.pattern(/^([0-9A-Fa-f]{4}:){7}[0-9A-Fa-f]{4}$/)
    ]);
    readonly phoneControl = new FormControl('+7 (999) 123-45-67', [
        Validators.pattern(/^\+7 \(\d{3}\) \d{3}-\d{2}-\d{2}$/)
    ]);
    readonly portControl = new FormControl('8080', [this.portValidator()]);
    readonly licenseControl = new FormControl('ABCD-1234-EFGH-5678', [
        Validators.pattern(/^[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}$/)
    ]);

    readonly currencyMask = this.makeCurrencyMaskOptions();
    readonly percentMask = this.makePercentMaskOptions();
    readonly timeMask = this.makeTimeMaskOptions();
    readonly ipAddressMask = this.makeIPAddressMaskOptions();
    readonly macAddressMask = this.makeMACAddressMaskOptions();
    readonly ipv6AddressMask = this.makeIPv6AddressMaskOptions();
    readonly phoneMask = this.makePhoneMaskOptions();
    readonly portMask = this.makePortMaskOptions();
    readonly licenseMask = this.makeLicenseMaskOptions();

    private makeTimeMaskOptions(): MaskitoOptions {
        const options = maskitoTimeOptionsGenerator({
            mode: 'HH:MM:SS',
            step: 1
        });

        return {
            ...options,
            plugins: [
                ...options.plugins,
                maskitoEventHandler('blur', (element) => {
                    const [hh = '', mm = '', ss = ''] = element.value.split(':');

                    maskitoUpdateElement(element, [hh, mm, ss].map((segment) => segment.padEnd(2, '0')).join(':'));
                })
            ]
        };
    }

    private makePercentMaskOptions(): MaskitoOptions {
        const postfix = '%';
        const min = 0;
        const max = 100;
        const step = 0.01;
        const options = maskitoNumberOptionsGenerator({
            postfix,
            min,
            max,
            maximumFractionDigits: 2
        });
        const stepPlugin: MaskitoPlugin = (element) => {
            const handler = (event: KeyboardEvent) => {
                if (event.key !== 'ArrowUp' && event.key !== 'ArrowDown') return;

                event.preventDefault();

                const currentValue = parseFloat(element.value.replace(postfix, '')) || 0;
                let newValue = event.key === 'ArrowUp' ? currentValue + step : currentValue - step;

                newValue = Math.round(newValue * 100) / 100;
                newValue = Math.max(min, Math.min(max, newValue));

                maskitoUpdateElement(element, `${newValue}${postfix}`);
            };

            element.addEventListener('keydown', handler);

            return () => element.removeEventListener('keydown', handler);
        };

        return {
            ...options,
            plugins: [
                ...options.plugins,
                maskitoCaretGuard((value) => [0, value.length - 1]),
                maskitoEventHandler('blur', (element) => {
                    if (element.value === postfix) {
                        maskitoUpdateElement(element, `0${postfix}`);
                    }
                }),
                stepPlugin
            ]
        };
    }

    private makeCurrencyMaskOptions(): MaskitoOptions {
        const postfix = '$';
        const thousandSeparator = ',';
        const min = 0;
        const max = 999999999.99;
        const step = 0.01;
        const options = maskitoNumberOptionsGenerator({
            postfix,
            thousandSeparator,
            min,
            max,
            maximumFractionDigits: 2,
            minimumFractionDigits: 2
        });
        const stepPlugin: MaskitoPlugin = (element) => {
            const handler = (event: KeyboardEvent) => {
                if (event.key !== 'ArrowUp' && event.key !== 'ArrowDown') return;

                event.preventDefault();

                const rawValue = element.value.replace(postfix, '').replaceAll(thousandSeparator, '');
                const currentValue = parseFloat(rawValue) || 0;
                let newValue = event.key === 'ArrowUp' ? currentValue + step : currentValue - step;

                newValue = Math.round(newValue * 100) / 100;
                newValue = Math.max(min, Math.min(max, newValue));

                maskitoUpdateElement(element, `${newValue}${postfix}`);
            };

            element.addEventListener('keydown', handler);

            return () => element.removeEventListener('keydown', handler);
        };

        return {
            ...options,
            plugins: [
                ...options.plugins,
                maskitoCaretGuard((value) => [0, value.length - 1]),
                maskitoEventHandler('blur', (element) => {
                    if (element.value === postfix) {
                        maskitoUpdateElement(element, `0${postfix}`);
                    }
                }),
                stepPlugin
            ]
        };
    }

    private makeIPAddressMaskOptions(): MaskitoOptions {
        return {
            mask: ({ value }) => {
                const octets = value.split('.');

                return octets.flatMap((octet, index) => {
                    const digitCount = Math.max(1, Math.min(3, octet.length));
                    const digits = Array.from<RegExp>({ length: digitCount }).fill(/\d/);

                    return index < 3 ? [...digits, '.'] : digits;
                });
            }
        };
    }

    private makeMACAddressMaskOptions(): MaskitoOptions {
        const hexChar = /[0-9A-Fa-f]/;

        return {
            mask: [
                hexChar,
                hexChar,
                ':',
                hexChar,
                hexChar,
                ':',
                hexChar,
                hexChar,
                ':',
                hexChar,
                hexChar,
                ':',
                hexChar,
                hexChar,
                ':',
                hexChar,
                hexChar
            ],
            postprocessors: [
                ({ value, selection }) => ({ value: value.toUpperCase(), selection })
            ]
        };
    }

    private makeIPv6AddressMaskOptions(): MaskitoOptions {
        const hexChar = /[0-9A-Fa-f]/;

        return {
            mask: [
                hexChar,
                hexChar,
                hexChar,
                hexChar,
                ':',
                hexChar,
                hexChar,
                hexChar,
                hexChar,
                ':',
                hexChar,
                hexChar,
                hexChar,
                hexChar,
                ':',
                hexChar,
                hexChar,
                hexChar,
                hexChar,
                ':',
                hexChar,
                hexChar,
                hexChar,
                hexChar,
                ':',
                hexChar,
                hexChar,
                hexChar,
                hexChar,
                ':',
                hexChar,
                hexChar,
                hexChar,
                hexChar,
                ':',
                hexChar,
                hexChar,
                hexChar,
                hexChar
            ],
            postprocessors: [
                ({ value, selection }) => ({ value: value.toLowerCase(), selection })
            ]
        };
    }

    private ipAddressValidator(): ValidatorFn {
        return (control: AbstractControl): ValidationErrors | null => {
            const value = control.value;

            if (!value) return null;

            const octets = value.split('.');

            if (octets.length !== 4) return { invalidIp: true };

            const isValid = octets.every((octet: string) => {
                const num = parseInt(octet, 10);

                return !isNaN(num) && num >= 0 && num <= 255 && octet === String(num);
            });

            return isValid ? null : { invalidIp: true };
        };
    }

    private makePhoneMaskOptions(): MaskitoOptions {
        // Use phone addon for more complex cases:
        // https://maskito.dev/addons/phone
        return {
            mask: [
                '+',
                '7',
                ' ',
                '(',
                /\d/,
                /\d/,
                /\d/,
                ')',
                ' ',
                /\d/,
                /\d/,
                /\d/,
                '-',
                /\d/,
                /\d/,
                '-',
                /\d/,
                /\d/
            ]
        };
    }

    private makePortMaskOptions(): MaskitoOptions {
        return maskitoNumberOptionsGenerator({
            min: 0,
            max: 65535,
            maximumFractionDigits: 0,
            thousandSeparator: ''
        });
    }

    private makeLicenseMaskOptions(): MaskitoOptions {
        const alphanumeric = /[A-Za-z0-9]/;

        return {
            mask: [
                alphanumeric,
                alphanumeric,
                alphanumeric,
                alphanumeric,
                '-',
                alphanumeric,
                alphanumeric,
                alphanumeric,
                alphanumeric,
                '-',
                alphanumeric,
                alphanumeric,
                alphanumeric,
                alphanumeric,
                '-',
                alphanumeric,
                alphanumeric,
                alphanumeric,
                alphanumeric
            ],
            postprocessors: [({ value, selection }) => ({ value: value.toUpperCase(), selection })]
        };
    }

    private portValidator(): ValidatorFn {
        return (control: AbstractControl): ValidationErrors | null => {
            const value = control.value;

            if (!value) return null;

            const port = parseInt(value, 10);

            return !isNaN(port) && port >= 0 && port <= 65535 ? null : { invalidPort: true };
        };
    }
}
