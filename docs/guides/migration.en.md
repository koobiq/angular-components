...
12. **21.0.0**: button supported colors — a default color of its own per style.
13. **21.0.0**: the button-toggle review — ARIA semantics, keyboard navigation and signal inputs.
14. **21.0.0**: the form-field review — signals, accessibility and the removal of `mixinColor`.
15. **21.0.0**: the theme service review — signals, `auto` mode and built-in persistence.
16. **21.0.0**: explicit prefix and suffix slots for tag content.
17. **21.0.0**: deprecation of the overlayscrollbars-based Scrollbar implementation.
18. **21.0.0**: the locale layer typing — a typed `getParams`, partial locale data and signals.
19. **21.0.0**: `multiple` on the selection list and tree became a real, changeable input.
20. **21.0.0**: the component review — closed internals, signal inputs and the behavior fixes it uncovered.
21. **21.0.0**: removal of the deprecated file-upload `fileQueueChanged`/`fileQueueChange` outputs.
22. **21.0.0**: the accordion state store moved into `core`, shared by every component that persists state.
23. **21.0.0**: accordion state saving is on by default, keyed on the document instead of instantiation order.
24. **21.0.0**: tree state saving is on by default, keyed on the value the tree control gives each node.
25. **21.0.0**: tabs, sidebar and content-panel remember what the user changed, on by default.
26. **21.0.0**: the filter bar remembers the selected filter and its edits, on by default.
...
**Most unmigrated imports break loudly.** `KbqSplitterComponent`, `KbqSplitterAreaDirective`, `KbqGutterDirective`, `KbqGutterGhostDirective` and `Direction` live at the `/deprecated` path only, so the build stops with `TS2305: … has no exported member`.

**`KbqSplitterModule` is the silent one — it is the case this migration exists for.** Both entry points export a class under that name, and the `kbq-splitter` selector exists on both sides, so an unmigrated import keeps compiling but now pulls in the rewritten splitter. That one projects `kbq-splitter-panel` children instead of `[kbq-splitter-area]` hosts and takes `orientation` rather than `direction`, so the group renders with no panels and no separators.

Templates need no changes of their own: once the import points at `/deprecated`, `<kbq-splitter>` with `[kbq-splitter-area]` hosts, `direction`, `useGhost`, `hideGutters` and `gutterSize` keeps working exactly as before.
...
It rewrites the `@koobiq/components/splitter` specifier to `@koobiq/components/splitter/deprecated` across every `.ts` file, and running it twice changes nothing. It does not move you onto the new splitter: that is separate manual work — areas become `kbq-splitter-panel`, `direction` becomes `orientation`, and sizes move from `min-width` and CSS `flex` onto the `size` and `minSize` inputs.

### 9. Overlay demotion removal (21.0.0)
...
### 10. App-switcher upgrade (21.0.0)
...
### 12. Button supported colors (21.0.0)
...
### 13. Button-toggle review (21.0.0)
...
### 15. Theme service review (21.0.0)
...
### 16. Explicit tag content slots (21.0.0)
...
### 17. Scrollbar overlayscrollbars implementation deprecation (21.0.0)
...
### 19. List and tree multiple selection (21.0.0)
...
`checked`, `disabled`, `indeterminate` and `tabIndex` stay accessor inputs. They are two-way state — the component writes them on click and the `ControlValueAccessor` writes them through the `KbqCheckable` host directive — and a `model()` cannot carry the `booleanAttribute` / `numberAttribute` transform they need. Reads and writes of those four are unchanged in TypeScript; a template binding is coerced now, see below.
...
**Generated ids come from the CDK `_IdGenerator`.** The shape is unchanged for a default `APP_ID` - the CDK omits the app id unless it was overridden - but the counter is 0-based and shared per prefix, so the first checkbox is `kbq-checkbox-0` where it used to be `kbq-checkbox-1`. An assertion on an exact id needs updating; one matching the shape does not.
...
**`checked`, `big` and `indeterminate` are `booleanAttribute` inputs.** `<kbq-checkbox checked>` used to pass the empty string, which is falsy, so the valueless attribute did nothing. The same coercion runs on a bound value, so `[checked]="items.length"` on an empty list, `[checked]="0"` and `NaN` now read as `true` - and the accepted template type widens to `unknown`, so a binding that used to fail under `strictTemplates` is accepted and reads as `true`. `required` defaults to `false` instead of `undefined` behind a `boolean` type, and `value` reports `string | undefined` instead of `string`; `id` still reports `string`, since binding `null` falls back to the generated id on the read side too.
...
### 21. File-upload deprecated output removal (21.0.0)
...
### 22. Accordion state store moved to core (21.0.0)
...
### 23. Accordion state saving on by default (21.0.0)
...
### 24. Tree state saving on by default (21.0.0)
...
### 25. Tabs, sidebar and content-panel state saving on by default (21.0.0)
...
### 26. Filter-bar state saving on by default (21.0.0)
