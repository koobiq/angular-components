...
**Two fixed bugs change behaviour.** A failing password strength check used to call `setErrors({ passwordStrength: true })`, wiping every other error on the control — it now merges. And a `PasswordRules.Length` hint compared against `undefined` bounds, so a valid-length password was always reported as failing; the bounds now default to `0` and `Infinity`, and the guard only throws when neither `min` nor `max` is set.
