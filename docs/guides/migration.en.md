...
Nothing to run: every input kept its name, so there is no markup to rewrite. What changed is what the component does with them.

**The landmark is named from the locale.** The host wrote `aria-label="breadcrumb"` as a _binding_ — an English literal that also overwrote an `aria-label` the consumer had authored on the same element. The name comes from `KbqA11yLocaleConfiguration.breadcrumbs` now, so a non-English application renders a translated name where it used to render English, and `<nav kbq-breadcrumbs aria-label="…">` finally survives change detection, which is what makes two trails on one page distinguishable. The element form `<kbq-breadcrumbs>` also carries `role="navigation"` now; `<nav kbq-breadcrumbs>` is untouched, because it has the role already.

**`disabled` does what it says.** `KbqBreadcrumbs.disabled` was published and documented as blocking interaction, and read nowhere — no class, no attribute, every link still clickable. It now disables every item and the expand button and takes them out of the tab order. Markup that bound it and worked around it having no effect will see the trail go read-only.

**`firstItemNegativeMargin` applies in every wrap mode.** The offset hung off `.kbq-overflow-items`, a wrapper that only `wrapMode="auto"` renders, so it silently did nothing in `wrap` and `none`. It sits on the host now. A trail in one of those two modes that never set the property shifts left by `--kbq-breadcrumb-item-negative-margin` — the alignment it was always supposed to have; bind `[firstItemNegativeMargin]="false"` to keep the old position.

**<kbd>PageUp</kbd> and <kbd>PageDown</kbd> scroll the page again.** The roving-focus primitive mapped them to "first item" / "last item" and called `preventDefault()`, so a trail near the top of a long page swallowed the paging keys. <kbd>Home</kbd> and <kbd>End</kbd> still make those jumps.

Two DOM details that only matter to a stylesheet reaching into the component: the default separator is a plain `<i class="kbq-breadcrumb__separator">` instead of an empty `KbqIcon`, so it no longer carries `kbq-icon`/`kbq-contrast-fade`; and the rendered items are wrapped in a `role="list"` container.

| Member                                                           | Change                                                                         |
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| `RdxRovingFocusGroupDirective.dir`                               | `Direction \| null`, default `null` — unset means "inherit the document"       |
| `RdxRovingFocusGroupDirective.handleMouseDown` / `handleMouseUp` | Removed; focus entry no longer branches on what moved the focus                |
| `RdxRovingFocusGroupDirective.onFocusableItemAdd` / `...Remove`  | Removed; the count is derived from `focusableItems()`                          |
| `RdxRovingFocusItemDirective.active`                             | Defaults to `false`, so `data-active` marks one item instead of all of them    |
| `KbqBreadcrumbItem.text` / `.disabled`                           | Default to `''` / `false` instead of `undefined` cast to the non-nullable type |

`<a kbq-button kbqBreadcrumb>` no longer overwrites the button's `color` and `kbqStyle`: a `color` or `kbqStyle` bound next to `kbqBreadcrumb` is kept, and the button follows a surrounding `KbqButtonGroupRoot` again. Only markup that bound one of the two and expected it to be ignored sees a difference.
#### Checkbox
