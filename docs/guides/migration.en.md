...
`.kbq-modal-container` carries `role="dialog"`, `aria-modal="true"` and the focus trap now, and it is named by `kbqTitle` — or by the new `kbqAriaLabel` option when there is no title, which is the confirm and header-less case. A `kbqCaption` becomes `aria-describedby`. A manually composed dialog is named and described the same way, by its own `kbq-modal-title` and `kbq-modal-caption`, so the two entry paths are announced alike. While a dialog is shown, every body child that does not contain an overlay is marked `inert`, and so is every dialog below the topmost one, so code that reaches into the page behind an open dialog no longer takes effect.
...
**The inputs are still decorators, on purpose.** Unlike the rest of this review series, `KbqModalComponent` did not move to `input()`, so no consumer read needs an added `()`. `ModalBuilderForService` applies the whole options object with `Object.assign(this.modalRef.instance, inputs)`, and a bulk assignment overwrites an `InputSignal` rather than writing through it. `kbqOnOk` and `kbqOnCancel` are the harder half: each is `@Input()` and `@Output()` on one property, holding either an `EventEmitter` or a callback and branched on at runtime, which has no signal equivalent. `kbqVisible` would collapse cleanly into `model()` and will move when those two do.
