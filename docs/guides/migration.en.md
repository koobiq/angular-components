...
`KbqTableCellContent` is removed. It was declared `selector: 'kbq-table td'`, and Angular's directive selector grammar has element names, attributes, classes and `:not(…)` — but no combinators. The parser calls `setElement()` for every bare tag it meets, so the second one overwrote the first and the effective selector was plain `td`: every template that imported `KbqTableModule` got a directive instance plus a live content query on every cell it rendered, whether or not the table was a `kbq-table`. The `kbq-table` element the selector named has never existed either — the component is an attribute on a native `<table>`.
...
| `tr:hover { background: none }` | Bind `[disableHover]` or set `--kbq-table-states-hover-background: transparent` |

Row hover and the pill corner radii cover `<th>` inside `<tbody>` as well as `<td>`, so a row led by a row header is highlighted and rounded as a unit instead of from its second cell on. That is also why a consumer rule keyed on the row rather than the cell used to be a silent no-op.

Every color moved behind a `--kbq-table-*` token — `--kbq-table-header-text`, `--kbq-table-body-text`, `--kbq-table-border-color`, `--kbq-table-states-hover-background` and `--kbq-table-sticky-header-background` — and the row radius behind `--kbq-table-size-row-border-radius`. The sticky-header background defaults to `--kbq-background-card` rather than `--kbq-background-bg`, because a table is far more often placed on a modal, a sidepanel or a card, and in dark theme the two surfaces differ visibly. For a table directly on the page background, set `--kbq-table-sticky-header-background: var(--kbq-background-bg)`.

Cell padding and the corner radii are logical properties, alignment goes through `--kbq-table-cell-text-align` (default `start`), and `<tfoot>` gets the library color and typography it was already being padded for. A pinned header publishes its measured height as `--kbq-table-size-sticky-header-height`, which the table uses as `scroll-margin-block-start` on its cells and their focusable content, so a control scrolled to the top edge of the container does not land under the header (WCAG 2.2 SC 2.4.11). Add `scroll-padding-block-start` to your own scroll container if it holds more than the table.

Reported by `table-cell-content`.
