...
Выбор темы сохраняется между визитами — по умолчанию в `localStorage`, причём смена темы в одной вкладке подхватывается остальными открытыми вкладками приложения. Приложения, которые собираются на сервере, могут хранить его в куке: тогда сервер сразу знает, какую тему рисовать. Для этого подключите `KbqThemeCookieStore` через токен `KBQ_THEME_STORE` — он читает куку прямо из входящего запроса, больше на сервере ничего настраивать не нужно.
