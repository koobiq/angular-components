...
8. **21.0.0**: удаление механизма понижения оверлея.
9. **21.0.0**: переход API app-switcher на сигналы.
10. **21.0.0**: ревью кнопки — атрибуты хоста, владение в группе и стили.
11. **21.0.0**: поддерживаемые цвета кнопки — свой дефолтный цвет у каждого стиля.
12. **21.0.0**: ревью группы кнопок — ARIA-семантика, навигация с клавиатуры и сигнальные входы.
13. **21.0.0**: ревью поля формы — сигналы, доступность и удаление `mixinColor`.
14. **21.0.0**: ревью сервиса темизации — сигналы, режим `auto` и сохранение выбора из коробки.
15. **21.0.0**: явные prefix- и suffix-слоты для содержимого тегов.
16. **21.0.0**: устаревание overlayscrollbars-реализации Scrollbar.
17. **21.0.0**: типизация слоя локализации — типизированный `getParams`, частичные данные локали и сигналы.
18. **21.0.0**: `multiple` у списка и дерева стал обычным изменяемым input.
19. **21.0.0**: ревью компонентов — сокрытие внутренних членов, переход входов на сигналы и исправления поведения.
20. **21.0.0**: удаление устаревших событий file-upload `fileQueueChanged`/`fileQueueChange`.
21. **21.0.0**: хранилище состояния аккордеона переехало в `core` и стало общим для всех компонентов.
22. **21.0.0**: сохранение состояния аккордеона включено по умолчанию, ключ выводится из документа.
23. **21.0.0**: сохранение состояния дерева включено по умолчанию, ключ — значение узла из контрола.
24. **21.0.0**: вкладки, боковая панель и контент-панель запоминают выбор пользователя, по умолчанию.
25. **21.0.0**: `filter-bar` запоминает выбранный фильтр и его изменения, по умолчанию.
...
### 8. Удаление механизма понижения оверлея (21.0.0)
...
### 9. Переход API app-switcher на сигналы (21.0.0)
...
### 11. Поддерживаемые цвета кнопки (21.0.0)
...
### 14. Ревью сервиса темизации (21.0.0)
...
### 15. Явные слоты содержимого тегов (21.0.0)
...
### 16. Устаревание overlayscrollbars-реализации Scrollbar (21.0.0)
...
### 17. Типизация слоя локализации (21.0.0)
...
### 18. Множественный выбор в списке и дереве (21.0.0)
...
### 20. Удаление устаревших событий file-upload (21.0.0)
...
### 21. Хранилище состояния аккордеона переехало в core (21.0.0)
...
### 22. Сохранение состояния аккордеона включено по умолчанию (21.0.0)
...
### 23. Сохранение состояния дерева включено по умолчанию (21.0.0)
...
### 24. Сохранение состояния вкладок, боковой панели и контент-панели включено по умолчанию (21.0.0)
...
### 25. Сохранение состояния filter-bar включено по умолчанию (21.0.0)
