...
**Присваивания в `fillTextOff`, `compact` и `cleaner` больше не компилируются.** Это read-only сигналы — задавайте их привязкой в шаблоне. `KbqFormField.cleaner` был записываемым только из-за внутреннего костыля, которого больше нет.
...
**Две исправленные ошибки меняют поведение.** Провалившаяся проверка стойкости пароля вызывала `setErrors({ passwordStrength: true })`, стирая все остальные ошибки контрола, — теперь они объединяются. А подсказка с `PasswordRules.Length` сравнивала длину с `undefined`, поэтому пароль подходящей длины всегда считался неверным; границы теперь по умолчанию `0` и `Infinity`, а исключение бросается, только если не задан ни `min`, ни `max`.
