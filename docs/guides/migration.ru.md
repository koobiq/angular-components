...
`[innerHTML]` на `<kbq-flag>` больше не поддерживается как способ передать флаг. Компонент рендерит `<ng-content />`, поэтому проецируемые узлы попадают в хост-элемент, и запись `innerHTML` из родителя нацелена на тот же элемент. Уживались они только потому, что собственное представление компонента не добавляло DOM; в ревью у строкового варианта появился свой слот, поэтому `<kbq-flag [innerHTML]="flag" />` превращается в `<kbq-flag [svg]="flag" />`.

Само значение не меняется: санитайзер Angular по-прежнему вырезает `<svg>` без обхода, поэтому флаг, приходящий из пакета строкой, всё так же проходит через `DomSanitizer.bypassSecurityTrustHtml` — и только для разметки, известной на этапе сборки. Привязка `[innerHTML]` на элементе-обёртке, спроецированном _внутрь_ флага, не трогается: она никогда не конфликтовала с хостом.

Три изменения, для которых нечего мигрировать:

**Флаг без `label` скрыт от экранных дикторов.** Флаг без подписи и без `decorative` попадал в дерево доступности как безымянная графика — тот самый случай, который руководство компонента уже называло недопустимым. Теперь это состояние безопасно по умолчанию, но по умолчанию скрывается и имя, заданное самим изображением (атрибут `alt`, `<title>` внутри SVG), — повторяйте его в `label`, если флаг несёт смысл.

**Инлайновый `<svg>` обрезается, а не вписывается с полями.** `object-fit: cover` не действует на инлайновый `<svg>`: это не замещаемый элемент, поэтому источник с соотношением сторон, отличным от формы, рисовался внутри прозрачных полос — заметнее всего в `square` и `circle`. Тот же флаг, переданный как `<img>`, обрезался всегда, и теперь оба варианта выглядят одинаково.

**Токены флага объявлены с нулевой специфичностью, а `--kbq-flag-empty-background` стал непрозрачным нейтральным** вместо полупрозрачного состояния disabled. Переопределение, которое раньше проигрывало `.kbq-flag` по порядку следования, теперь выигрывает — лишний `!important` можно убрать, — а заглушка `empty` больше не перенимает оттенок поверхности под ней.

Закрывает `flag-inner-html`: привязку он переписывает за вас, остальное сообщает.
