...
`softWrap`, `viewAll`, `canDownload`, `activeFileIndex` и `files` теперь опираются на сигналы. Это входы на геттере и сеттере с теми же типами и теми же двусторонними событиями — в них пишет и сам компонент, и привязка, а `model()` не умеет нести нужную им трансформацию `booleanAttribute` / `numberAttribute`. Четыре из пяти раньше были обычными публичными полями, поэтому больше не являются собственными свойствами: их не видно в `Object.keys`, в spread и в `JSON.stringify`, а поле подкласса с тем же именем перекрывает геттер с сеттером при `useDefineForClassFields`.
