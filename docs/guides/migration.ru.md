...
7. **21.0.0**: удаление механизма понижения оверлея.
8. **21.0.0**: переход API app-switcher на сигналы.
9. **21.0.0**: ревью кнопки — атрибуты хоста, владение в группе и стили.
10. **21.0.0**: поддерживаемые цвета кнопки — свой дефолтный цвет у каждого стиля.
11. **21.0.0**: ревью группы кнопок — ARIA-семантика, навигация с клавиатуры и сигнальные входы.
12. **21.0.0**: ревью поля формы — сигналы, доступность и удаление `mixinColor`.
13. **21.0.0**: ревью сервиса темизации — сигналы, режим `auto` и сохранение выбора из коробки.
14. **21.0.0**: явные prefix- и suffix-слоты для содержимого тегов.
15. **21.0.0**: устаревание overlayscrollbars-реализации Scrollbar.
16. **21.0.0**: типизация слоя локализации — типизированный `getParams`, частичные данные локали и сигналы.
17. **21.0.0**: `multiple` у списка и дерева стал обычным изменяемым input.
18. **21.0.0**: ревью компонентов — сокрытие внутренних членов, переход входов на сигналы и исправления поведения.
19. **21.0.0**: удаление устаревших событий file-upload `fileQueueChanged`/`fileQueueChange`.
20. **21.0.0**: хранилище состояния аккордеона переехало в `core` и стало общим для всех компонентов.
21. **21.0.0**: сохранение состояния аккордеона включено по умолчанию, ключ выводится из документа.
22. **21.0.0**: сохранение состояния дерева включено по умолчанию, ключ — значение узла из контрола.
23. **21.0.0**: вкладки, боковая панель и контент-панель запоминают выбор пользователя, по умолчанию.
24. **21.0.0**: `filter-bar` запоминает выбранный фильтр и его изменения, по умолчанию.
...
**Атрибуты на хосте теперь выбираются по тегу.** До 21.0.0 отключённая кнопка рендерила `disabled` _и_ `aria-disabled="true"` независимо от того, чем был хост. На якоре `disabled` — невалидный атрибут, браузер его игнорировал, а `aria-disabled` на нативной `<button>` лишь повторял то, что уже сказано нативным атрибутом. Теперь каждый хост получает тот атрибут, который к нему применим:
...
**Кастомные проперти `--kbq-button-icon-size-vertical-padding` и `--kbq-button-icon-size-content-padding` удалены.** Их никто не читал и до 21.0.0, так что переопределение и раньше ни на что не влияло — просто удалите его. Иконочные кнопки используют `--kbq-button-icon-size-horizontal-padding` и `--kbq-button-size-content-padding`.
...
### 10. Поддерживаемые цвета кнопки (21.0.0)
...
### 13. Ревью сервиса темизации (21.0.0)
...
### 15. Устаревание overlayscrollbars-реализации Scrollbar (21.0.0)

До 21.0.0 `@koobiq/components/scrollbar` оборачивал стороннюю библиотеку `overlayscrollbars`: компонент `KbqScrollbar` (`kbq-scrollbar` / `[kbq-scrollbar]`) и низкоуровневая директива `KbqScrollbarDirective` (`[kbqScrollbar]`) со входами `options`, `events`, `defer` и сырым доступом к `scrollbarInstance`.

В 21.0.0 `@koobiq/components/scrollbar` — это новый, не зависящий от сторонних библиотек компонент `KbqScrollbar` с селектором `<kbq-scrollbar>` и другим публичным API. Прежняя реализация никуда не делась — она переехала без изменений в `@koobiq/components/scrollbar/deprecated` и будет удалена в одном из будущих мажорных релизов.
...
До 21.0.0 `multiple` у `kbq-list-selection` и `kbq-tree-selection` был статическим host-атрибутом, который
...
### 18. Ревью компонентов (21.0.0)

В 21.0.0 компоненты прошли полное ревью. Каждое ревью закрывало члены, которые никогда не были частью контракта компонента, переводило входы на сигналы там, где в этом и был его смысл, и попутно исправляло найденные ошибки поведения. Ниже перечислено только то, что доходит до потребителя, поэтому у компонента, чьё ревью ничего видимого снаружи не изменило, подраздела нет.
...
Большинство из них не переписывают код, а сообщают о местах вызова: чем заменить удалённый член или сигнальный вход — привязкой в шаблоне, другим членом или ничем — это решение, которое схематик принять не может. В каждом подразделе ниже назван закрывающий его схематик, если он есть, и указано, что именно он меняет за вас, если меняет. Чтобы получить отчёт ещё раз, запустите нужный отдельно:
...
`KbqAlert` завершил переход на сигналы и закрыл свойства, которые никогда не были частью его контракта. Привязки в шаблоне не тронуты — `[compact]`, `[alertStyle]` и `[alertColor]` работают ровно как раньше; изменились программный доступ и значение одного входа.

`alertColor` был несимметричной парой `get`/`set`: сеттер принимал цвет, а геттер возвращал построенный из него CSS-класс. Прочитав присвоенный `'error'`, вы получали `'kbq-alert_error'`, поэтому `alert.alertColor === KbqAlertColors.Error` не выполнялось никогда, а одно чтение с последующей записью клало класс в цвет и давало `kbq-alert_kbq-alert_error` — класс, которому не соответствует ни одно правило темы, из-за чего алерт терял фон. Теперь это входной сигнал только для чтения, и он отдаёт исходный цвет.

| Свойство                                                                           | Что было                   | Что стало                                                       |
| ---------------------------------------------------------------------------------- | -------------------------- | --------------------------------------------------------------- |
| `compact`                                                                          | `boolean`                  | `InputSignalWithTransform`, `booleanAttribute`                  |
| `alertStyle`                                                                       | `KbqAlertStyles \| string` | `InputSignal<'default' \| 'colored'>`                           |
| `alertColor`                                                                       | пара `get`/`set`           | `InputSignalWithTransform` только для чтения, значение изменено |
| `icon` / `iconItem` / `button` / `title` / `control` / `closeButton` / `isColored` | `public`                   | `protected`                                                     |

| Что было                                        | Что делать вручную                                                                              |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `.compact` / `.alertStyle`                      | Читать как вызов — `alert.compact()`; значение не изменилось                                    |
| `.alertColor`                                   | `alert.alertColor()` отдаёт `'error'`, а не `'kbq-alert_error'` — уберите разбор строки класса  |
| `.alertColor = …`                               | Вход только для чтения: задавайте его привязкой `[alertColor]`                                  |
| `[alertColor]="'danger'"`                       | Оба входа сужены до литералов своих перечислений, поэтому опечатка больше не компилируется      |
| Проецируемое содержимое внутри элемента-обёртки | Запросы слотов используют `descendants: false`: размещайте слоты прямыми детьми `<kbq-alert>`   |
| `A11yModule` / `PlatformModule` через модуль    | `KbqAlertModule` больше их не реэкспортирует — импортируйте их из `@angular/cdk` самостоятельно |

**`<kbq-alert compact>` теперь делает то, что написано.** Без трансформации `booleanAttribute` голый атрибут привязывал пустую строку, а она ложна, поэтому алерт оставался обычного размера. Разметка, в которой атрибут стоял для красоты, после обновления станет компактной.

Дополнения, для которых мигрировать нечего: событие `closed`, возникающее при активации спроецированной кнопки закрытия (алерт по-прежнему не скрывает себя сам), и `exportAs` у компонента и всех трёх директив. Три исправления достаются бесплатно: спроецированная иконка статуса окрашивается реактивно, а не один раз после инициализации содержимого, поэтому меняющийся `[alertColor]` больше не оставляет красную иконку на зелёном алерте; отступ иконки в обычном размере берётся из токена, соответствующего состоянию; а в светлой теме иконки предупреждения и успеха в варианте по умолчанию затемнены до минимума контраста 3:1 по WCAG для нетекстовых элементов.

В отличие от остального в этом разделе, ревью алерта вышло в 20.2.0, и `alert-signals` зарегистрирован на эту версию: `ng update`, проходящий через 20.2.0, запустит его, а проекту, который уже перешагнул эту версию, схематик нужно запустить вручную.

Закрывается схематиком `alert-signals`: чтения `compact` и `alertStyle` переписываются, остальное сообщается в отчёте.

#### Autocomplete

Автоматическую миграцию на сигналы пережили четыре входа на геттере и сеттере и один вход-приёмник записи — на панели и её триггере.

Особый случай — `classList`. Он был объявлен как `@Input('class')`, его сеттер накапливал имена классов в объекте, который привязывает шаблон панели, и попутно очищал `className` у хоста. Теперь это внутренний `computed` поверх сигнального входа `class` — `class="…"` на `<kbq-autocomplete>` работает ровно как раньше.

`isOpen` был несимметричным: `set isOpen(v)` сохранял флаг, а `get isOpen()` возвращал этот флаг **и** `showPanel`, поэтому запись `true` с последующим чтением давала `false`, если в панели не было опций. Теперь это `computed` — `attached() && showPanel()`, — а записываемая половина `attached` принадлежит триггеру.

| Что было                                  | Как мигрировать вручную                                                          |
| ----------------------------------------- | -------------------------------------------------------------------------------- |
| `.autoActiveFirstOption` / `.openOnFocus` | Читать как вызовы — переписывается за вас                                        |
| `.showPanel` / `.isOpen`                  | Читать как вызовы — переписывается за вас                                        |
| `.autocompleteDisabled` у триггера        | Читать как вызов — переписывается за вас                                         |
| `.showPanel = …`                          | `showPanel.set(…)` — переписывается за вас                                       |
| `.displayWith(value)`                     | `displayWith()(value)` — переписывается за вас                                   |
| `.isOpen = …`                             | `attached.set(…)`, хотя это собственное состояние триггера                       |
| `.displayWith = …` / `.openOnFocus = …`   | Это входы `input()`; привязывайте их в шаблоне                                   |
| `.classList`                              | Задавайте `class` на `<kbq-autocomplete>`; классы по-прежнему попадают на панель |

`displayWith` — единственный член, значение которого само является функцией, поэтому чтение и вызов теперь разделены: `displayWith()(value)`. Вызов с одной парой скобок вернёт саму функцию, а в шаблоне это выведет объект функции вместо подписи.

**`autoActiveFirstOption`, `openOnFocus` и `kbqAutocompleteDisabled` стали `booleanAttribute`-входами.** Для `autoActiveFirstOption` и `kbqAutocompleteDisabled` это совпадает с `coerceBooleanProperty`, который они и так использовали, поэтому ничего не изменилось. У `openOnFocus` приведения не было вовсе, и он меняется в обе стороны: атрибут без значения или `0` раньше читались как ложь, а теперь означают `true`, а `'false'` раньше читался как истина и теперь означает `false`.

**Привязка `[autoActiveFirstOption]` перекрывает `KBQ_AUTOCOMPLETE_DEFAULT_OPTIONS`** даже при значении `undefined` — и перекрывала до этого релиза, потому что старый сеттер приводил каждую запись привязки. Значение по умолчанию из токена применяется только к входу, который никто не привязал: чтобы решал токен, не привязывайте вход вовсе.

**Генерируемые `id` панели приходят из `_IdGenerator` в CDK.** При `APP_ID` по умолчанию формат не изменился (`kbq-autocomplete-0`, `kbq-autocomplete-1`, …); приложение или тест с собственным `APP_ID` получит его внутри — например, `kbq-autocomplete-a1` под TestBed. Это `id` элемента панели, поэтому селекторы и снимки, которые на него смотрят, может понадобиться поправить.

**`class` на `<kbq-autocomplete>` принимает всё то же, что и обычная привязка `[class]` в Angular** — строку, массив, `Set` или объект `{ имяКласса: boolean }`. У элемента нет статического атрибута `class`, поэтому значение привязки приходит во вход как есть; старый сеттер молча игнорировал всё, что не строка.

`options` остался контент-запросом `QueryList`: и `ActiveDescendantKeyManager`, и поток закрытия панели опираются на семантику его `changes`.

**Классы из атрибута `class` теперь заменяют друг друга на панели, а не накапливаются.** Старый сеттер сливал каждое полученное значение в объект, который никогда не очищал, поэтому привязка `[class]`, менявшаяся с `"a"` на `"b"`, оставляла панели оба класса.

Закрывается схематиком `autocomplete-signals`: чтения переписываются, остальное сообщается в отчёте.

#### Badge

`badgeColor` был опубликован как вход, сеттер которого принимал цвет, а геттер возвращал CSS-класс, — чтение никогда не совпадало с записью:
...
Теперь это сигнальный вход, и он возвращает тот цвет, который бейдж и показывает: пустое значение, `null` или `undefined` откатываются к `fade-contrast` прямо в трансформе входа, так что чтение не вернёт цвет, которого на хосте нет. Класс `kbq-badge_<color>` по-прежнему попадает на хост — из внутреннего `computed`, — поэтому стили и скриншоты не изменились: разницу видит только программное чтение.

| Что было                | Как мигрировать вручную                                                                          |
| ----------------------- | ------------------------------------------------------------------------------------------------ |
| `.compact` / `.outline` | Читать как `compact()` / `outline()` — переписывается за вас                                     |
| `.badgeColor`           | `badgeColor()`, и ожидать сырой цвет вместо `kbq-badge_<color>`                                  |
| `.badgeColor = …`       | Привязать `[badgeColor]` в шаблоне — вход доступен только на чтение                              |
| `.iconItem`             | Удалён; сам бейдж этот контент-запрос тоже не читал                                              |
| `KbqBadgeCssStyler.*`   | Стали `private`, а `isIconButton` удалён; контракт — классы отступов у иконок, которые он ставит |

**`compact` и `outline` теперь `booleanAttribute`-входы.** `<kbq-badge compact>` раньше передавал пустую строку, которая ложна, поэтому атрибут ничего не делал и бейдж оставался обычного размера; теперь он становится компактным. И наоборот, `[compact]="'false'"` — непустая строка, раньше истинная, — теперь означает `false`.

Закрывается схематиком `badge-signals`: чтения `compact` и `outline` переписываются, остальное сообщается в отчёте.
...
**Генерируемые `id` приходят из `_IdGenerator` в CDK.** При `APP_ID` по умолчанию формат не изменился - CDK не добавляет идентификатор приложения, если его не переопределяли, - но счётчик считается с нуля и общий на префикс, поэтому первый чекбокс теперь `kbq-checkbox-0`, а был `kbq-checkbox-1`. Проверку на конкретный `id` надо поправить, проверку на формат - нет.
...
**`checked`, `big` и `indeterminate` стали `booleanAttribute`-входами.** `<kbq-checkbox checked>` раньше передавал пустую строку, которая ложна, поэтому атрибут без значения ничего не делал. То же приведение работает и для привязанного значения: `[checked]="items.length"` на пустом списке, `[checked]="0"` и `NaN` теперь читаются как `true`, а принимаемый тип в шаблоне расширился до `unknown`, поэтому привязка, которая раньше падала под `strictTemplates`, принимается и читается как `true`. `required` по умолчанию `false`, а не `undefined` под типом `boolean`, а `value` возвращает `string | undefined` вместо `string`; `id` по-прежнему возвращает `string`, потому что привязанный `null` откатывается к сгенерированному значению и на чтении.
...
Теперь он возвращает `number | undefined`, а значение, которое не приводится к числу начисто — атрибут `maxHeight` без значения, `'200px'` — возвращает `undefined`, а не `NaN`. `[maxHeight]="undefined"` раньше давал `NaN`; это единственное изменение в рантайме. Места вызова, которые и так были неправильными, перестали компилироваться.
...
**У `KbqCodeBlock` не осталось входов на декораторе `@Input`.** `softWrap`, `viewAll`, `canDownload`, `files`, `activeFileIndex` и `hideTabs` - это `WritableSignal` поверх backing-входа, который несёт трансформацию `booleanAttribute` / `numberAttribute`. Привязки в шаблоне не меняются: `[softWrap]`, `[(viewAll)]` и бесзначный `<kbq-code-block softWrap>` работают как раньше. Меняется программный доступ: чтение становится вызовом, запись - `.set(…)`. `model()` был бы очевидной формой, но `ModelOptions` не несёт `transform`, а без него бесзначный атрибут передаёт пустую строку и молча выключает возможность.

| Что было                                    | Как мигрировать вручную                                                |
| ------------------------------------------- | ---------------------------------------------------------------------- |
| `.softWrap` / `.viewAll` / `.canDownload`   | Читать как вызов - переписывается за вас                               |
| `.files` / `.activeFileIndex` / `.hideTabs` | Читать как вызов - переписывается за вас                               |
| `.softWrap = …`                             | `.softWrap.set(…)` - переписывается за вас; составная форма сообщается |
| `.canLoad = …` / `.codeFiles = …`           | Привязать атрибут: оба стали backing-входами                           |
...
**`hideTabs` возвращает привязанное значение, а `tabsHidden()` - то, что происходит с заголовком.** Правило «единственный файл без `filename` прячет панель» теперь выводится, а не записывается в `hideTabs`, поэтому чтение самого входа его больше не подмешивает. `tabsHidden` публичен именно ради этого чтения.

**`canLoad` и `codeFiles` теперь подставляются, а не пишут.** Устаревшие алиасы писали в `canDownload` и `files`, поэтому побеждал тот из пары, который стоял в шаблоне позже. Теперь любой из двух атрибутов включает кнопку скачивания, а `codeFiles` применяется, пока `files` пуст.

**`activeFileIndex` за пределами `files` показывает первый файл, а пустой `files` не показывает кода вообще.** Раньше оба случая доходили до `files[activeFileIndex]` и падали на неопределённом результате — хватало `<kbq-code-block />` или `[files]="[]"`. Сам индекс остаётся таким, каким его привязали: сброс писал `activeFileIndexChange` обратно в `[(activeFileIndex)]`, пока родитель ещё обновлялся, и тот получал не тот файл, а при другом порядке привязок — `NG0100`.

**`hideTabs` теперь выводится, а не записывается.** Единственный файл без `filename` по-прежнему прячет панель вкладок, но компонент больше не пишет для этого `true` в собственный вход: запись защёлкивала панель на всю жизнь компонента, поэтому позже названные файлы её не возвращали и все файлы кроме первого оставались недоступны, а `hideTabsChange` срабатывал на каждое присваивание `files`. Отсюда два следствия: `[hideTabs]="false"` больше не показывает панель для одинокого безымянного файла — правило побеждает, как и обещало описание входа, — а чтение `hideTabs` возвращает то, что происходит с заголовком, тогда как `hideTabsChange` срабатывает только при изменении самой привязки.

**Помимо этого закрыты две давние утечки:** неудачная загрузка `highlight.js` больше не оставляет `pending` включённым до конца жизни страницы, а плагин нумерации строк ставит свой `<style>` и слушатель `copy` один раз, а не на каждый блок кода.

Закрывается схематиком `code-block-signals`: чтения и простые записи переписываются, остальное сообщается в отчёте.
...
**`<kbq-dl wide>` и `<kbq-dl wide="">` раньше ничего не делали.** Атрибут без значения передаёт пустую строку, которая ложна, — при том что `<kbq-dl resizable>` по соседству работал. Теперь он приводится, и атрибут означает `true`. Обе формы передают одно и то же значение, поэтому мигрируют одинаково.

**`<kbq-dl vertical>` не был бездействующим — он закреплял список горизонтальным.** Вход без приведения содержал `''`, а это не `null`, поэтому ветка с точкой перелома не выполнялась и список никогда не переключался на `verticalBreakpoint`. Поведение сохраняет перезапись в `[vertical]="false"`; удаление атрибута, наоборот, отдаёт решение точке перелома.

**`wide="false"` и `vertical="false"` меняют смысл на противоположный.** Непустая строка истинна, поэтому раньше они означали _`true`_, тогда как `booleanAttribute("false")` — это `false`. Это форма с самым заметным изменением отрисовки: `<kbq-dl vertical="false" wide="false">` раньше рисовался широким и вертикальным без разделителя, а теперь — ни широким, ни вертикальным, зато с видимым разделителем.
...
**`minWidth`, `dtMinWidth` и `ddMinWidth` стали числовыми входами** и возвращают `number | undefined` — именно это и содержал список без привязок. Числовой литерал ведёт себя ровно как раньше: `Math.max` приводит аргументы к числу, поэтому `<kbq-dl dtMinWidth="120">` давал `120` и до, и после изменения. Изменилось другое — значение, не являющееся конечным числом: `<kbq-dl dtMinWidth>` и `<kbq-dl dtMinWidth="abc">` раньше доходили до арифметики строкой и превращались в `0`, а теперь возвращают `undefined`, поэтому раскладка откатывается к измеренной ширине термина, а `?? fallback` на месте вызова срабатывает.

**`dtWidth` стал записываемым сигналом вместо `model()`.** Это единственный числовой вход, чьё значение уходит обратно к потребителю через `[(dtWidth)]`, а `model()` не принимает `transform`, поэтому `dtWidth="120"` без приведения доходил до раскладки строкой: `setDtWidth` начинается с `Number.isFinite`, который для строки ложен, так что значение пропускало зажим по `dtMinWidth` и колонка могла нарисоваться уже собственного минимума. Теперь это вход с алиасом плюс `linkedSignal` и явный выход `dtWidthChange`. `[(dtWidth)]` не изменился, чтение и запись `dtWidth()` тоже; поправить нужно только код, который типизировал член как `ModelSignal<number | null>` - там теперь `WritableSignal<number | null>`. Значение, не являющееся конечным числом, читается как `null` - состояние, возвращающее пропорции колонок по умолчанию.

**Привязки тоже меняют смысл**, и ошибки компиляции, которая бы на них указала, нет. `[vertical]="row.vertical"` со значением `undefined` оставляла вход `undefined`, а это не `null`, поэтому список оставался закреплённым горизонтальным; теперь там `null`, и решает точка перелома. У `wide` любое ложное значение, не являющееся булевым, меняет смысл на противоположный: `[wide]="items.length"` с `0` или `[wide]="label"` с пустой строкой были ложными, а теперь дают `true`. Числовая привязка, вычисляющаяся не в число, возвращает `undefined` вместо того, чтобы дойти до арифметики.

Сообщается схематиком `dl-attribute-coercion` — он покрывает и статические атрибуты, и привязки, и в `.html`-файлах, и в inline-шаблонах.

#### Dropdown

До потребителя ревью доходит в четырёх местах: ARIA-семантика триггера и элемента списка, сигнатура публичного метода `activateSafeArea()`, расчёт ширины панели и перевод входов, выходов и запросов компонента на сигналы.

`aria-expanded` — не глобальный ARIA-атрибут. Его нельзя ставить на элемент, роль которого вычисляется как `generic`: `<div>` или `<span>` без роли, `<a>` без `href`. А триггер вешается на тот элемент, который выбрал потребитель, — поэтому раньше атрибут нарушал `aria-allowed-attr`.

Теперь компонент публикует атрибут только там, где роль способна его нести: `<button>`, `<summary>`, `<a href>` или хост с собственной ролью. Все триггеры библиотеки к ним относятся. А ваш триггер без роли атрибут потеряет — вместе с селектором или тестом, который на него опирался.

Хост элемента списка не всегда активируется браузером сам. Для строки с дополнительным действием пакет документирует `<div>`, и до сих пор с клавиатуры такую строку было не нажать. Теперь она активируется клавишами Enter и Space: компонент воспроизводит нажатие клавиши как нажатие мышью, поэтому обработчик `(click)` и закрытие панели ведут себя так же, как при работе мышью.

Хосты, которые браузер активирует сам, компонент пропускает: `<button>` и `<a href>`. Пропускает он и элемент с ролью `menuitem` — там нажатие воспроизводит менеджер клавиш самого списка.

| Что было                                               | Что делать вручную                                                                                                                      |
| ------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| `activateSafeArea(owner, triangle, panelRect, onExit)` | Передавайте точку указателя и геттер прямоугольника: `activateSafeArea(owner, { x, y }, () => element.getBoundingClientRect(), onExit)` |
| Наследник, переопределяющий `handleActionKeydown`      | Переопределяйте `handleKeydown`: он обрабатывает клавиши Enter и Space и делегирует случай Tab                                          |
| `.kbq-dropdown-item_with-icon` в селекторе             | Класса больше нет — выбирайте проецируемую иконку                                                                                       |
| `[aria-expanded]` на триггере без роли                 | Задайте хосту `role` либо уберите селектор                                                                                              |
| Обработчик нажатия на `.kbq-dropdown__panel`           | Панель обрабатывает собственное закрытие на `.kbq-dropdown__content`                                                                    |

**Панель больше не может вырасти за пределы собственной области.** Токен `--kbq-dropdown-size-container-width-min` откатывался к статическим `200px` везде, где вход не задавал его явно. В том числе при явном `panelWidth`: `[panelWidth]="150"` рисовал панель шириной `200px` в области шириной `150px`.

Теперь при явном `panelWidth` и при `panelMinWidth: null` токен схлопывается в `0px`. Панель, которая опиралась на этот минимум, не запрашивая его, станет уже — задайте `panelMinWidth`, чтобы его сохранить.

Изменения, для которых нет места вызова:

- Фокус возвращается на триггер, только если на момент закрытия он всё ещё был внутри оверлея. Если вы нажали снаружи, фокус останется на том элементе, по которому вы нажали.
- Клавиши Home и End двигают подсветку в выпадающем списке без поля поиска — раньше они прокручивали страницу.
- Нажатие на элемент управления внутри `kbqDropdownFooter` больше не закрывает панель.
- Отключённые элементы публикуют `aria-disabled`.
- Соседний элемент, задетый указателем по пути к вложенному меню, остаётся с фоном по умолчанию. Правило для этого было и раньше, но его селектор не совпадал ни с чем.
- `<kbq-dropdown>` сохраняет классы, которые не были перенесены на панель: вход `class` раньше очищал хост-элемент целиком, унося с собой и привязку `[class.x]` от другой директивы.
- Пакет экспортирует `KbqDropdownItemActionHost` и `KBQ_DROPDOWN_ITEM_ACTION_HOST`, а прямые реэкспорты `fadeInItems` и `transformDropdown` объявлены устаревшими в пользу `kbqDropdownAnimations`.

**Двадцать из двадцати одного входа, выхода и запроса компонента стали сигналами.** Выпадающий список был последним компонентом ревью с входами-декораторами. У семнадцати из них чтение стало вызовом:
...
Привязки не изменились. `[xPosition]`, `class`, `[kbqDropdownTriggerFor]` и остальные сохранили имена. Изменился только программный доступ.

Семь свойств — это `model()`, поэтому запись в них стала `.set(…)`: `xPosition`, `yPosition`, оба флага перекрытия, `offsetX`, `openByArrowDown` и `restoreFocus`. Они доступны на запись, потому что их меняет другой компонент у того экземпляра, который ему передали: `kbq-split-button` и `kbq-navbar-item` сами позиционируют панель, а кнопка действия у элемента списка отключает возврат фокуса. У `model()` нет `transform`, поэтому эти семь больше не приводят строковый атрибут к типу: пишите `[overlapTriggerX]="true"`, а не `overlapTriggerX="true"`.

Остальные доступны только на чтение: запись в них больше не компилируется, а не проходит незаметно, как раньше.

Изменения, о которых схематик сообщает, но которые не исправляет:

| Что было                                    | Что делать вручную                                                                                      |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `panel.items.changes` и `.first`            | `items` — сигнал обычного массива: возьмите `toObservable(panel.items)` или сам массив `items()`        |
| `panel.items.reset(…)`                      | Вызывайте `adoptItems(…)`                                                                               |
| `closed.pipe(…)` и `dropdownClosed.pipe(…)` | Оба стали `output()` — оберните их в `outputToObservable(panel.closed)`                                 |
| `panel.closed.complete()`                   | Метода больше нет: Angular закрывает подписку вместе с панелью                                          |
| `panel.classList`                           | Теперь `protected`. Это один `computed`; свои классы задавайте через `class` на `<kbq-dropdown>`        |
| Класс, реализующий `KbqDropdownPanel`       | Изменились все свойства. Необязательный `adoptItems` заменяет `items.reset(…)`, который вызывал триггер |

На месте осталось одно свойство: `KbqDropdownItem.textElement` — это по-прежнему `@ViewChild`, потому что он реализует `KbqTitleTextRef`, а также его реализуют ещё пять компонентов.

`KbqOptionActionParent` в `@koobiq/components/core` описывает триггер структурно, поэтому TypeScript не сверяет его с настоящим классом. Здесь он приведён в соответствие: `restoreFocus` стал `WritableSignal<boolean>`, а `dropdownClosed` — `OutputRef<void>`. Тому, кто реализует этот контракт вручную, нужно сделать то же самое: расхождение проявится во время работы, а не при компиляции.

`ListKeyManagerOption.disabled` теперь принимает `boolean | Signal<boolean>`. Свой элемент списка, который читает `item.disabled` как обычное свойство, на сигнальном элементе сломается: сигнал — это функция, поэтому все элементы читаются как отключённые. Пропуск отключённых стрелками и поиск по первым буквам перестанут работать, и никакой ошибки при этом не будет.

Закрывается схематиком `dropdown-signals`; строки в двух первых таблицах — ручные шаги.
...
| `.ngAfterContentInit`           | Больше не реализован; классы иконок следуют за content-запросом сами               |
...
**Заблокированная ссылка несёт `aria-disabled` вместо `disabled`.** На `<a>` и `<span>` атрибут `disabled` невалиден, и скринридер всё равно доходил до ссылки через список ссылок и объявлял её рабочей. Класс `kbq-disabled` и `tabindex="-1"` не изменились; если вы опирались на селектор `a[kbq-link][disabled]`, используйте `.kbq-disabled` или `[aria-disabled]`.

**Правило `kbq-link_print` наконец доходит до ссылки.** Блок `@media print` разрешался в селектор потомка, поэтому печатаемый URL не появлялся ни у кого. Теперь печать страницы со связанным `[print]` дописывает URL после текста ссылки, как и описано.
...
**`transparent` теперь `booleanAttribute`-вход, и он меняется в обе стороны.** `<kbq-loader-overlay transparent>` раньше передавал пустую строку, которая ложна, поэтому атрибут без значения давал _заливку_ — противоположность тому, как он читается; `[transparent]="0"` и `NaN` читались как ложь так же. Теперь все они означают `true`. И наоборот, `[transparent]="'false'"` был непустой строкой, то есть означал `true`, а теперь означает `false`. Это единственное изменение здесь, которое не даёт ошибки компиляции, поэтому схематика сообщает про каждый файл с атрибутом без значения.
...
`markdownText` был единственным входом компонента, и его сеттер выполнял отрисовку — именно поэтому автоматическая миграция на сигналы его пропустила. Теперь готовый HTML — это `computed`, а вход — обычный `input()`.

| Что было                        | Как мигрировать вручную                                               |
| ------------------------------- | --------------------------------------------------------------------- |
| `.markdownText`                 | Читать как `markdownText()` — переписывается за вас                   |
| `.markdownText = …`             | Привязать `[markdownText]` в шаблоне — вход доступен только на чтение |
| `resultHtml.set(…)` в подклассе | Стал read-only `computed` — задавайте `markdownText`                  |

Два исправления поведения следуют из проверки `if (value && …)`, которая была в старом сеттере.

**Очистка `markdownText` теперь очищает вывод.** Сеттер перерисовывал содержимое только для истинного значения, поэтому возврат к `null` или `''` навсегда оставлял на экране предыдущий HTML.

**Спроецированное содержимое стало постоянным запасным вариантом.** `<kbq-markdown>`, который одновременно проецирует содержимое и привязывает `[markdownText]`, возвращается к спроецированному содержимому каждый раз, когда вход пуст, а не только в первый раз. Спроецированный текст перечитывается при каждом изменении, поэтому содержимое, которое появляется уже после первого показа - например, под `@if`, - тоже подхватывается.

Закрывается схематиком `markdown-signals`: чтения `markdownText` переписываются, остальное сообщается в отчёте.

#### Notification center

**Адаптер дат должен быть доступен в корневом инжекторе.** Раньше `KbqNotificationCenterModule` перечислял `KbqNotificationCenterService` в своих `providers`, поэтому сервис создавался в том инжекторе, куда импортировали модуль, и подхватывал `DateAdapter`, объявленный там же. Модуль больше не предоставляет сервис — остался единственный экземпляр с `providedIn: 'root'`, — поэтому адаптер, объявленный на функциональном модуле или на компоненте, до него не доходит, и первое же внедрение падает с `NG0201 No provider found for DateAdapter`. Объявляйте адаптер при старте приложения:
...
| Что было                                                                     | Что делать вручную                                                                                                                                                                 |
| ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `KBQ_NOTIFICATION_CENTER_SCROLL_STRATEGY_FACTORY_PROVIDER`                   | Больше не экспортируется, импорт падает с `TS2305`. `KBQ_NOTIFICATION_CENTER_SCROLL_STRATEGY` и `kbqNotificationCenterScrollStrategyFactory` на месте — соберите провайдер вручную |
| `onReload.emit()` / `onNextPage.emit()` / `onDelete.emit(…)`                 | Все три потока теперь `Subject`, метода `.emit()` нет — вызывайте `.next()`                                                                                                        |
| `trigger.backdropClass` / `panelClass` / `offset` / `scrolledToBottomOffset` | Сигнальные входы: чтение стало вызовом, а запись — привязкой в шаблоне, у `input()` нет `.set()`                                                                                   |
| `service.changes.subscribe((state) => …)`                                    | `changes` — это `Observable<void>`, то есть просто сигнал «что-то изменилось». Подписчику всегда приходит `undefined`                                                              |

`KbqReadStateDirective`, которую элемент уведомления подключает через `hostDirectives`, переименовала свои обработчики: теперь они названы по тому, что измеряют, а не по вызывавшим их событиям — `mouseenterHandler()` → `startDwell()` и `mouseleaveHandler()` → `endDwell()`. Оба новых имени принимают необязательный аргумент канала со значением по умолчанию «указатель», поэтому переименованный вызов значит ровно то же, что и раньше. `timestamp` стал геттером только для чтения и возвращает `number | undefined` — время начала самого раннего незавершённого удержания, либо `undefined`, пока хост простаивает.

Директива теперь измеряет и удержание с клавиатуры, причём каналы отслеживаются независимо: удержание заканчивается, только когда хост покинули оба. Хост, удерживающий фокус дольше `timeToRead`, будет помечен прочитанным, даже если указатель его не касался, а уход указателя больше не завершает удержание, которое всё ещё держит фокус.

Сообщает `notification-center-signals`. Переименование обработчиков выполняет за вас `read-state-dwell-handlers`.
...
Поверхность подписи триггера переехала на сигнальные входы, а внутренности оверлея закрыты. Все входы сохранили свои алиасы, поэтому `[hiddenItemsText]` и `[hiddenItemsTextFormatter]` привязываются ровно как раньше — изменился только программный доступ.

| Член                       | Было                            | Стало                              |
| -------------------------- | ------------------------------- | ---------------------------------- |
| `hiddenItemsText`          | пара геттер/сеттер              | `InputSignal<string \| undefined>` |
| `hiddenItemsTextFormatter` | переопределяемый метод          | вход, хранящий функцию             |
| `overlayDir`               | public                          | `protected`                        |
| `triggerRect`              | public                          | `protected`                        |
| `onRemoveMatcherItem`      | `$event: any`                   | `$event: Event`                    |
| `selectEvents`             | экспорт из `core/select/events` | удалён вместе с модулем            |

| Что было                       | Что делать вручную                                                                  |
| ------------------------------ | ----------------------------------------------------------------------------------- |
| `.hiddenItemsText = …`         | Привязать `[hiddenItemsText]` — у `input()` нет `.set()`                            |
| `.hiddenItemsText`             | Читать как `hiddenItemsText()`, тип — `string \| undefined`                         |
| `.hiddenItemsTextFormatter`    | `hiddenItemsTextFormatter()(template, count)`; переопределение становится привязкой |
| `.overlayDir` / `.triggerRect` | Стали `protected`; используйте API открытия/закрытия и входы панели                 |
| `selectEvents`                 | Удалить импорт — это была одна константа, значение которой равнялось её же имени    |

**В режиме множественного выбора порядок значений теперь задаёт панель.** `sortValues()` всегда был описан как сортировка «по порядку в панели», но компаратор по умолчанию — `a.value - b.value` — этого порядка не давал никогда: на строковых значениях вычитание даёт `NaN`, а его сортировка считает за «равно», поэтому выбор просто сохранял порядок, в котором значения пришли; на числовых сортировка шла по самому числу и совпадала с панелью только тогда, когда опции объявлены по возрастанию. Теперь по умолчанию берётся настоящий индекс опции в панели. Значению, которому не соответствует ни одна опция в панели (`KbqVirtualOption` при виртуальном скролле или `showPreselectedValues`), места в этом порядке нет, поэтому оно встаёт после всех значений, у которых опция есть.

Порядок виден в трёх местах: в массиве, который уходит в форму, в тегах триггера и в том, какая опция подсвечивается при открытии панели. Если хост сам пересортировывал полученное значение в порядок панели, эту сортировку можно убрать. Чтобы оставить другой порядок, привяжите `[sortComparator]`: `(a, b) => a.value - b.value` воспроизводит прежнее поведение для числовых значений, а `() => 0` — для всех остальных типов, потому что компаратор, объявляющий всё равным, оставляет порядок поступления нетронутым.

Ещё два исправления, для которых мигрировать нечего: подписка на локаль, созданная в конструкторе, не отписывалась, и сервис из корневого инжектора держал каждый созданный и уничтоженный селект всё время жизни приложения; а сам селект теперь несёт ARIA-роли combobox/listbox/option и управляемую с клавиатуры кнопку удаления тега, поэтому написанные вручную `role` и `aria-*` на хосте стали дубликатами.

Сообщает `select-signal-inputs`.
...
То, что осталось геттером и сеттером, осталось по причине, которую задаёт код, а не из-за трансформации: backing-вход несёт `booleanAttribute`, а сигнал поверх него и читается как сигнал. `KbqTagList` реализует `KbqFormFieldControl`, а `KbqTagInput` — `KbqTagTextControl`; они объявляют `value`, `id`, `placeholder`, `required` и `disabled` обычными свойствами. `KbqTag.disabled` остался обычным `boolean`, потому что менеджер фокуса пропускает элементы, читая его как значение, а сигнал — это функция, она всегда истинна. `disabled`, `tabindex` и `value` у тега и `draggable`, `tabIndex` у списка подмешивают состояние, которое сигнал отследить не может: form control списка, чей `disabled` — обычное свойство, и проецированный текст. У них синтаксис чтения и записи не изменился.

Всё остальное переехало:

| Что было                                                        | Как мигрировать вручную                                                                       |
| --------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `.addOnBlur` / `.separators` у tag input                        | Читать как вызовы — переписывается за вас                                                     |
| `.selected` / `.editable` / `.selectable` / `.removable` у тега | Читать как вызовы — переписывается за вас                                                     |
| `.removable` у списка тегов                                     | Читать как вызов — переписывается за вас                                                      |
| `.separatorKeyCodes = …` / `.tagList = …` у tag input           | Привязать `[kbqTagInputSeparatorKeyCodes]` / `[kbqTagInputFor]`; зато теперь оба можно читать |
| Любая другая запись в переехавший член                          | Привязать атрибут; о каждой форме записи сообщается                                           |
| `tag.selected = …`                                              | Вызвать `select()`, `deselect()` или `toggleSelected()`                                       |

**`KbqTag.selected` доступен только для чтения.** Смена привязки `[selected]` по-прежнему вызывает `selectionChange`, как это делал старый сеттер, — список тегов на него подписан, — так что привязка работает ровно как раньше. Из кода то же событие вызывают `select()`, `deselect()` и `toggleSelected()`. Метода `.set()` нет, потому что запись в обход события оставила бы список тегов в неведении.

**`distinct` стал `booleanAttribute`-входом.** Атрибут `distinct` без значения раньше передавал пустую строку, которая ложна, поэтому дубликаты тегов всё равно принимались.

**Генерируемые `id` берутся из CDK `_IdGenerator`**, а не из счётчика на уровне модуля. Формат при `APP_ID` по умолчанию не изменился: CDK опускает идентификатор приложения, когда он равен `ng`, а счётчик по-прежнему начинается с 0, поэтому реальное приложение так же получает `kbq-tag-list-0` и `kbq-tag-list-input-0`. Идентификатор попадёт в `id`, только если `APP_ID` задан явно, — сразу перед счётчиком и без разделителя: `kbq-tag-list-input-myapp0`. Список тегов сообщает `id` своего input, если тот есть, поэтому наружу через form-field выходят оба.

Закрывается схематиком `tags-signals`: чтения переехавших членов переписываются, в том числе через переменные ссылок в шаблонах, а о каждой записи сообщается в отчёте.
...
Особый случай — `canGrow`: его геттер возвращал `!maxRowLimitReached && привязанное значение`, поэтому он сообщал `false`, как только textarea упирался в `maxRows`, — хотя потребитель просил рост. Свёрнутое значение стало внутренним computed `growing`, а `canGrow()` возвращает то, что было привязано. На пределе строк элемент остаётся со своей полосой прокрутки, а нативная ручка не появляется: `kbq-textarea_max-row-limit-reached` ставит `resize: unset`, и это правило идёт в стилях после `kbq-textarea-resizable`, побеждая по порядку.

| Что было                                                | Как мигрировать вручную                                                                                                                                                    |
| ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.maxRows` / `.maxRowLimitReached`                      | Читать как вызовы — переписывается за вас                                                                                                                                  |
| `.freeRowsHeight`                                       | `freeRowsHeight()`, и ожидать `undefined` без привязки — сообщается                                                                                                        |
...
| `.grow()` / отвязанная ссылка на `.grow`                | Теперь `protected`; textarea сам пересчитывает высоту при каждом изменении значения и входов, а изменение раскладки, которого он не видит, покрывает `stateChanges.next()` |

**`maxRows` и `freeRowsHeight` возвращают `number | undefined`.** Оба были объявлены ненулевыми, хотя непривязанный `maxRows` содержал `undefined`, и `maxRowLimitReached` сравнивался именно с ним: `rowsCount > undefined` ложно — благодаря этому неограниченный рост вообще работал. С `freeRowsHeight` иначе: `ngOnInit` записывал в этот вход измеренную высоту строки, поэтому непривязанное чтение возвращало число, как только отрабатывал первый тик. Теперь подстановка внутренняя, а вход остаётся `undefined`, и место вызова с `gap + 'px'` начинает выдавать `"undefined" + "px"` без единой диагностики — такие чтения схематик сообщает, а не переписывает.
...
**Генерируемые `id` берутся из CDK `_IdGenerator`**, а не из счётчика на уровне модуля. Формат при `APP_ID` по умолчанию не изменился: CDK опускает идентификатор приложения, когда он равен `ng`, а счётчик на префикс по-прежнему начинается с 0, поэтому реальное приложение так же получает `kbq-textarea-0`. Идентификатор попадёт в `id`, только если `APP_ID` задан явно; счётчик теперь общий на префикс, а не на модуль.
...
| `.min` / `.max`                                                       | `min()` / `max()` — сообщается, а не переписывается: чтение без вызова всегда истинно, а значение — привязанное, а не разобранная дата |
| `.format = …` / `.min = …` / `.max = …` / `.kbqValidationTooltip = …` | Привязать в шаблоне; входы доступны только на чтение                                                                                   |

**Повторно привязанный `kbqValidationTooltip` больше не накапливает подписки.** Сеттер подписывался на `incorrectInput` при каждом срабатывании, поэтому после того, как вход привязывали несколько раз, одно отклонённое нажатие открывало все когда-либо привязанные подсказки. Теперь это `effect` с очисткой, и заодно отвязанная подсказка получает обратно свой триггер и задержку — сеттер оставлял её на триггере `manual` без обработчиков.
...
**Генерируемые `id` берутся из CDK `_IdGenerator`**, а не из счётчика на уровне модуля. Формат при `APP_ID` по умолчанию не изменился: CDK опускает идентификатор приложения, когда он равен `ng`, а счётчик по-прежнему начинается с 0, поэтому реальное приложение так же получает `kbq-timepicker-0`. Идентификатор попадёт в `id`, только если `APP_ID` задан явно, — сразу перед счётчиком и без разделителя: `kbq-timepicker-myapp0`.
...
Раньше `KbqToastComponent` держал `readonly service = inject(KbqToastService)` и обращался через него ко всему сервису. Теперь он получает только узкий контракт `KbqToastStack`, который действительно использует, — через новый токен `KBQ_TOAST_STACK`, предоставляемый и `KbqToastService`, и `KbqToastContainerComponent`. Наследник, зарегистрированный через `KBQ_TOAST_FACTORY`, — это задокументированная точка расширения, поэтому именно такой код ломается чаще всего:
...
| Что было                                                             | Что делать вручную                                                                                                                                               |
| -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.service`                                                           | `close()` для закрытия либо `inject(KBQ_TOAST_STACK)` для остального                                                                                             |
| `.elementRef`                                                        | `inject<ElementRef<HTMLElement>>(ElementRef)`                                                                                                                    |
| `.ttl` / `.delay`                                                    | Обратный отсчёт переехал в сервис — передавайте время жизни аргументом `duration` метода `show()`                                                                |
| `.isTemplateRef()` / `.themePalette` / `.toastStyle`                 | Удалены: они существовали только ради прежнего способа отрисовки                                                                                                 |
| `.style` / `.icon` / `.role` / `.closeButton`                        | Стали `protected`. `data`, `id`, `hovered`, `focused`, `animationState`, `close()` и `onAnimation()` по-прежнему публичны                                        |
| `service.animation.getValue()` / `.value`                            | Теперь это `Subject<AnimationEvent>`, а не `BehaviorSubject`: позднему подписчику ничего не воспроизводится                                                      |
| Компонент из `KBQ_TOAST_FACTORY`, не наследующий `KbqToastComponent` | Отклоняется: `show()` читает числовой `id`, по которому сервис ведёт свой стек, и бросает исключение вместо добавления тоста, который потом нельзя будет закрыть |

**Переданные данные больше не изменяются.** Раньше `show()` подставлял значения по умолчанию — `style` в `contrast` и `icon` в `true` — записывая их прямо в объект вызывающего кода; теперь оба значения вычисляются внутри тоста. Если что-то читало эти ключи обратно после `show()` или выводило тот же объект в другом месте (именно так делает `kbq-notification-center`), значения по умолчанию нужно подставлять самостоятельно.

Ещё три изменения поведения, для которых мигрировать нечего. Автоматическое закрытие приостанавливается, пока на тосте указатель или фокус; у всего стека один общий тик, который останавливается на пустом стеке, а тост, показанный через `showTemplate()`, таких слушателей не несёт и не приостанавливается. Каждый тост — это live-регион: `role="alert"` для стилей ошибки и предупреждения, `role="status"` в остальных случаях, — а сам стек стал именованным `role="region"`, название которого берётся из нового ключа `toastRegion` раздела `a11y` данных локали. Фокус передаётся дальше только при закрытии с клавиатуры — на следующий тост или обратно на элемент, который держал его в момент появления тоста; при закрытии мышью фокус остаётся там, куда его поставил браузер.

Сообщает `toast-stack-and-defaults`.

#### Tooltip

**`ignoreTooltipPointerEvents` теперь по умолчанию `false`.** Раньше значением по умолчанию было `true`, из-за чего панель любого тултипа была прозрачной для указателя. Это нарушает WCAG 1.4.13 _Content on Hover or Focus_: до подсказки, которую нужно читать какое-то время, невозможно дотянуться, а пользователь с сильным увеличением, у которого указатель то и дело оказывается над панелью, её теряет.

Смена значения молчаливая. Разметка, которая про этот вход ничего не знала, компилируется как прежде, но ведёт себя иначе — панель начинает перехватывать клики, предназначенные тому, над чем она висит. Добавьте `[ignoreTooltipPointerEvents]="true"` там, где панель накрывает кликабельный элемент.

Уже отключено, и делать ничего не нужно: встроенные подсказки для обрезанного текста (`kbq-title`, `kbqEllipsisCenter`, подсказки опций и таймзон) и любой тултип с `kbqTrigger` в значении `manual` или `none` — наведение на такую панель её не открывает и не закрывает, поэтому защищать там нечего.
...
| Член                                                                                            | Было              | Стало                           |
| ----------------------------------------------------------------------------------------------- | ----------------- | ------------------------------- |
| `valueChange`                                                                                   | объявленный выход | удалён                          |
| `getPanelClasses()`, `getPanelTheme()`, `isRtl()`, `transformOrigin`                            | public            | удалены                         |
| `hiddenItemsText`, `hiddenItemsTextFormatter`                                                   | геттер / метод    | сигнальные входы                |
| `hiddenItems`, `colorForState`                                                                  | обычные поля      | `WritableSignal` / `Signal`     |
| `options`, `tags`, `overlayDir`, `triggerRect`, `panelDoneAnimatingStream`, `changeDetectorRef` | public            | `protected`                     |
| `KbqTreeSelectChange`                                                                           | `value: any`      | обобщённый, `value` типизирован |

`valueChange` стоит пояснить отдельно. Он был объявлен и задокументирован как вторая половина двусторонней привязки к входу `value`, которого не существует, и никто никогда в него ничего не отправлял — то есть `(valueChange)` не срабатывал ни разу. Его удаление тоже ничего не меняет в рантайме: не найденный ни у одной директивы `(x)` Angular считает слушателем DOM-события, а тот молчит ровно так же.

| Что было                                                                    | Что делать вручную                                                                |
| --------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| `valueChange`                                                               | Слушать `(selectionChange)`                                                       |
| `.getPanelClasses()` / `.getPanelTheme()` / `.isRtl()` / `.transformOrigin` | Привязать `[panelClass]`, если панель нужно оформить                              |
| `.hiddenItemsText = …` и подобные                                           | Привязать вход — сигнальному члену нельзя присвоить значение                      |
| `.hiddenItemsText` и подобные                                               | Читать как вызовы                                                                 |
| `.options` / `.tags` / `.overlayDir` / …                                    | Стали `protected`; используйте API открытия/закрытия, входы и `(selectionChange)` |

`KbqTreeSelectChange` стал обобщённым, а его `value` больше не `any` — обработчику, который полагался на неявное расширение типа, нужен аргумент типа.

Три исправления, для которых мигрировать нечего: встроенное дерево настраивается через `KbqTreeSelection.initializeForEmbedding()` вместо второго ручного `ngAfterContentInit()`, который оставлял дублирующиеся подписки на списки запросов, никогда не пересоздаваемые заново, — из-за чего каждое изменение набора опций обрабатывалось по несколько раз, причём на горячем пути фильтрации поиском; мёртвый провайдер `{ provide: KbqTree, useExisting: KbqTreeSelect }` удалён, поскольку `KbqTreeSelect` никогда структурно не соответствовал `KbqTree`, и всё, что внедряло `KbqTree` изнутри tree-select, получало объект, только похожий на нужный; и компонент отрисовывает `id` на хосте и несёт ARIA combobox, поэтому `[attr.for]` подписи поля формы теперь указывает на существующий элемент.
...
### 20. Хранилище состояния аккордеона переехало в core (21.0.0)

Сохранение состояния аккордеона теперь построено на хранилище, общем для всей библиотеки, — через тот же
токен смогут сохранять состояние и другие компоненты. Специфичный для аккордеона API хранилища удалён:

| Удалено                              | Использовать вместо                        |
| ------------------------------------ | ------------------------------------------ |
...
| `KbqAccordionItemSnapshot`           | удалён вместе с форматом, который описывал |

Замены импортируются из `@koobiq/components/core`. Если подставить `KBQ_STATE_STORE` в `providers` самого
аккордеона, подмена ограничится им — так же, как раньше работал отдельный токен.

Собственное хранилище теперь переносит непрозрачные данные: `getState()` возвращает `unknown` вместо
типизированного состояния, а нормализует прочитанное каждый компонент сам. Если вы реализовывали
`KbqAccordionStateStore`, расширьте сигнатуры и уберите привязку к типам аккордеона:
...
`removeState` — новый обязательный метод: у хранилища появилось полноценное удаление, его вызывает
`clearSavedState()` на аккордеоне.

`KbqAccordion.saveItemState()` и `KbqAccordionItem.getState()` также удалены: аккордеон сохраняет состояние
целиком через `saveState()`. `KbqAccordionState` теперь список значений раскрытых секций (`string[]`), а не
карта «идентификатор секции — снимок». Состояние, сохранённое в прежнем формате, мигрируется при чтении,
поэтому пользователи не потеряют раскрытые секции.

Схематики миграции для самого переезда нет: изменение затрагивает реализации хранилищ и провайдеры DI,
а не шаблоны. Смену дефолта, приехавшую вместе с ним, закрывает `accordion-state-saving-default` —
см. следующий раздел.

### 21. Сохранение состояния аккордеона включено по умолчанию (21.0.0)

`KbqAccordion.useStateSaving` по умолчанию равен `true`. Аккордеон, который никто не настраивал, теперь
запоминает раскрытые пользователем секции и восстанавливает их при следующем рендере. Передайте
`[useStateSaving]="false"` там, где начальным состоянием управляет приложение.

Такой дефолт возможен только потому, что ключ больше не зависит от порядка создания компонентов. Если
`stateSavingKey` пуст, ключ выводится из положения аккордеона в документе — цепочка имён тегов до
`<body>`, обрезанная первым встреченным `id`, который становится якорем:
...
Поэтому всё, что выше `id`, можно перестраивать, не сдвигая ключ, а закрепить ключ автор может как через
`id`, так и через `stateSavingKey`. Перестройка ниже якоря ключ сдвигает, и сохранённое под прежним
остаётся лежать до истечения срока. Чтобы выводить ключ из того, чего DOM не знает, — например, из
текущего маршрута, — подставьте свою стратегию через `KBQ_STATE_SAVING_KEY_RESOLVER`.

Что ещё меняется вместе с дефолтом:

- **`defaultValue` применяется только при первом визите.** Со второго побеждает сохранённое состояние — в
  том числе когда пользователь свернул все секции.
- **Секция без `[value]` сохраняется по позиции.** `KbqAccordionItem.value` возвращает позицию внутри
  своего аккордеона вместо идентификатора: идентификатор содержит глобальный счётчик, который сдвигается,
  как только на странице раньше создаётся что-то ещё. Задавайте секциям явные `[value]`, если набор
  секций может меняться и везде, где сравниваются значения из `valueChange`.
- **Оба инпута живут на хост-директиве.** `useStateSaving` и `stateSavingKey` принадлежат
  `KbqStateSaving`, которая подключается через `hostDirectives`, — так теперь добавляется сохранение
  любому компоненту, нашему или вашему. Разметки это не касается, а вот программного доступа к ним у
  `KbqAccordion` — касается.
- **Предупреждение dev-режима об отсутствии `stateSavingKey` убрано.** Ненастроенный аккордеон теперь
  обычный случай. Предупреждение остаётся только тогда, когда ключ не удаётся вывести вовсе — если хост в
  момент чтения не находится в документе.

Формат хранения:

- Записи пишутся под префиксом `kbq.state.`, поэтому не могут столкнуться с ключом приложения.
- Каждая запись помечена временем сохранения. Запись, которую `KBQ_STATE_SAVING_TTL` (по умолчанию
  90 дней) не читали и не перезаписывали, удаляется при следующем создании хранилища — именно это не даёт
  накапливаться ключам, осиротевшим после перестройки разметки. Чтение обновляет отметку, поэтому
  состояние, которое открывают, но не меняют, не истекает под активным пользователем.
- Запись, сделанная 20.2.0 под голым ключом без префикса, по-прежнему читается, поэтому обновление не
  сбрасывает то, что было у пользователей. Она никогда не перезаписывается и не удаляется: ключ без
  префикса не обязательно наш, и приложение, хранящее собственный `settings`, не должно потерять его
  из-за компонента с `stateSavingKey="settings"`. Первое же сохранение переносит состояние под префикс.
  Этот мост убирается в следующем мажоре.

Схематика `accordion-state-saving-default` сообщает о каждом потребителе, которого затрагивает дефолт.
Она только предупреждает: разметка, поведение которой изменилось, — это ровно та разметка, где инпут не
упомянут, а отключить сохранение везде значило бы отобрать возможность, ради которой релиз и делается.

### 22. Сохранение состояния дерева включено по умолчанию (21.0.0)

`kbq-tree-selection` и `kbq-tree` сохраняют раскрытые узлы, а `useStateSaving` по умолчанию равен `true`.
Ненастроенное дерево теперь возвращается с теми ветвями, которые пользователь оставил открытыми.
Передайте `[useStateSaving]="false"` там, где начальным состоянием распоряжается приложение.

Это та же хост-директива `KbqStateSaving`, которую подключает аккордеон, поэтому оба инпута, ключ
хранения, префикс `kbq.state.` и срок жизни записи ведут себя ровно так, как описано в предыдущем разделе.

**Выделение не сохраняется.** Оно принадлежит форме, к которой привязано дерево, и восстановление его из
хранилища перезаписало бы значение, заданное приложением. Хранится только раскрытие.

Что проверить в своём коде:

- **`getValue` стал ключом сохранения.** Раскрытие хранится по значению, которое контрол возвращает для
  узла, — по третьему аргументу конструктора `FlatTreeControl` и той же идентичности, которую дерево уже
  использует для выделения. Значение должно быть строкой, стабильной между перезагрузками и уникальной в
  пределах дерева: объект узла создаётся заново при каждой замене данных, поэтому ключом служить не может.
  Если два узла делят одно значение, раскроется первый из них — так же, как поступает выделение.
- **Дерево на `NestedTreeControl` не сохраняет ничего.** У этого контрола `getValue` нет вовсе. В
  dev-режиме один раз выводится предупреждение; чтобы его убрать, снимите `useStateSaving` с этого дерева.
- **Раскрытие, выполненное самим приложением, само по себе не сохраняется.** `treeControl.expandAll()`,
  `collapseAll()` и прямая запись в `expansionModel` — не действия пользователя; вызовите после них
  `saveState()` у дерева, чтобы зафиксировать. Следующее раскрытие, выполненное пользователем, всё равно
  сохранит получившееся состояние целиком.

Что не требует внимания:

- **Узлы, приходящие позже, дожидаются.** Значение, узел которого ещё не загружен, применяется, как только
  тот появится, — лениво загружаемое дерево восстанавливается по мере загрузки ветвей. До тех пор значение
  сохраняется, поэтому запись изменения, сделанного тем временем, не теряет ещё загружающиеся ветви.
- **Пока активен поисковый фильтр, ничего не сохраняется.** `filterNodes()` заменяет набор раскрытых узлов
  на все раскрываемые узлы, попавшие в результат, а затем возвращает прежний, поэтому раскрытое во время
  поиска — это вид результатов, а не состояние.
- **Дерево внутри оверлея не сохраняет состояние.** `kbq-tree-select` отрисовывает такое в своей панели:
  там дерево на момент инициализации не находится в документе, а значит, не имеет стабильного ключа, — да
  и раскрытие в панели выбора всё равно временное. Для потребителей `kbq-tree-select` ничего не меняется.

Что легко упустить: несколько деревьев с одним `treeControl` делят одну модель раскрытия, но сохраняются
каждое под своим ключом. Восстановление определяет то, которое инициализировалось последним, а изменение
записывает только то, с которым взаимодействовал пользователь, — у остальных под их ключами остаётся
прежнее. Дайте им по отдельному контролу или снимите `useStateSaving` со всех, кроме одного.

Новое у дерева: `saveState()`, `clearSavedState()` и `hasSavedState` — вместе с инпутами `useStateSaving`
и `stateSavingKey`, которые пробрасывает хост-директива.

Схематик `tree-state-saving-default` сообщает обо всех потребителях, до которых дотягивается новое
значение по умолчанию, о контролах, чей `getValue` стоит перепроверить, и о программном раскрытии, которое
больше не записывается само. Он только предупреждает — по той же причине, что и у аккордеона.

### 23. Сохранение состояния вкладок, боковой панели и контент-панели включено по умолчанию (21.0.0)

`kbq-tab-group`, `kbq-sidebar` и `kbq-content-panel-container` сохраняют состояние, которое меняет
пользователь, а `useStateSaving` по умолчанию равен `true` у всех трёх. Это та же хост-директива
`KbqStateSaving`, которую подключают аккордеон и дерево, поэтому оба инпута, ключ хранения, префикс
`kbq.state.` и срок жизни записи ведут себя ровно так, как описано в предыдущих разделах.

Каждый из них не трогает хранилище, пока состоянием распоряжается приложение, поэтому изменение
затрагивает только ту разметку, которая об этом молчит:

| Компонент                     | Запоминает                                               | Не сохраняет, пока                        |
| ----------------------------- | -------------------------------------------------------- | ----------------------------------------- |
| `kbq-tab-group`               | выбранную вкладку — по `tabId` и по позиции              | привязаны `selectedIndex` или `activeTab` |
| `kbq-sidebar`                 | был ли открыт и какой ширины его закрыли в последний раз | привязан `opened`                         |
| `kbq-content-panel-container` | был ли открыт и до какой ширины его растянули            | привязан `opened`                         |

Передайте `[useStateSaving]="false"` там, где начальное состояние принадлежит приложению по другой
причине.

Что проверить в своём коде:

- **Задайте вкладкам `tabId`.** Выбор хранится и по идентификатору, и по позиции, но перестановку вкладок
  переживает только идентификатор — без него по позиции восстановится другая вкладка, о чём в dev-режиме
  выводится предупреждение. Если сохранённый идентификатор больше не соответствует ни одной вкладке,
  используется позиция; если не подходит ни то ни другое, не восстанавливается ничего.
- **`KbqContentPanelContainer.opened` теперь `openedInput`.** Пока его никто не привязывает, он читается
  как `undefined`, а не `false`, — именно так панель отличает привязанный `opened` от непривязанного.
  Разметки это не касается: `<kbq-content-panel-container [opened]="true">` работает как прежде, — а вот
  чтение с компонента больше не компилируется. Публичное чтение состояния — `isOpened()`, как и раньше.
- **Ширина контент-панели восстанавливается даже при привязанном `[opened]`.** Выхода `widthChange` нет,
  поэтому результат перетаскивания до приложения никогда не доходил, а `[width]` задаёт ширину, с которой
  панель начинает, а не ту, что у неё сейчас. Двойной клик по краю панели возвращает заданную ширину, и этот
  сброс тоже сохраняется.

Что не требует внимания:

- **`kbq-tab-nav-bar` не сохраняет состояние никогда.** Это навигационный вариант, где активную ссылку
  определяет роутер; восстанавливать там имеет смысл URL.
- **Компонент, отрисованный внутри оверлея, состояние не сохраняет.** На момент инициализации он не
  находится в документе, а значит, не имеет стабильного ключа.
- **Ширина боковой панели — та, что была при последнем закрытии**: ровно её он и так использует повторно при
  открытии, поэтому для панели, которую пользователь не растягивает, ничего не меняется.

Новое у всех трёх: `saveState()`, `clearSavedState()` и `hasSavedState` — вместе с инпутами
`useStateSaving` и `stateSavingKey`, которые пробрасывает хост-директива.

Два компонента, о которых просили заодно, разобраны отдельно. Внутри **`kbq-sidepanel`** нет ничего, что
меняет пользователь: ширина — одно из предустановленных значений, положение задаёт вызывающий код при
`open()`. И восстановить себя он не может: он существует, только пока открыт. Зато у него есть признак,
который стоит помнить, — был ли он открыт: его хранит `KbqSidepanelService`, а открывает панель заново
приложение; чтобы это включить, задайте панели `stateSavingKey`. **`kbq-filter-bar`** идёт следующим
разделом и второе сомнение снимает, а не обходит: то, что фильтр — двусторонний `model()`, как раз и
позволяет восстановлению дойти до приложения.

Схематик `state-saving-default` сообщает обо всех потребителях, до которых дотягивается новое значение по
умолчанию, о вкладках, чей выбор откатится к позиции, и о чтениях `opened`, которые больше не
компилируются. Он только предупреждает — по той же причине, что и у аккордеона с деревом.

### 24. Сохранение состояния filter-bar включено по умолчанию (21.0.0)

`kbq-filter-bar` запоминает, какой фильтр выбран и какие изменения в него внесли, а `useStateSaving`
по умолчанию равен `true`. Это та же хост-директива `KbqStateSaving`, что и у компонентов выше, поэтому
оба инпута, ключ хранения, префикс `kbq.state.` и TTL работают ровно так, как описано там.

**Одно отличие.** Компоненты выше не трогают контролируемый инпут: боковая панель с привязанным `opened`
не сохраняет ничего вообще. `filter-bar` восстанавливается поверх значения, которое привязка `[filter]`
подставила при инициализации, потому что `filter` — это `model()`: восстановление пишет через него,
поднимается `filterChange`, и приложение загружает данные для восстановленного фильтра ровно так же, как
для только что выбранного пользователем. Выигрывает только изменение, сделанное после этого.

То есть привязка — не способ отказаться. Передайте `[useStateSaving]="false"` для компонента, фильтром которого
целиком распоряжается приложение.

Что проверить в своём коде:

- **Первый запрос данных может уйти за другим фильтром, чем раньше.** Выполняется то же самое, что уже
  делает `(filterChange)`, — но теперь это происходит один раз при старте, с восстановленным фильтром, до
  того как пользователь что-либо трогал. Привязка `[filter]`, которая раньше определяла стартовый набор
  данных, больше его не определяет.
- **Проецируйте пайпы из того фильтра, который отдаёт компонент.** Восстановление заменяет объект фильтра и
  каждый его пайп новыми копиями — ровно так же, как это уже делает выбор фильтра в `<kbq-filters>`.
  Привяжите `[(filter)]` или присвойте то, что приходит в `(filterChange)`. Пайп, спроецированный из
  массива, которым компонент больше не владеет, — это не тот пайп, что лежит в `filter`: его правка не доходит
  до сохранённого состояния, а кнопка удаления перестаёт срабатывать.
- **Фильтр опознаётся по `name`.** У `KbqFilter` нет id, поэтому переименование сохранённого фильтра
  теряет всё, что было для него записано, а имя, которого больше нет в `filters`, ничего не
  восстанавливает. Список, загружаемый с сервера, дожидаются: восстановление применяется, как только
  названный фильтр появляется, и отменяется, как только фильтр меняет что-то ещё.
- **Держите `compareWith` в согласии с этим.** Восстановленные значения возвращаются новыми объектами, а
  не теми экземплярами опций, что лежат в `pipeTemplates`, поэтому пайп, сравнивающий опции по ссылке, ни
  с чем не совпадёт. Компаратор по умолчанию сравнивает по id — потому встроенным пайпам ничего и не
  нужно.

Что не требует внимания:

- **От каждого пайпа хранится только проекция** — его `id` (или `name`, если `id` нет) и значение. Всё
  остальное собирается заново из `filters` и `pipeTemplates`: пайп, собранный из шаблона, несёт его
  `compareWith` и границы дат, а они запись в хранилище не переживают. Пайп, чей шаблон исчез,
  пропускается.
- **Сохранение фильтров не тронуто.** Поток сохранения у `<kbq-filters>`, `KbqSaveFilterEvent` и
  `filterSavedSuccessfully()` — это приложение, которое хранит именованный фильтр; здесь же бар
  запоминает на одном устройстве, какой из них был в работе.
- **`saveFilterState()` и `restoreFilterState()` к этому не относятся** и не изменились. Они снимают
  копию фильтра в памяти в пределах одной сессии. До сохранённого дотягиваются `clearSavedState()` и
  `hasSavedState`; метода `saveState()` у бара нет, потому что он пишет при каждом изменении сам.

Schematic `filter-bar-state-saving-default` сообщит обо всех потребителях, до которых дотягивается новое
значение по умолчанию, о привязках `[filter]`, которые теперь перекрываются восстановлением, и о местах,
где важны идентичность фильтра и `compareWith`. Он только предупреждает — по той же причине, что и
предыдущие.

### 25. Кнопка сброса у списка тегов очищает сама (21.0.0)

До 21.0.0 `<kbq-cleaner>`, спроецированный в `<kbq-tag-list>`, сам по себе ничего не делал: список глушил
встроенную очистку, а удаление выполнял ваш обработчик `(click)`. Теперь кнопка сброса удаляет теги сама и
оставляет неактивные на месте, поэтому обработчик ей больше не нужен. Удаляет она через то же событие
`removed`, что и иконка удаления внутри тега, — то есть достаточно той привязки, которая нужна этой иконке.
...
Оставленный обработчик отработает **вместе** со встроенной очисткой и подавить её не сможет: хост-слушатель
самого компонента срабатывает раньше, — поэтому его нужно убрать.

Чтобы удалялись и неактивные теги, привяжите предикат, который их принимает. Стрелочную функцию прямо в
шаблоне писать нельзя, поэтому нужно поле:
...
У списка с `removable="false"` удаления нет вовсе, поэтому кнопки сброса у него теперь тоже нет.

#### Запуск миграции

Schematic `tag-list-cleaner` запускается автоматически:
...
Чтобы посмотреть изменения, не записывая их, используйте `--fix=false`:
...
Обработчик `(click)` снимается с каждого `<kbq-cleaner>`, спроецированного в `<kbq-tag-list>`, — и в
`.html`-файлах, и в инлайновых шаблонах. Кнопки сброса вне списка тегов не трогаются. Снятое выражение
печатается для каждого файла, потому что обработчик мог делать не только очистку.

#### Что нужно исправить вручную

- **Всё, что обработчик делал помимо очистки.** Верните эту часть в другую привязку; снятое выражение есть
  в логе.
- **Список, который никто не слушает.** Если ни один тег в списке не сообщает о `removed`, встроенной
  очистке некуда идти. Schematic оставит обработчик и скажет об этом — добавьте привязку `(removed)`, после
  чего обработчик можно убрать.
- **Список с `removable="false"`.** Кнопки сброса у него больше нет. Schematic оставит обработчик и скажет
  об этом.
- **У неактивного тега больше не отрисовывается `kbqTagRemove`.** Для этого schematic нет: иконка скрыта, а
  клавиша `Delete` и так отказывала, то есть сам протокол удаления не изменился, — но тест или стиль,
  который рассчитывал на иконку у неактивного тега, придётся поправить.
