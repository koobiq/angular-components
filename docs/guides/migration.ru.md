...
`KbqTagList` реализует `KbqFormFieldControl`, а `KbqTagInput` — `KbqTagTextControl`; оба интерфейса объявляют свои члены обычными свойствами. А большинство входов `KbqTag` подмешивают состояние списка тегов: `disabled` возвращает и своё состояние, и состояние списка. `model()` не умеет нести трансформацию `booleanAttribute`, которая нужна атрибуту без значения, поэтому они остались геттерами и сеттерами — но теперь поверх сигналов: **синтаксис чтения и записи не изменился**, и момент чтения тоже: привязки на хосте компонента вычисляет то представление, которое объявляет элемент, поэтому `[class.kbq-disabled]` у тега и раньше проверялся заново на каждом проходе родителя.

Переехала горстка входов `KbqTagInput`, которые ни от кого не зависят. В частности, `separatorKeyCodes` был сеттером без геттера — теперь его наконец можно прочитать.

| Что было                     | Как мигрировать вручную                                                  |
| ---------------------------- | ------------------------------------------------------------------------ |
| `.addOnBlur` / `.separators` | Читать как вызовы — переписывается за вас                                |
| `.separatorKeyCodes = …`     | Привязать `[kbqTagInputSeparatorKeyCodes]`; зато теперь его можно читать |
| `.addOnBlur = …`             | Привязать `[kbqTagInputAddOnBlur]`                                       |
...
Закрывается схематиком `tags-signals`: чтения `addOnBlur` и `separators` переписываются, остальное сообщается в отчёте.
