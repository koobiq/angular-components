...
**Двадцать свойств стали сигналами.** Выпадающий список был последним компонентом ревью с входами-декораторами. Теперь чтение любого из них — это вызов:
...
Шесть свойств — это `model()`, поэтому запись в них стала `.set(…)`: `xPosition`, `yPosition`, оба флага перекрытия, `offsetX` и `openByArrowDown`. Они доступны на запись потому, что их меняет хост, который сам позиционирует панель: так делают `kbq-split-button` и `kbq-navbar-item`. У `model()` нет `transform`, поэтому эти шесть больше не приводят строковый атрибут к типу: пишите `[overlapTriggerX]="true"`, а не `overlapTriggerX="true"`.
...
| `panel.closed.pipe(…)`                | `closed` — это `output()`. Оберните: `outputToObservable(panel.closed)`                               |
...
Два свойства остались на месте. `KbqDropdownItem.textElement` — это по-прежнему `@ViewChild`, потому что он реализует `KbqTitleTextRef`, а так же его реализуют ещё пять компонентов. `KbqDropdownTrigger.dropdownClosed` — по-прежнему `EventEmitter`, потому что `KbqOptionActionComponent` подписывается на него через `.pipe()` по контракту, который `OutputEmitterRef` не поддерживает.
