...
`errorColor` добирался до иллюстрации императивно: `ngAfterContentInit()` читал вход ровно один раз и вызывал `KbqEmptyStateIcon.setErrorColor()`, который присваивал `color = 'error'` внедрённому `kbq-icon-item`. Из-за этого подкраска была необратимой — возврат `[errorColor]` в `false` оставлял красную иконку над чёрным текстом, — не срабатывала для значения, переключённого в `true` после инициализации, и вовсе не происходила, когда иконка обёрнута слотом, а не несёт его сама: директива искала иконку через `inject(KbqIconItem)`, который видит её только на собственном хост-элементе. Два примера самого компонента используют именно обёрнутую разметку. Цвета заголовка и текста были реактивными всегда — они держатся на классах хоста.

Теперь иллюстрация следует за `[errorColor]` сама, в обеих формах разметки и в обе стороны, поэтому у метода не осталось вызывающего.

| Член                                 | Что было                                        | Что стало                    |
| ------------------------------------ | ----------------------------------------------- | ---------------------------- |
| `KbqEmptyStateIcon.setErrorColor()`  | публичный, вызывался один раз при инициализации | удалён                       |
| `KbqEmptyState.ngAfterContentInit()` | публичный хук жизненного цикла                  | удалён                       |
| `KbqEmptyState.icon`                 | публичный `@ContentChild` с типом `\| null`     | `protected` `contentChild()` |

| Что было                  | Что делать вручную                                                      |
| ------------------------- | ----------------------------------------------------------------------- |
| `.setErrorColor()`        | Привязать `[errorColor]` на `<kbq-empty-state>` и убрать вызов          |
| `.icon` у `KbqEmptyState` | Стал `protected`: это запрос, питавший класс `kbq-empty-state_has-icon` |

**Четыре токена темы переименованы.** `--kbq-empty-state-title` и `--kbq-empty-state-color` читаются как токен размера и цвет компонента, хотя это цвет заголовка и цвет _текста_; пара для ошибки повторяла ту же форму. Теперь это `--kbq-empty-state-title-color`, `--kbq-empty-state-text-color`, `--kbq-empty-state-error-title-color` и `--kbq-empty-state-error-text-color`. Каждое новое имя читает старое как запасное значение, поэтому существующее переопределение продолжает работать — но старые имена объявлены устаревшими, и запасные значения будут убраны.

Два исправления, для которых мигрировать нечего. `empty-state.scss` подключает свой слой токенов сам, а не полагается на вторую запись в `styleUrls`, поэтому всё, что переиспользует этот файл через границу пакета, получает пользовательские свойства вместе с правилами, которые их читают; раньше при таком повторном использовании забирались только правила, и каждый `max-width` в них разрешался в `none`. И заголовок размечен настоящим заголовком по всей библиотеке — он несёт типографику `subheading` при `size="normal"` и `headline` при `size="big"`, так что `<h2 kbq-empty-state-title>` — это то, о чём просит компонент; отступ браузера обнулён, поэтому вёрстка не меняется. Заглушке, которая появляется на уже прочитанной странице, по-прежнему нужен `role="status"` на хосте — или `role="alert"` для состояния ошибки: своего `role` компонент не проставляет.

Сообщает `empty-state-error-color`.
#### Loader overlay
