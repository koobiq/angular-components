...
Ревью починило два контракта, по которым и оценивают компонент: контракт формы и примитив `KbqFileList`.

`writeValue()` присваивал значение через сеттеры `file`/`files`, а те вызывают `cvaOnChange` — половину `ControlValueAccessor`, идущую от представления к модели. Обратный вызов Angular, идущий от модели к представлению, тем самым заново входил в конвейер представления: каждый `setValue`/`patchValue` помечал контрол **грязным** и дважды отправлял `valueChanges`, а поскольку `FormControl.reset()` вызывает `markAsPristine()` _до_ `setValue()`, сброшенный загрузчик возвращался грязным. Этот же метод отправлял публичный выход `(fileChange)`/`(filesChange)`, так что фоновый `patchValue` было не отличить от выбора файла пользователем. Теперь `writeValue()` пишет список напрямую и не отправляет ни того, ни другого.

| Что изменилось                       | Было                                   | Стало                                              |
| ------------------------------------ | -------------------------------------- | -------------------------------------------------- |
| `writeValue()`                       | писал через сеттер `file` / `files`    | пишет список напрямую, не отправляет выходов       |
| `KbqFileList.remove(item)`           | возвращал оставшиеся, удалял все копии | возвращает удаленный элемент, удаляет первую копию |
| `KbqFileList.removeAt(index)`        | переписывал список при любом индексе   | игнорирует индекс за пределами списка              |
| `hasFocus`                           | публичное поле, всегда `false`         | удалено                                            |
| скрытый input одиночного загрузчика  | `multiple`                             | выбор одного файла                                 |
| `[multiple]` у одиночного загрузчика | пробрасывался в это поле               | больше не является входом                          |
| `KbqInputFileMultipleLabel`          | экспортируемый интерфейс               | помечен устаревшим                                 |

| Что искать                          | Что сделать вручную                                                                 |
| ----------------------------------- | ----------------------------------------------------------------------------------- |
| `.remove(item)`                     | Возвращается удаленный элемент; для позиции используйте `removeAt(index)`           |
| `.hasFocus`                         | Отслеживайте фокус через `cdkMonitorSubtreeFocus` или пару `(focusin)`/`(focusout)` |
| `(fileChange)` / `(filesChange)`    | Подпишитесь на контрол, если обработчик должен видеть программную запись            |
| `KbqInputFileMultipleLabel`         | Используйте `KbqMultipleFileUploadLocaleConfiguration`                              |
| `<kbq-single-file-upload multiple>` | Уберите атрибут или перейдите на `<kbq-multiple-file-upload>`                       |

**Пустое перетаскивание больше не уничтожает список.** Пустая папка разворачивается в ноль файлов, поэтому обработчику перетаскивания может прийти пустой массив. Множественный загрузчик передавал его прямо в слияние, и при `addStrategy="replace"` это означало `replace([])`: все уже приложенные пользователем файлы исчезали, `(filesChange)` сообщал пустой список, а контрол оставался грязным. Теперь выбор, который ничего не передал, вообще не считается выбором: список не меняется и значение не отправляется, но перетаскивание по-прежнему считается взаимодействием и помечает контрол как touched. У `removeAt()` был соседний вариант того же дефекта: при индексе за любым из концов `splice` ничего не делал, но список все равно переписывался новым массивом — будя всех потребителей `list()` ради пустой операции, — а `itemRemoved` отправлялся с `undefined` внутри кортежа, типизированного как `[T, number]`.

Контрол помечается как touched, когда фокус покидает загрузчик, поэтому стандартный `ErrorStateMatcher` показывает ошибку `required` пользователю, который прошел мимо по Tab и ничего не приложил, — раньше эта ошибка оставалась невидимой до отправки формы. Удаление файла из середины списка переводит фокус на кнопку, занявшую его место, а не роняет его на `<body>`; проецируемые сообщения `kbq-hint` связаны с полем выбора файла через `aria-describedby`, само поле получает `aria-invalid`, пока контрол в состоянии ошибки, а добавление и удаление файлов объявляются в live-регионе самого компонента.

Три изменения, о которых стоит знать, но переписывать под них ничего не нужно. Одиночный загрузчик отдает поле выбора одного файла, поэтому системный диалог больше не предлагает множественный выбор, лишние файлы из которого компонент молча выбрасывал; все, что перетаскивание передает после первого файла, сообщается через новый выход `(rejected)` — как и дубликаты, которые множественный загрузчик пропускает при стратегии `concat` по умолчанию. `accept` описан тем, чем является: нативным атрибутом, который фильтрует только системный диалог и никогда — перетащенный файл, а для отклонения по-прежнему нужен валидатор, причем `FileValidators.isCorrectExtension` принимает тот же массив. И 22 пользовательских свойства `--kbq-file-upload-*`, которые не читало ни одно правило, удалены, включая оба токена `*-states-focused-focus-outline-color`; их установка никогда ни на что не влияла.

В локали появилась секция `fileUpload.a11y` с объявлениями для live-региона, поэтому в собранный вручную `KbqLocaleData`, зарегистрированный через `KBQ_LOCALE_DATA`, нужно добавить `fileAdded`, `fileRemoved` и `filesNotAdded`. В публичных типах теперь указаны `KbqBaseFileUploadLocaleConfiguration` и `KbqMultipleFileUploadLocaleConfiguration` вместо устаревших псевдонимов `*LocaleConfig`; псевдонимы по-прежнему указывают на те же типы, так что менять вызовы не нужно.

Отчет формирует `file-upload-cva-and-primitives`.
