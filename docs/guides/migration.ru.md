...
Теперь это сигнальный вход, и он возвращает то, что было записано. Класс `kbq-badge_<color>` по-прежнему попадает на хост — из внутреннего `computed`, — поэтому стили и скриншоты не изменились: разницу видит только программное чтение.
