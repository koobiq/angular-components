...
У управления разворачиванием не было семантики раскрытия, а состояние публиковалось на элементе, которым нельзя воспользоваться. `aria-expanded` стоял на хосте `<kbq-clamped-text>` и на контейнере `[kbqClampedList]` — оба без роли, где этот атрибут ничего не сообщает, — а `kbqClampedListTrigger`, который принимает клик и клавиши Enter/Space, не имел ни `role`, ни `aria-expanded`, ни `aria-controls`. Теперь все четыре принадлежат триггеру, поэтому прописанный вручную `role="button"` на нём стал дубликатом; явные `role` или `tabindex` на элементе по-прежнему выигрывают у значений директивы.

`aria-expanded` больше не отрисовывается ни на одном из контейнеров. Селектор или проверка, читавшие состояние с обёртки, должны читать его с триггера.

`kbqClampedListTrigger` больше не проставляет `kbq-clamped-text__toggle`. У этого класса есть единственное правило — в стилях clamped-text, которые подключаются только при появлении `KbqClampedText`, — поэтому у триггера `kbqClampedList` отступ был или не был в зависимости от того, оказался ли на странице посторонний компонент. Собственных отступов у триггера списка теперь нет: размещайте его вместе со списком и уберите написанные под него переопределения `margin-top`. Переключатель clamped-text класс сохраняет, а его отступ настраивается через `--kbq-clamped-text-size-toggle-margin`.

| Что было                          | Что делать вручную                                            |
| --------------------------------- | ------------------------------------------------------------- |
| `aria-expanded` на контейнере     | Читать его с элемента `[kbqClampedListTrigger]`               |
| Сброс `.kbq-clamped-text__toggle` | Убрать — триггер списка больше не заимствует отступ           |
| `role="button"` на триггере       | Убрать: роль задаёт директива                                 |
| `.hasToggle.set(…)`               | `hasToggle` — `Signal` только для чтения, его пишет измерение |
| `.text()` / `.textContainer()`    | `protected`; получайте элемент из собственного шаблона        |

`isCollapsed` у `KbqClampedText` стал `model()`, поэтому `isCollapsedChange` означает то, что и было написано в документации, — действие пользователя. Он больше не срабатывает на первом собственном измерении компонента и не отражает обратно значение, записанное родителем в `[isCollapsed]`: привязка к параметру маршрута перестала записывать его при каждой загрузке. Обработчику, который узнавал начальное состояние из события при монтировании, теперь нужно читать его из `[(isCollapsed)]`. `KbqClampedList` это не затрагивает: его `isCollapsed` и раньше был `model()`.

Четыре исправления, для которых мигрировать нечего. Space и Enter на триггере вызывают `preventDefault()`, поэтому Space больше не прокручивает страницу при разворачивании, а нативный `<button>` в роли хоста перестал переключаться дважды из-за синтетического клика. Сворачивание прокручивает с `{ block: 'nearest', inline: 'nearest' }` вместо центрирования по обеим осям, а `[scrollOnCollapse]="false"` отключает это совсем. `rows` принимает строковый атрибут, поэтому `rows="3"` компилируется при `strictTemplates` так же, как уже компилировался `debounceTime="300"`, а `[debounceTime]` перечитывается на каждом изменении размера, а не однократно в `ngAfterViewInit`. И до первого измерения больше ничего не отрисовывается: переключатель появляется, когда известно, что содержимое не помещается, а текст до этого остаётся свёрнутым, а не уезжает в разметку целиком — как это делала собранная заранее страница документации.

Сообщает `clamped-text-disclosure`.
