...
До потребителя ревью доходит в четырёх местах: ARIA-семантика триггера и элемента списка, сигнатура публичного метода `activateSafeArea()`, расчёт ширины панели и — самое крупное — перевод входов, выходов и запросов компонента на сигналы.

`aria-expanded` — не глобальный ARIA-атрибут. Его нельзя ставить на элемент, роль которого вычисляется как `generic`: голый `<div>` или `<span>`, `<a>` без `href`. А триггер вешается на тот элемент, который выбрал потребитель, — поэтому раньше атрибут регулярно нарушал `aria-allowed-attr`.

Теперь он публикуется только там, где роль способна его нести: `<button>`, `<summary>`, `<a href>` или хост с собственной ролью. Все триггеры библиотеки к ним относятся. А ваш триггер без роли атрибут потеряет — вместе с селектором или тестом, который на него опирался.

Хост элемента списка не всегда активируется браузером сам. Для строки с дополнительным действием пакет документирует `<div>`, и до сих пор с клавиатуры такую строку было не нажать. Теперь она активируется по <kbd>Enter</kbd> и <kbd>Space</kbd>: клавиша воспроизводится как клик, поэтому обработчик `(click)` и закрытие панели ведут себя так же, как при работе мышью.

Хосты, которые браузер активирует сам, пропускаются. Пропускаются и роли дочерних элементов составных виджетов (`menuitem`, `option`, `treeitem`, …): там клик воспроизводит их собственный менеджер клавиатуры.

| Что было                                               | Что делать вручную                                                                                                                     |
| ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- |
| `activateSafeArea(owner, triangle, panelRect, onExit)` | Передавать точку указателя и геттер прямоугольника: `activateSafeArea(owner, { x, y }, () => element.getBoundingClientRect(), onExit)` |
| Наследник, переопределяющий `handleActionKeydown`      | Переопределять `handleKeydown`: он владеет <kbd>Enter</kbd>/<kbd>Space</kbd> и делегирует случай <kbd>Tab</kbd>                        |
| `.kbq-dropdown-item_with-icon` в селекторе             | Класса больше нет; выбирайте проецируемую иконку                                                                                       |
| `[aria-expanded]` на триггере без роли                 | Задать хосту `role` либо убрать селектор                                                                                               |
| Обработчик клика на `.kbq-dropdown__panel`             | Собственное закрытие панели переехало на `.kbq-dropdown__content`                                                                      |

**Панель больше не может вырасти за пределы собственной области.** Токен `--kbq-dropdown-size-container-width-min` откатывался к статическим `200px` везде, где вход не задавал его явно. В том числе при явном `panelWidth`: `[panelWidth]="150"` рисовал панель в 200px внутри коробки в 150px.
...
- Фокус возвращается на триггер, только если на момент закрытия он всё ещё был внутри оверлея. Если вы кликнули снаружи, фокус останется на том элементе, по которому кликнули.
- <kbd>Home</kbd> и <kbd>End</kbd> двигают подсветку в выпадающем списке без поля поиска — раньше они прокручивали страницу.
- Клик по элементу управления внутри `kbqDropdownFooter` больше не закрывает панель.
...
- `KbqDropdownItemActionHost` и `KBQ_DROPDOWN_ITEM_ACTION_HOST` экспортированы, а голые реэкспорты `fadeInItems` и `transformDropdown` объявлены устаревшими в пользу `kbqDropdownAnimations`.

**Двадцать из двадцати одного входа, выхода и запроса компонента стали сигналами.** Выпадающий список был последним компонентом ревью с входами-декораторами. У этих семнадцати чтение стало вызовом:
...
Остальные доступны только на чтение, так что запись в них — ошибка компиляции, а не молчаливое ничего.

О чём схематик сообщает, но не исправляет:

| Что было                                    | Как перенести вручную                                                                                 |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `panel.items.changes` / `.first`            | `items` — сигнал обычного массива. Возьмите `toObservable(panel.items)` или сам массив `items()`      |
| `panel.items.reset(…)`                      | Вызовите `adoptItems(…)`                                                                              |
| `closed.pipe(…)` / `dropdownClosed.pipe(…)` | Оба стали `output()`. Оберните: `outputToObservable(panel.closed)`                                    |
...
| Класс, реализующий `KbqDropdownPanel`       | Изменились все свойства. Необязательный `adoptItems` заменяет `items.reset(…)`, который делал триггер |

На месте осталось одно свойство: `KbqDropdownItem.textElement` — это по-прежнему `@ViewChild`, потому что он реализует `KbqTitleTextRef`, а так же его реализуют ещё пять компонентов.
