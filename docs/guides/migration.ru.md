...
24. **21.0.0**: фильтр-бар запоминает выбранный фильтр и его изменения, по умолчанию.
...
### 24. Сохранение состояния фильтр-бара включено по умолчанию (21.0.0)
...
не сохраняет ничего вообще. Фильтр-бар восстанавливается поверх значения, которое привязка `[filter]`
...
То есть привязка — не способ отказаться. Передайте `[useStateSaving]="false"` для бара, фильтром которого
...
- **Проецируйте пайпы из того фильтра, который отдаёт бар.** Восстановление заменяет объект фильтра и
  ...
  массива, которым бар больше не владеет, — это не тот пайп, что лежит в `filter`: его правка не доходит
