...
**Формат генерируемых `id` изменился** с `kbq-tag-list-1` / `kbq-tag-list-input-1` на `kbq-tag-list-a1` / `kbq-tag-list-input-a1`. Список тегов сообщает `id` своего input, если тот есть, поэтому наружу через form-field выходят оба.
