...
Два компонента, о которых просили заодно, намеренно оставлены в стороне. У **`kbq-sidepanel`** нет
ничего, что меняет пользователь: ширина — одно из предустановленных значений, положение задаёт вызывающий код при `open()`, — и это
оверлей, у которого нет стабильного ключа; сохраняйте его состояние через компонент, который его
открывает. **`kbq-filter-bar`** — впереди: фильтром через двусторонний `model()` распоряжается
приложение, и за ним обычно стоит собственный механизм сохранённых фильтров.
