...
**`panelWidth="auto"` у `kbq-select` и `kbq-tree-select`** больше не опускается ниже `panelMinWidth` (по умолчанию 200). Триггер шириной меньше 200px раньше давал панель ровно по своей ширине, теперь — 200px. Если вы на это полагались, поставьте `panelMinWidth="0"`. Автоматической миграции для этого нет — затрагивает ли вас изменение, зависит от отрендеренной ширины триггера.
