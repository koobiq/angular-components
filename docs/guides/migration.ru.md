...
**Спроецированное содержимое стало постоянным запасным вариантом.** `<kbq-markdown>`, который одновременно проецирует содержимое и привязывает `[markdownText]`, возвращается к спроецированному содержимому каждый раз, когда вход пуст, а не только в первый раз. Сам спроецированный текст по-прежнему считывается однократно, сразу после первого показа компонента: его последующее изменение по-прежнему не обновляет вывод.
