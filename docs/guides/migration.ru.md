...
`name` — несущий контракт радиокнопки, и он не соблюдался ни в одну сторону. `UniqueSelectionDispatcher` из CDK глобален для приложения и рассылает каждое уведомление всем слушателям; сравнение по `name` внутри слушателя — единственная изоляция, которая там есть. `<kbq-radio-button>` вне группы никогда не получал имени, поэтому `undefined === undefined` объединял все одиночные радиокнопки приложения в одну группу выбора: выбор одной снимал выбор со всех остальных — в других маршрутах, оверлеях и диалогах. Теперь кнопка получает имя, уникальное для неё самой, и **две одиночные радиокнопки, которые раньше сбрасывали друг друга, независимы**. Разметке, которая на это полагалась, нужно сказать об этом явно: положить кнопки в группу или задать им одинаковый `[name]`.

В обратную сторону `ngOnInit` безусловно перезаписывал заданный `[name]` именем группы, поэтому задокументированный вход ничего не делал. Теперь он учитывается, и кнопка со своим именем внутри группы образует собственную группу выбора.

| Член                                                      | Было                       | Стало                           |
| --------------------------------------------------------- | -------------------------- | ------------------------------- |
| `KbqRadioButton.isFocused`                                | публичный вход, не читался | удалён                          |
| `KbqRadioButton.radioGroup`                               | `KbqRadioGroup`            | `KbqRadioGroup \| null`         |
| `KbqRadioButton.name`                                     | `undefined` вне группы     | имя группы, иначе уникальный id |
| `KbqRadioButton.focus()`                                  | голый `element.focus()`    | `focus(origin?: FocusOrigin)`   |
| `KbqRadioGroup.focus()`                                   | —                          | передаёт фокус выбранной кнопке |
| `KbqRadioGroup.color`                                     | —                          | красит все кнопки группы        |
| `--kbq-radio-size-big-top`, `--kbq-radio-size-normal-top` | объявлены, не использованы | удалены                         |

У `isFocused` не было ни host-привязки, ни ссылки в шаблоне, ни потребителей — привязка к нему никогда ни на что не влияла. Состояние фокуса несут классы `FocusMonitor` на хосте; читайте их или вызывайте `focus(origin)`. `radioGroup` всегда внедрялся с `{ optional: true }` , а восклицательный знак это лишь скрывал, поэтому кнопка вне группы и раньше возвращала `null` под типом без `null`.

Работа над доступностью ничего не требует мигрировать, но меняет то, что произносит скринридер. `<kbq-radio-group>` — это `radiogroup`, а такая роль не берёт название из содержимого: свяжите её обычным атрибутом `aria-label` / `aria-labelledby` на элементе, как это теперь сделано в поставляемых примерах; отдельно стоящая подпись оставляет группу без названия. `kbq-hint`, вложенный в кнопку, попадает в `aria-describedby`, а не в её название, поэтому вариант объявляется своим текстом, а не текстом вместе с подписью. `[required]` доходит до группы как `aria-required`, а цвет ошибки — как `aria-invalid`.

`[color]` на группе — одна привязка на все её кнопки, поэтому палитру ошибки больше не нужно повторять для каждого варианта. А нативный `input` теперь несёт значение варианта, поэтому группа внутри обычной `<form>` отправляет это значение, а не строку `"on"`: корректно передаются только значения, приводимые к строке, и группе, участвующей в нативной отправке, по-прежнему нужен явный `[name]` — сгенерированный меняется между запусками приложения.

Сообщает `radio-name-and-aria`.
