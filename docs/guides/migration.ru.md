...
Редактор рисовал вокруг своей панели два `<div class="cdk-visually-hidden" aria-hidden="true" tabindex="0">`, чтобы заметить <kbd>Tab</kbd> за его пределы. Это ровно то, что запрещает правило `aria-hidden-focus` в axe-core, а так как панель отключает собственную ловушку фокуса, пользователь клавиатуры действительно на них попадал: две живые точки табуляции, которые ничего не озвучивают. Они удалены, граница определяется по первому и последнему табулируемому контролу самой панели, а вместе с ними удалён и `KbqFocusRegionItem` — директива, которая их помечала.

Вместе с ними переехала и семантика виджета. На хосте был `tabindex` и ни одного ARIA-атрибута, поэтому `<kbq-inline-edit>` был неизвестным элементом, который просто умеет принимать фокус. Теперь `role="button"`, `aria-expanded`, `aria-disabled` и доступное имя живут на `.kbq-inline-edit__view-content`, а пока содержимое режима просмотра интерактивно — на `.kbq-inline-edit__focus-anchor`: на хосте они вложили бы кнопку `kbqInlineEditMenu` внутрь роли виджета. Кольцо фокуса по-прежнему рисуется на хосте через `cdk-keyboard-focused`.

| Что было                                          | Что делать вручную                                                              |
| ------------------------------------------------- | ------------------------------------------------------------------------------- |
| `kbqFocusRegionItem` / `KbqFocusRegionItem`       | Убрать — граница Tab определяется на панели                                     |
| `modeAsReadonly()`                                | `mode()` — переписывается автоматически                                         |
| `edit.modeChange.subscribe(...)`                  | `toObservable(edit.mode)` — выход теперь принадлежит модели                     |
| Обработчик `[setValueHandler]`                    | Расширить параметр до `unknown` и сузить внутри                                 |
| Полный объект `KbqA11yLocaleConfiguration`        | Добавить новый обязательный ключ `edit` или перейти на частичный провайдер      |
| Селекторы `.kbq-inline-edit[tabindex]` / `:focus` | Нацелить на `.kbq-inline-edit__view-content` / `.kbq-inline-edit__focus-anchor` |

**Tab работает в редакторе-селекте.** Редактор сам открывает выпадающий список селекта, а тот живёт в собственном оверлее — поэтому <kbd>Tab</kbd> нажимался за пределами панели редактора, где его обработчик его не видел, а селект отменял клавишу и закрывался, оставляя поле открытым, а фокус — на `<body>`. Теперь клавиша перехватывается на документе, пока панель открыта, и поле сохраняется и передаёт очередь соседу, как любое другое. <kbd>Tab</kbd>, которым селект обходит собственный футер, не затронут.

**Режим теперь одно свойство.** Раньше его собирали из трёх: `protected signal()`, публичный `computed()` `modeAsReadonly`, существовавший только чтобы отдать его на чтение, и выход `modeChange`, который питала написанная руками подписка `toObservable(...).pipe(skip(1))`. Всё это — один `model()`. Он порождает выход с тем же именем и тем же значением, поэтому привязки `(modeChange)` не затронуты, но срабатывает он синхронно на запись, а не в микрозадаче позже, и отдельным свойством класса больше не является — программная подписка идёт через `toObservable(inlineEdit.mode)`. Взамен режим стал записываемым: `[(mode)]` открывает и закрывает редактор из хоста, чего раньше не умел никто, кроме `toggleMode()`.

**Сохранение теперь делает то, что обещает.** `commit()` был задокументирован как «та же валидация, что и при обычном сохранении», но проверял `KbqFormField.invalid` — кэшированный вердикт `ErrorStateMatcher`, то есть `touched || submitted`. Контрол, которого не касались, сообщал о себе как о валидном, и значение записывалось — ровно в задокументированном сценарии с селектом, сохраняющимся по `(selectionChange)`. Теперь `save()` сам помечает проецируемые контролы как touched и проверяет валидность самого контрола. Вторая половина этого переноса: раньше каждое нажатие клавиши в режиме редактирования помечало все контролы touched и окрашивало `required`-поле в ошибку, пока пользователь ещё набирал значение, которое сделает его валидным, — директиву-обход, сбрасывавшую `touched` при первом вводе, можно удалить.

Ещё три изменения доходят до потребителя. `saved` и `canceled` стали публичными, поэтому на них можно подписаться из `viewChild(KbqInlineEdit)`. Панель читает `--kbq-inline-edit-panel-shadow` — токен, который она объявляла и тут же обходила, так что его переопределение наконец меняет тень. А приватные классы `.kbq-mask`, `.kbq-mask__fade` и `.kbq-mask__container` переименованы в `.kbq-inline-edit__menu-mask*`, возвращая общее на вид пространство имён.

Выполняет `inline-edit-a11y-and-types`: чтения `modeAsReadonly` переписываются, остальное — сообщается.
