...
Запускать нечего: имена входов не менялись, переписывать разметку не нужно. Изменилось то, что компонент с ними делает.

**Имя ориентира берётся из локали.** Хост писал `aria-label="breadcrumb"` _привязкой_ — английским литералом, который вдобавок затирал `aria-label`, заданный потребителем на том же элементе. Теперь имя приходит из `KbqA11yLocaleConfiguration.breadcrumbs`, поэтому неанглоязычное приложение получает переведённое имя вместо английского, а `<nav kbq-breadcrumbs aria-label="…">` наконец переживает обнаружение изменений — именно это и позволяет различать две цепочки на одной странице. Элементная форма `<kbq-breadcrumbs>` теперь ещё и несёт `role="navigation"`; `<nav kbq-breadcrumbs>` не изменился, роль у него уже есть.

**`disabled` делает то, что обещает.** `KbqBreadcrumbs.disabled` был опубликован и задокументирован как блокировка взаимодействия, но не читался нигде: ни класса, ни атрибута, все ссылки оставались кликабельными. Теперь он отключает все пункты и кнопку раскрытия и выводит их из последовательности перехода по <kbd>Tab</kbd>. Разметка, которая привязывала его и обходила отсутствие эффекта, увидит цепочку в режиме только для чтения.

**`firstItemNegativeMargin` действует во всех режимах переноса.** Отступ висел на `.kbq-overflow-items` — обёртке, которую отрисовывает только `wrapMode="auto"`, поэтому в режимах `wrap` и `none` он молча ничего не делал. Теперь он на хосте. Цепочка в одном из этих двух режимов, которая никогда не задавала свойство, сдвинется влево на `--kbq-breadcrumb-item-negative-margin` — это и есть то выравнивание, которое подразумевалось; чтобы сохранить прежнее положение, привяжите `[firstItemNegativeMargin]="false"`.

**<kbd>PageUp</kbd> и <kbd>PageDown</kbd> снова прокручивают страницу.** Примитив roving-focus сопоставлял их с переходом к первому и последнему пункту и вызывал `preventDefault()`, поэтому цепочка в начале длинной страницы съедала клавиши постраничной прокрутки. <kbd>Home</kbd> и <kbd>End</kbd> по-прежнему выполняют эти переходы.

Две детали разметки, которые важны только стилям, дотягивающимся внутрь компонента: разделитель по умолчанию — обычный `<i class="kbq-breadcrumb__separator">`, а не пустой `KbqIcon`, поэтому классов `kbq-icon`/`kbq-contrast-fade` на нём больше нет; отрисованные пункты обёрнуты в контейнер с `role="list"`.

| Член                                                             | Что изменилось                                                                      |
| ---------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `RdxRovingFocusGroupDirective.dir`                               | `Direction \| null` со значением `null` — не задан, значит наследуется от документа |
| `RdxRovingFocusGroupDirective.handleMouseDown` / `handleMouseUp` | Удалены: вход фокуса больше не зависит от того, чем фокус был перемещён             |
| `RdxRovingFocusGroupDirective.onFocusableItemAdd` / `...Remove`  | Удалены: счётчик выводится из `focusableItems()`                                    |
| `RdxRovingFocusItemDirective.active`                             | По умолчанию `false`, поэтому `data-active` помечает один пункт, а не все           |
| `KbqBreadcrumbItem.text` / `.disabled`                           | Значения по умолчанию `''` и `false` вместо `undefined` под ненулевым типом         |

`<a kbq-button kbqBreadcrumb>` больше не перезаписывает `color` и `kbqStyle` кнопки: привязка `color` или `kbqStyle` рядом с `kbqBreadcrumb` сохраняется, а кнопка снова следует за окружающим `KbqButtonGroupRoot`. Разница видна только разметке, которая привязывала одно из двух и рассчитывала, что это будет проигнорировано.
#### Checkbox
