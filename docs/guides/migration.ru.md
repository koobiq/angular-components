...
**Спроецированное содержимое стало живым запасным вариантом.** `<kbq-markdown>`, который одновременно проецирует содержимое и привязывает `[markdownText]`, возвращается к спроецированному содержимому, как только вход опустеет. Раньше спроецированное содержимое читалось один раз и только если вход в этот момент был пустым.
