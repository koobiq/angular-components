...
`size` был последним входом на геттере и сеттере у спиннера — и именно поэтому автоматическая миграция на сигналы его пропустила: сеттер одновременно сохранял размер и вычислял радиус SVG-круга. Теперь радиус — это `computed`, а `size` — обычный `input()`. `id`, `value` и `mode` стали сигналами ещё в 20.2.0 и не изменились.

| Что было                             | Как мигрировать вручную                                                        |
| ------------------------------------ | ------------------------------------------------------------------------------ |
| `.size`                              | Читать как `size()` — переписывается за вас                                    |
| `.size = …`                          | Привязать `[size]` в шаблоне — вход доступен только на чтение                  |
...
**`value` стал `numberAttribute`-входом.** `value="40"` раньше передавал строку `"40"`, которую арифметика процентов приводила к числу случайно. Привязка `null` или `undefined` раньше давала `0`, а теперь даёт `NaN` — передавайте число или не привязывайте вход вовсе.

Закрывается схематиком `progress-spinner-signals`: чтения `size` переписываются, остальное сообщается в отчёте.
