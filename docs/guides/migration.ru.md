...
Главное здесь — правка платформы, которая никогда не была частью компонента. Конструктор `KbqNumberInput` вызывал `Object.defineProperty(Object.getPrototypeOf(this.nativeElement), 'valueAsNumber', …)` — а это `HTMLInputElement.prototype`, а не сам элемент. Создание одного числового поля koobiq подменяло `valueAsNumber` для **каждого `<input>` в приложении**, включая сторонние виджеты и нативные поля `type="date"`/`type="number"`, — навсегда и без отката. Подмена считала `,` десятичным разделителем и возвращала `null` там, где спецификация DOM требует `NaN`. Патч удалён; чтение числа с учётом локали теперь живёт на самой директиве: `numberInput.valueAsNumber`.

`KbqNumberInput` также объявлял `implements KbqFormFieldControl<any>` и не заполнял ничего из этого. Приватному полю `control`, стоящему за `ngControl`, нигде в файле не присваивалось значение, поэтому `ngControl` всегда был `undefined`, а `id`, `placeholder`, `empty`, `required` и `errorState` существовали только ради интерфейса. Во время выполнения ничего не ломалось, потому что провайдер `KbqFormFieldControl` живёт на соседней директиве `KbqInput`, которая объявлена на том же хосте, — но члены были публичными, и чтение `numberInputRef.errorState` давало `undefined` без ошибки типов. Интерфейс и шесть членов удалены.

| Что было                                                         | Что делать вручную                                                        |
| ---------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `numberInput.ngControl` / `.errorState` / `.empty` / `.required` | Читать их у соседнего `KbqInput` или у `<kbq-form-field>`                 |
| `inputElement.valueAsNumber`                                     | `numberInput.valueAsNumber` либо значение привязанного контрола           |
| `import { MinValidator, MaxValidator }`                          | `KbqMinValidator`, `KbqMaxValidator` — старые имена оставлены на минор    |
| `import { MIN_VALIDATOR, MAX_VALIDATOR }`                        | `KBQ_MIN_VALIDATOR`, `KBQ_MAX_VALIDATOR` — старые имена оставлены         |
| `<input kbqNumberInput type="number">`                           | Убрать атрибут — директива сбрасывает его в `text` и пишет предупреждение |
| `@use '@koobiq/components/input/input-theme'`                    | `@use '@koobiq/components/input/input-typography'`                        |

Имена `MinValidator` / `MaxValidator` без префикса совпадают с экспортами `@angular/forms`, поэтому импорт обоих в одном файле перекрывал валидатор фреймворка. У `type="number"` был видимый симптом: нативное числовое поле применяет алгоритм проверки значения при присваивании и отбраковывает всё, что рендерит директива, как только в значении появляется разделитель разрядов или запятая в дробной части. Любое дробное значение — и любое значение ≥ 1000 при включённом `withThousandSeparator` — молча исчезало из поля, хотя модель его сохраняла.

Исправления, для которых мигрировать нечего: `min`, `max`, `step` и `bigStep` приводят значение к числу, поэтому статический `min="3"` хранит число `3`, а не строку `"3"`, а объявления `ngAcceptInputType_*` позволяют статической форме атрибута компилироваться при `strictTemplates`; валидаторы больше не применяют `parseInt` к привязанному значению, поэтому `[min]="0.5"` проверяет по `0.5`, а не по `0`, а привязанный `[min]="0"` попадает в DOM, а не отбрасывается проверкой на истинность; шаг считается в целочисленном пространстве по десятичной шкале, поэтому одно нажатие стрелки на `1.005` с `step="0.001"` рисует `1,006`, а не `1,0059999999999998`; `startFormattingFrom` читается из входа, а не только из локали, вместо того чтобы быть объявленным и никем не использоваться; `KbqNumberInput` получил семантику spinbutton (`role`, `aria-valuenow`, `aria-valuetext`, `aria-valuemin`, `aria-valuemax`, `inputmode`); а `KbqInputPassword` выдаёт идентификаторы в собственном пространстве `kbq-input-password-`, поэтому страница с текстовым полем и полем пароля больше не отдаёт два элемента с одинаковым `id`, и `<label for>` формы указывает на свой контрол.

Сообщает `input-number-surface`.
#### Loader overlay
