...
Иконки попадали в дерево доступности, а кнопки-иконки кнопками не были. Теперь `<i kbq-icon>` рендерится с `aria-hidden="true"`: у иконки нет собственного текста, и почти везде, где её используют библиотека и её потребители, она повторяет подпись, которая уже есть в кнопке, ссылке или строке рядом. Иконка, которая является единственным носителем смысла, должна сказать об этом сама:

```html
<i kbq-icon="kbq-triangle-exclamation_16" role="img" aria-hidden="false" aria-label="Ошибка"></i>
```

`kbq-icon-button` не скрывается никогда, а на элементе, который не является нативным `<button>` — это 129 из 180 мест использования в самом репозитории, — он добавляет недостающую семантику: `role="button"`, `aria-disabled` и активацию по Enter и Space. Если событие уже `defaultPrevented`, он не вмешивается, поэтому обработчик на том же элементе, вызывающий `preventDefault()`, остаётся единственным владельцем клавиши. В режиме разработки компонент предупреждает, если у него нет ни `aria-label`, ни `aria-labelledby`: глиф не даёт ему никакого доступного имени.

`KbqIconButton` и `KbqIconItem` предоставляют DI-токен `KbqIcon`. Метаданные хоста наследуются через `ɵɵInheritDefinitionFeature`, а DI-токены — нет, поэтому `contentChild(KbqIcon)` раньше проходил мимо кнопки-иконки, несущей класс `.kbq-icon`. Именно из-за этого ссылка, содержащая только кнопку-иконку, не получала отступов, а элемент навигационной панели, построенный на icon-item, никогда не сворачивался.

| Что было                        | Что делать вручную                                                            |
| ------------------------------- | ----------------------------------------------------------------------------- |
| `small` на обычном `[kbq-icon]` | Убрать — его никто не читал; `KbqIconButton.small` не затронут                |
| `contentChild(KbqIcon)`         | Сузить запрос, если нужна только «голая» иконка                               |
| `(keydown…)` на кнопке-иконке   | Вызвать `preventDefault()`, чтобы сохранить единоличную обработку клавиши     |
| `[tabindex]` на кнопке-иконке   | `number \| null` вместо `any`; числовая строка по-прежнему приводится к числу |
| `.name` у наследника `KbqIcon`  | Переопределять защищённый `appliesMaxHeight`                                  |

Три исправления, для которых мигрировать нечего. Поток разрешения SVG больше не завершается на первом неразрешённом имени — а конфигурация со шрифтовыми иконками по умолчанию гарантирует его на первом же кадре, — поэтому устаревший `<svg>` удаляется на ветке отката, встроенный `max-height` не только выставляется, но и сбрасывается, и элемент с меняющимся `[kbq-icon]` рисует именно ту иконку и того размера, которые у него запросили. `autoColor` стал реактивным: включение после инициализации подписывается, выключение сбрасывает `hasError`. И токены `--kbq-icon-*-color` больше не объявляются на `.kbq-icon`: они читаются с дизайн-токеном в качестве запасного значения `var()`, поэтому контейнер наконец может перекрасить иконки внутри себя — а чтение такого токена через `getComputedStyle` теперь возвращает пустую строку.

Сообщает `icon-semantics-and-api`.
