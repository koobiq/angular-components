...
7. **20.3.0**: контейнер оверлея панели действий.
8. **20.3.0**: устаревание прежнего splitter.
9. **21.0.0**: удаление механизма понижения оверлея.
10. **21.0.0**: переход API app-switcher на сигналы.
11. **21.0.0**: ревью кнопки — атрибуты хоста, владение в группе и стили.
12. **21.0.0**: поддерживаемые цвета кнопки — свой дефолтный цвет у каждого стиля.
13. **21.0.0**: ревью группы кнопок — ARIA-семантика, навигация с клавиатуры и сигнальные входы.
14. **21.0.0**: ревью поля формы — сигналы, доступность и удаление `mixinColor`.
15. **21.0.0**: ревью сервиса темизации — сигналы, режим `auto` и сохранение выбора из коробки.
16. **21.0.0**: явные prefix- и suffix-слоты для содержимого тегов.
17. **21.0.0**: устаревание overlayscrollbars-реализации Scrollbar.
18. **21.0.0**: типизация слоя локализации — типизированный `getParams`, частичные данные локали и сигналы.
19. **21.0.0**: `multiple` у списка и дерева стал обычным изменяемым input.
20. **21.0.0**: ревью компонентов — сокрытие внутренних членов, переход входов на сигналы и исправления поведения.
21. **21.0.0**: удаление устаревших событий file-upload `fileQueueChanged`/`fileQueueChange`.
22. **21.0.0**: хранилище состояния аккордеона переехало в `core` и стало общим для всех компонентов.
23. **21.0.0**: сохранение состояния аккордеона включено по умолчанию, ключ выводится из документа.
24. **21.0.0**: сохранение состояния дерева включено по умолчанию, ключ — значение узла из контрола.
25. **21.0.0**: вкладки, боковая панель и контент-панель запоминают выбор пользователя, по умолчанию.
26. **21.0.0**: `filter-bar` запоминает выбранный фильтр и его изменения, по умолчанию.
...
### 7. Контейнер оверлея панели действий (20.3.0)

`KbqActionsPanelConfig.overlayContainer` теперь задаёт элемент, **внутрь которого** отрисовывается панель, а не якорь, относительно которого она позиционировалась. Панель прижимается к нижнему краю по центру этого элемента и меняет размер вместе с ним.

Что это меняет для кода, который уже задаёт опцию:

- **Элемент с `overflow: hidden` обрезает панель**, а элемент, прокручивающий собственное содержимое, уносит её вместе с ним. При `overflow: visible` анимация появления рисует панель ниже элемента.
- **Элемент изменяется, пока панель открыта**: в нём появляется узел с оверлеем, а `static` повышается до `position: relative` — оверлей позиционируется относительно него.
- **`maxWidth` больше не игнорируется** при заданном `overlayContainer`.
- **Глобально предоставленный `OverlayContainer` обходится**, пока опция задана.

#### Запуск миграции

Схематик `actions-panel-overlay-container` зарегистрирован на 20.3.0: `ng update`, проходящий через эту версию, запустит его сам, а проекту, который её уже перешагнул, схематик нужно запустить вручную:

```bash
ng g @koobiq/components:actions-panel-overlay-container --project <your project>
```

Он только сообщает о местах вызова: переписывать нечего — опция та же, изменился её смысл.

### 8. Устаревание прежнего splitter (20.3.0)

`@koobiq/components/splitter` теперь отдаёт переписанный splitter: он раскладывает панели через CSS grid и добавляет ограничения размеров, сворачивание, точки прилипания, работу с клавиатуры и ARIA-шаблон `separator`. Прежняя реализация — `KbqSplitterComponent`, `KbqSplitterAreaDirective`, `KbqGutterDirective`, `KbqGutterGhostDirective`, `KbqSplitterModule` и перечисление `Direction` — переехала в `@koobiq/components/splitter/deprecated` без единого изменения и будет удалена в одной из следующих мажорных версий.

**Большинство старых импортов ломается громко.** `KbqSplitterComponent`, `KbqSplitterAreaDirective`, `KbqGutterDirective`, `KbqGutterGhostDirective` и `Direction` живут только по пути `/deprecated`, поэтому сборка останавливается на `TS2305: … has no exported member`.

**`KbqSplitterModule` ломается молча — ради этого случая миграция и нужна.** Класс с таким именем есть по обоим путям, селектор `kbq-splitter` — тоже, поэтому импорт, который не переписали, продолжает компилироваться, но подтягивает уже новый splitter. Тот проецирует дочерние `kbq-splitter-panel` вместо хостов `[kbq-splitter-area]` и принимает `orientation` вместо `direction`, так что группа отрисовывается без панелей и без разделителей.

Шаблоны править не нужно: как только импорт указывает на `/deprecated`, `<kbq-splitter>` с хостами `[kbq-splitter-area]`, `direction`, `useGhost`, `hideGutters` и `gutterSize` работает ровно как раньше.

#### Запуск миграции

Схематик `splitter-deprecated-path` зарегистрирован на 20.3.0: `ng update`, проходящий через эту версию, запустит его сам, а проекту, который её уже перешагнул, схематик нужно запустить вручную:

```bash
ng g @koobiq/components:splitter-deprecated-path --project <your project>
```

Он переписывает спецификатор `@koobiq/components/splitter` в `@koobiq/components/splitter/deprecated` во всех файлах `.ts`; повторный запуск ничего не портит. На новый splitter он не переводит — это отдельная ручная работа: области становятся `kbq-splitter-panel`, `direction` — `orientation`, а размеры переезжают с `min-width` и CSS `flex` на инпуты `size` и `minSize`.

### 9. Удаление механизма понижения оверлея (21.0.0)
...
### 10. Переход API app-switcher на сигналы (21.0.0)
...
### 12. Поддерживаемые цвета кнопки (21.0.0)
...
### 15. Ревью сервиса темизации (21.0.0)
...
### 16. Явные слоты содержимого тегов (21.0.0)
...
### 17. Устаревание overlayscrollbars-реализации Scrollbar (21.0.0)
...
### 18. Типизация слоя локализации (21.0.0)
...
### 19. Множественный выбор в списке и дереве (21.0.0)
...
### 21. Удаление устаревших событий file-upload (21.0.0)
...
### 22. Хранилище состояния аккордеона переехало в core (21.0.0)
...
### 23. Сохранение состояния аккордеона включено по умолчанию (21.0.0)
...
### 24. Сохранение состояния дерева включено по умолчанию (21.0.0)
...
### 25. Сохранение состояния вкладок, боковой панели и контент-панели включено по умолчанию (21.0.0)
...
### 26. Сохранение состояния filter-bar включено по умолчанию (21.0.0)
