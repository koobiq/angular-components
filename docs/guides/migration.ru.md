...
**Провайдеры `{ provide: KBQ_DROPDOWN_HOST, … }` удаляются** вместе со ставшим невалидным импортом `KBQ_DROPDOWN_HOST` и с массивом `providers`, если после удаления он остался пустым.
...
**Формы провайдера, которые схематик не смог переписать.** `useFactory`, возвращающий объектный литерал, или вызов `inject(KBQ_DROPDOWN_HOST)` попадают в предупреждение об оставшемся `KBQ_DROPDOWN_HOST`. Их нужно убрать вручную.
