...
Вертикальный разделитель задавался как `height: 100%`. Процентная высота считается от _определённой_ высоты родителя, а у flex-строки её нет — поэтому разделитель получал `0` и исчезал ровно в тех панелях инструментов, ради которых он и существует. Каждый вызывающий код в этом репозитории обнаружил это самостоятельно и зафиксировал высоту у себя, двое — через `!important`. Теперь разделитель растягивается на свою flex- или grid-линию (`align-self: stretch`), что заодно переопределяет `align-items`, который иначе центрировал бы его с нулевой высотой.

Плата — на другом конце: за пределами такой линии (ячейка таблицы, обычный блок) вертикальный разделитель больше не берёт высоту у родителя. Задайте её через `--kbq-divider-size-vertical-height` на самом разделителе — там объявлено значение по умолчанию.
...
Локальный `height`, который существовал только ради обхода этого схлопывания, можно убрать. Как и `!important` на отступах: отступы `paddings` задаются одним классом, ориентация подбирается через `:where()`, поэтому они больше не перебивают собственный класс потребителя.

У компонента появилась и семантика разделителя: он отдаёт `role="separator"`, а при `vertical` — ещё и `aria-orientation="vertical"`. Написанные вручную `role` или `aria-orientation` на `<kbq-divider>` теперь дублируют его. Новый вход `decorative` отдаёт вместо этого `role="presentation"` — для разделителя, который лишь повторяет границу, уже переданную заголовком, группой или самой разметкой; написанный вручную `aria-hidden` по-прежнему учитывается и означает то же самое.

| Паттерн                              | Ручная миграция                                                                  |
| ------------------------------------ | -------------------------------------------------------------------------------- |
| `.vertical = …` / `.paddings = …`    | Привязка `[vertical]` / `[paddings]` — сигнальный вход не принимает присваивание |
| `.vertical` / `.paddings`            | Читать как вызовы                                                                |
| `<kbq-divider role="…">`             | Убрать: компонент отдаёт `role="separator"` сам                                  |
| `<kbq-divider aria-orientation="…">` | Убрать: значение следует за `vertical`                                           |
| `height` у вертикального разделителя | Убрать или перенести в `--kbq-divider-size-vertical-height`                      |

Оба входа сохранили имена, поэтому привязки `[vertical]` и `[paddings]` не меняются. Сообщает `divider-signals-and-aria`.
