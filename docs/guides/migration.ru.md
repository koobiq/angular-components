...
| ---------------------------------------------------------- | -------------------------------------------------------------------------------- |
| `.displayWith` / `.autoActiveFirstOption` / `.openOnFocus` | Читать как вызовы — переписывается за вас                                        |
...
**`autoActiveFirstOption`, `openOnFocus` и `kbqAutocompleteDisabled` стали `booleanAttribute`-входами**, поэтому атрибут без значения означает `true`.

**Привязка `[autoActiveFirstOption]` перекрывает `KBQ_AUTOCOMPLETE_DEFAULT_OPTIONS`** даже при значении `undefined` — значение по умолчанию из токена применяется только к входу, который никто не привязал. Чтобы решал токен, не привязывайте вход вовсе.

**Формат генерируемых `id` панели изменился** с `kbq-autocomplete-1` на `kbq-autocomplete-a1`. Это значение `aria-owns` у триггера.
