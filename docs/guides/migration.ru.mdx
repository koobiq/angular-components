...
```bash
ng g @koobiq/angular-components:new-icons-pack --project PROJECT_NAME
```
...
```bash
ng g @koobiq/angular-components:css-selectors --fix=true --project PROJECT_NAME
```

Для ручного контроля добавьте `--fix=false`. Скрипт подсветит места, где нужно удалить (заменить) цвета и названия типографики:

```bash
ng g @koobiq/angular-components:css-selectors --fix=false --project PROJECT_NAME
```
...
```bash
ng g @koobiq/angular-components:loader-overlay-size-attr --project PROJECT_NAME
ng g @koobiq/angular-components:empty-state-size-attr --project PROJECT_NAME
```
...
```bash
ng g @koobiq/components:v20-upgrade --project PROJECT_NAME
```
...
```bash
ng g @koobiq/components:filter-bar-signals --project PROJECT_NAME
```
...
```bash
ng g @koobiq/components:actions-panel-overlay-container --project PROJECT_NAME
```
...
```bash
ng g @koobiq/components:app-switcher-signals --project PROJECT_NAME
```
...
```bash
ng g @koobiq/components:button-supported-colors --project PROJECT_NAME
```
...
```bash
ng g @koobiq/components:form-field-signals --project PROJECT_NAME
```
...
```bash
ng g @koobiq/components:SCHEMATIC_NAME --project PROJECT_NAME
```
