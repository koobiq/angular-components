...
**`firstItemNegativeMargin` действует во всех режимах переноса.** Отступ висел на `.kbq-overflow-items` — обёртке, которую отрисовывает только `wrapMode="auto"`, поэтому в режимах `wrap` и `none` он молча ничего не делал. Теперь его несёт и `.kbq-breadcrumbs__list` — обёртка, которая отрисовывается в этих двух режимах. Но не хост: отрицательный внешний отступ на flex-элементе вычитается из места, которое ему выделяет flex-раскладка, поэтому цепочку перестало бы сжимать, а режим `auto` — он решает, что сворачивать, измеряя собственную ширину, — счёл бы, что помещаются все пункты, и не свернул бы ничего. Цепочка в одном из этих двух режимов, которая никогда не задавала свойство, сдвинется влево на `--kbq-breadcrumb-item-negative-margin` — это и есть то выравнивание, которое подразумевалось; чтобы сохранить прежнее положение, привяжите `[firstItemNegativeMargin]="false"`.
