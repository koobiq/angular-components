...
`name` — несущий контракт радиокнопки, и он нарушался в обе стороны. `UniqueSelectionDispatcher` из CDK глобален для приложения и рассылает каждое уведомление всем слушателям; сравнение по `name` внутри слушателя — единственная изоляция. `<kbq-radio-button>` вне группы не получал имени, поэтому `undefined === undefined` объединял все одиночные радиокнопки приложения в одну группу выбора: выбор одной снимал выбор со всех остальных — в других маршрутах, оверлеях и диалогах. Теперь кнопка получает имя, уникальное для нее самой, и **две одиночные радиокнопки, которые раньше сбрасывали друг друга, независимы**. Разметку, которая на это полагалась, нужно поправить: положите кнопки в группу или задайте им одинаковый `[name]`.
