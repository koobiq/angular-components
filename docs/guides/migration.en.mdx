...
- **21.0.0**: the agent skill for AI coding agents is refreshed where it was installed.
...
| `closeDirective.sidepanelResult = …`                         | An input signal now: bind `kbq-sidepanel-close="…"` in the template        |
| `closeDirective.sidepanelResult` read                        | `sidepanelResult()`                                                        |
...
### Agent skill refresh (21.0.0)

If you set up the Koobiq skill for AI coding agents with `ng generate @koobiq/components:agent-skills` or during `ng add`, the update brings it to 21.0.0 as well: the copies in `.claude/skills/koobiq-angular` and `.agents/skills/koobiq-angular`, and the managed block of rules in `AGENTS.md` and `CLAUDE.md`. See [AI agents](/en/main/ai-agents/overview).

#### Running the migration

The `agent-skills-refresh` schematic runs automatically:

```bash
ng update @koobiq/components@21
```

A workspace that never installed the skill is not touched. Files of the skill you changed are kept and listed in the output: merge the new version into them by hand, or replace them:
