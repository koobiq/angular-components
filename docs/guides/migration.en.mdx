...
- **Up to 18.5.3**: a safe baseline with updated theming and icons.
- **18.6**: token update.
- **18.22**: component attribute changes.
- **20.0.0**: the move to Angular 20: removal of deprecated APIs and package renames.
- **20.2.0**: the move of the filter-bar API to signals.
- **20.2.0**: one shared mechanism for dropdown panel width.
- **21.0.0**: the actions panel overlay container.
- **21.0.0**: legacy splitter deprecation.
- **21.0.0**: removal of the overlay demotion mechanism.
- **21.0.0**: the move of the app-switcher API to signals.
- **21.0.0**: the button review — host attributes, group ownership and styles.
- **21.0.0**: button supported colors — a default color of its own per style.
- **21.0.0**: the button-toggle review — ARIA semantics, keyboard navigation and signal inputs.
- **21.0.0**: the form-field review — signals, accessibility and the removal of `mixinColor`.
- **21.0.0**: the theme service review — signals, `auto` mode and built-in persistence.
- **21.0.0**: explicit prefix and suffix slots for tag content.
- **21.0.0**: deprecation of the overlayscrollbars-based Scrollbar implementation.
- **21.0.0**: the locale layer typing — a typed `getParams`, partial locale data and signals.
- **21.0.0**: `multiple` on the selection list and tree became a real, changeable input.
- **21.0.0**: the component review — closed internals, signal inputs and the behavior fixes it uncovered.
- **21.0.0**: removal of the deprecated file-upload `fileQueueChanged`/`fileQueueChange` outputs.
- **21.0.0**: the accordion state store moved into `core`, shared by every component that persists state.
- **21.0.0**: accordion state saving is on by default, keyed on the document instead of instantiation order.
- **21.0.0**: tree state saving is on by default, keyed on the value the tree control gives each node.
- **21.0.0**: tabs, sidebar and content-panel remember what the user changed, on by default.
- **21.0.0**: the filter bar remembers the selected filter and its edits, on by default.
- **21.0.0**: the tag list cleaner removes the tags itself and leaves the disabled ones in place.
...
### Panel width unification (20.2.0)
...
### Actions panel overlay container (21.0.0)
...
### Legacy splitter deprecation (21.0.0)
...
### Overlay demotion removal (21.0.0)
...
### Button supported colors (21.0.0)
...
### Explicit tag content slots (21.0.0)
...
### Scrollbar overlayscrollbars implementation deprecation (21.0.0)
...
### List and tree multiple selection (21.0.0)
...
**`firstItemNegativeMargin` applies in every wrap mode.** The offset hung off `.kbq-overflow-items`, a wrapper that only `wrapMode="auto"` renders, so it silently did nothing in `wrap` and `none`. It is carried by `.kbq-breadcrumbs__list` as well now — the wrapper those two modes render instead. Not by the host: a negative margin on a flex item is subtracted from the space the flex algorithm hands it, so the trail would stop being squeezed, and `auto` mode — which decides what to collapse by measuring its own box — would conclude that every item fits and collapse nothing. A trail in one of those two modes that never set the property shifts left by `--kbq-breadcrumb-item-negative-margin` — the alignment it was always supposed to have; bind `[firstItemNegativeMargin]="false"` to keep the old position.
...
| Member                                                           | Change                                                                                   |
| ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `RdxRovingFocusGroupDirective.dir`                               | `Direction \| null` (the Angular CDK type), default `null` — unset inherits the document |
...
`vertical` and `paddings` were accessor pairs carrying a coercion by hand. They are `input()`s with `booleanAttribute` now, so a read is a call and a write is a template binding. Both kept their names, so `[vertical]` and `[paddings]` bindings are untouched — only TypeScript that reaches the component through a reference has to change.
...
The trade is at the other end: outside such a line — a table cell, a plain block — a vertical divider no longer takes its height from the parent. A table cell is worth calling out, because it is one of the few places a percentage height did resolve; a divider sitting in one used to work and now needs a height of its own. Give it one through `--kbq-divider-size-vertical-height`, declared on the divider itself, which is where its default lives.
...
That cuts both ways. A margin you already set, which was quietly losing to the old three-class chain, now applies. `kbq-actions-panel` in this repository had asked for `8px` around its divider and been given the divider's `4px` for as long as the token existed; it gets `8px` now. Check your own spacing rather than assuming nothing moved.

`align-self: stretch` is set for both orientations, not just the vertical one. A horizontal divider in a flex column that sets `align-items` used to shrink to its empty content — nothing to see — and now spans the column. That is the same bug seen from the other side, but it does mean a horizontal divider can become visible where it previously was not.

**The token carries the alignment with it.** A flex item with a height of its own is never stretched, so a divider that keeps `align-self: stretch` alongside one would silently fall back to `flex-start` and override the alignment its row asked for. The divider reads the same token for both properties, so setting a height hands the alignment back to the row in the same stroke. There is nothing else to write:
...
The row is what decides where that divider sits. In a row with no `align-items` of its own, items are stretched and a divider with a height lands at the top — set `align-self` there if you want it centred, the same as you would for any other item of a definite size.

The component also has separator semantics. It renders `role="separator"`, and `aria-orientation="vertical"` when `vertical` is set, so a hand-rolled `role` or `aria-orientation` on `<kbq-divider>` is now a duplicate. The new `decorative` input renders `role="presentation"` instead, for a divider that only repeats a boundary a heading, a group or the layout already conveys; a hand-rolled `aria-hidden` is still honoured and says the same thing.
...
A comparison such as `divider.vertical === true` is the case worth hunting for. It keeps compiling and quietly compares against the signal itself, so it is always `false` and nothing complains.

Reported by `divider-signals-and-aria`, which resolves a member access against the receiver's declared type — so a `vertical` on something that is not a `KbqDivider` is left alone, and a receiver whose type is only inferred is not recognised.
...
- <kbd>Home</kbd> and <kbd>End</kbd> move the highlight in a dropdown without a search field, where they used to scroll
  the page.
...
| `KbqEmptyState.size`                 | `@Input()`, writable                    | `input()`                    |
...
**`size` was the last decorator on the component, and the file-upload subclass is why.** `KbqFileUploadEmptyState` extends `KbqEmptyState` and assigned `this.size = 'big'` right after `super()` — an assignment an `input()` cannot take, which is what held the migration back. That subclass redeclares the input with its own default now, `override readonly size = input<KbqDefaultSizes>('big')`, so `<kbq-file-upload-empty-state>` still renders at `big` with no binding and a `[size]` binding still wins. Nothing changes for anyone rendering it; what changes is that reading `size` off a `KbqEmptyState` is a call, and assigning to it is gone.

The report for this one is matched on the receiver rather than the member, because `size` is too common a property name to touch blind — `file.size` and `blob.size` sit in the same files. Only a read like `emptyState.size` is flagged; a read through a variable named something else is not reported, and the schematic's summary says so instead of letting you assume it was covered.

**The four theme tokens were renamed.** `--kbq-empty-state-title` and `--kbq-empty-state-color` read as a size token and a component color, but they are the title color and the _text_ color; the error pair repeated the shape. They are `--kbq-empty-state-title-color`, `--kbq-empty-state-text-color`, `--kbq-empty-state-error-title-color` and `--kbq-empty-state-error-text-color` now. The old names are removed, not deprecated: an override of one is ignored and a `var()` read of one resolves to nothing, in both cases silently. Inside the library the rename touched `multiple-file-upload.component.scss`, which set the title and text colors of the empty state for its default, disabled and error states.

Two fixes with nothing to migrate. `empty-state.scss` loads its own token layer instead of relying on a second `styleUrls` entry, so anything reusing the stylesheet across a package boundary gets the custom properties together with the rules that read them; such a reuse used to take the rules alone, and every `max-width` in it resolved to `none`. The library has exactly one such reuse: `KbqFileUploadEmptyState` lists `../empty-state/empty-state.scss` in its own `styleUrls`. Its caption was inheriting the body color, because the declaration resolved to nothing — so the arriving token layer would have turned it into the secondary foreground. `dropzone.scss` pins `--kbq-empty-state-text-color` to `var(--kbq-foreground-contrast)` instead, and the dropzone renders exactly as before. And the title is written as a real heading everywhere in the library — it carries `subheading` typography at `size="normal"` and `headline` at `size="big"`, so `<h2 kbq-empty-state-title>` is what the component asks for, with the user-agent margin zeroed so nothing moves. An empty state inserted into a page that has already been read still needs `role="status"` — `role="alert"` for the error variant — on the host; the component adds none of its own.
...
The navbar finished its move to signals, and `KbqNavbarItem` and `KbqNavbarBrand` own a `KbqTooltipTrigger` instead of extending one. Template bindings keep working — `[expanded]`, `[collapsable]`, `[collapsedText]`, `[kbqPlacement]` and `[kbqTrigger]` are unchanged — except the tooltip content of an item or a brand.

**`[kbqTooltip]` on `kbq-navbar-item` and `kbq-navbar-brand` is `[tooltipText]` now.** `kbqTooltip` is the tooltip directive's selector, so wherever `KbqToolTipModule` was imported next to the navbar, the host matched that directive a second time: `NG0309` in development, two tooltips in production. On other elements `kbqTooltip` is unchanged.

| Pattern                                                                                   | Manual migration                                                                                                                              |
| ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `kbqTooltip` on an item or a brand                                                        | `tooltipText`, in every binding form — rewritten for you                                                                                      |
| `navbar.expanded` / `configuration` / `openOver` / `tabIndex`                             | Read as calls, write `expanded` and `tabIndex` with `.set(…)` — rewritten for you                                                             |
| `item.isCollapsed` / `collapsable` / `collapsedText`, `brand.collapsed` / `collapsedText` | Read as calls — rewritten for you                                                                                                             |
| `horizontal` / `vertical` of `KbqNavbarRectangleElement`                                  | `isHorizontal()` / `isVertical()`, and `orientation = 'horizontal'` for a write — rewritten for you                                           |
| `item.disabled` / `brand.disabled`                                                        | Removed: they suppressed the tooltip. Read `item.navbarFocusableItem.disabled`, and control the tooltip with `[kbqTooltipDisabled]`           |
| `item.content` / `show()` / `hide()` / `visibleChange`                                    | Go through the owned trigger: `item.tooltip.content`, `item.tooltip.show()`                                                                   |
| `item.updateDropdown()`                                                                   | Private now: the item refreshes its dropdown itself. Delete the call                                                                          |
| `logo.hovered` / `title.hovered`                                                          | Removed: bind the pointer enter and leave events on the element                                                                               |
| `navbar.animationDone`                                                                    | Removed: it has not emitted since the toggle animation was removed, so a subscription never ran. Bind `(expandedChange)` to react to a toggle |
| `KbqNavbarContainerPositionType`                                                          | Removed: it had no consumer. Delete the import                                                                                                |
| `kbq-navbar-item[disabled]` in a stylesheet                                               | A disabled item renders `aria-disabled="true"` and `.kbq-disabled`, never the `disabled` attribute                                            |

**An item or brand without a title no longer suppresses its tooltip.** `[tooltipText]` on an icon-only item shows without `[kbqTooltipDisabled]="false"`.

**Screen readers announce the navbar.** The horizontal and the vertical navbar are both `role="navigation"` landmarks with an `aria-label` input. An item written as `<kbq-navbar-item>` rather than `<a>` or `<button>`, and not wrapping a button or a form field, is `role="button"` and answers to Enter and Space; `<kbq-navbar-divider>` is `role="separator"`. A collapsed item or brand publishes its title as `aria-label`, so give an icon-only item without a title an `aria-label` of its own — a tooltip never named it.

**`Ctrl+/` toggles one vertical navbar** — the one holding focus, or else the first on the page — instead of all of them at once.

**The navbar left the CDK overlay layer.** It reads `--kbq-navbar-z-index`, `--kbq-navbar-toggle-z-index` and `--kbq-navbar-vertical-open-over-z-index` (990, 991 and 989), so overlays render above it. The hard-coded sizes became tokens too: `--kbq-navbar-vertical-size-expanded-width`, `--kbq-navbar-item-vertical-size-height`, `--kbq-navbar-toggle-size-circle` and `--kbq-navbar-brand-vertical-size-title-max-width`. `!important` is gone from the collapsed brand title and the item icon color, so an override that relied on losing to it may start applying.

Additive: an icon marked with `kbqNavbarItemSuffix` is shown after the title. Without the attribute every icon of an item is rendered before the title, wherever it is written. Two fixes come for free: a vertical navbar item with a clipped title shows its tooltip, and a horizontal navbar collapses its items before its content overflows.

Handled by `navbar-signals-and-aria`: the `kbqTooltip` binding, the signal reads and writes and the orientation booleans are rewritten, the rest is reported.
...
### File-upload deprecated output removal (21.0.0)
...
### Accordion state store moved to core (21.0.0)
...
### Accordion state saving on by default (21.0.0)
...
### Tree state saving on by default (21.0.0)
...
### Tabs, sidebar and content-panel state saving on by default (21.0.0)
...
### Filter-bar state saving on by default (21.0.0)
...
### Tag list cleaner clears by itself (21.0.0)
