...
Every color moved behind a `--kbq-table-*` token — `--kbq-table-header-text`, `--kbq-table-body-text`, `--kbq-table-border-color`, `--kbq-table-states-hover-background` and `--kbq-table-sticky-header-background` — and the row radius behind `--kbq-table-size-row-border-radius`. No value changed: a pinned header still takes `--kbq-background-bg`, and the point of the token is that a table on a card, in a modal or in a sidepanel can now be pointed at that surface — `--kbq-table-sticky-header-background: var(--kbq-background-card)` — instead of out-specifying `.kbq-table > thead > tr > th`.
