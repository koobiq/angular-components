...
**The flag tokens are declared at zero specificity, and `--kbq-flag-empty-background` is an opaque neutral instead of the translucent `disabled` state.** An override that used to lose to `.kbq-flag` on source order now wins, so a redundant `!important` can go. The `empty` placeholder no longer takes on the hue of the surface behind it.
