...
**The flag tokens are declared at zero specificity, and `--kbq-flag-empty-background` is an opaque neutral instead of the translucent `disabled` state.** An override that used to lose to `.kbq-flag` on source order now wins, so a redundant `!important` can go. The `empty` placeholder no longer takes on the hue of the surface behind it. One flag's proportions no longer need a class at all: the new `aspectRatio` input takes a CSS `aspect-ratio` value directly, and the variable stays the way to set one ratio for a whole group.
