...
### Контент-панель для выделенной строки

Компонент [Content panel](/ru/components/content-panel) показывает быстрый просмотр строки, выделенной в таблице, сдвигая таблицу, а не перекрывая её.

<Example id="ag-grid-and-content-panel" />

### Длинные списки в ячейках

Длинный список можно уместить в одну строку ячейки: [Overflow items](/ru/components/overflow-items) скрывает то, что не помещается по ширине, и пересчитывает скрытое при изменении ширины колонки. Скрытые элементы можно раскрыть прямо в ячейке или показать полным списком в [Popover](/ru/components/popover). Чтобы высота строки менялась вместе с содержимым, включите для колонки `autoHeight`.
