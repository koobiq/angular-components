...
Длинный список можно уместить в одну строку ячейки. [Clamped list](/ru/components/clamped-list) раскрывает скрытые элементы по клику, а [Overflow items](/ru/components/overflow-items) скрывает то, что не поместилось по ширине, и это можно показать в [Popover](/ru/components/popover). Чтобы высота строки менялась вместе с содержимым, включите для колонки `autoHeight`.
