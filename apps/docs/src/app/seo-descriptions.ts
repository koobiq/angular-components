...
export const DOCS_SEO_DESCRIPTIONS = {
    ...
    "list": {
        "en": "A list shows a set of related items and lets the user select one or several of them. To pick a single value from a closed set inside a form use Select, and for hierarchical data use Tree.",
        "ru": "Список показывает набор связанных элементов и позволяет выбрать один или несколько. Для выбора одного значения из закрытого набора в форме используйте Select, а для иерархических данных — Tree."
    },
    "loader-overlay": {
