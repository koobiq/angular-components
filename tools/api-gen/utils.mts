...
} from './rendering/entities/renderables.mjs';
...
export function getDirectiveMetadata(classPath: ts.SourceFile): Map<string, any> | null {
    ...
    (objectExpression.properties as ts.NodeArray<ts.PropertyAssignment>)
        .forEach((prop) => {
            ...
            if (prop.initializer.kind === ts.SyntaxKind.ArrayLiteralExpression) {
                const arrayData = (prop.initializer as ts.ArrayLiteralExpression).elements
                    .map((literal) => (literal as ts.StringLiteral).text);
...
export function sortCategorizedMethodMembers(
    methodEntryA: MemberEntryRenderable,
    docB: MemberEntryRenderable
) {
    // Sort deprecated docs to the end
    if (!methodEntryA.isDeprecated && docB.isDeprecated) { return -1; }

    if (methodEntryA.isDeprecated && !docB.isDeprecated) { return 1; }

    // Break ties by sorting alphabetically on the name
    if (methodEntryA.name < docB.name) { return -1; }
...
export function sortCategorizedPropertyMembers(
    ...
    // Sort deprecated docs to the end
    if (!propertyEntryA.isDeprecated && propertyEntryB.isDeprecated) { return -1; }
    ...
    // Break ties by sorting alphabetically on the name
    if (propertyEntryA.name < propertyEntryB.name) { return -1; }
