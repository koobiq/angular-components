...
import { DocEntry, FunctionEntry, MemberEntry, MemberTags, ParameterEntry, PropertyEntry } from '../entities';
import {
    isClassEntry,
    isClassMethodEntry,
    isConstantEntry,
...
export function mapDocEntryToCode(entry: DocEntry): CodeTableOfContentsData {
    ...
    if (isTypeAliasEntry(entry)) {
        ...
        const deprecatedLineNumbers: number[] = [];
...
function getTags(member: PropertyEntry): string[] {
    return member.memberTags.map((tag) => ANGULAR_PROPERTY_DECORATORS[tag] ?? tag).filter((tag) => !!tag);
}
...
function appendPrefixAndSuffix(entry: DocEntry, codeTocData: CodeTableOfContentsData): void {
    const appendFirstAndLastLines = (data: CodeTableOfContentsData, firstLine: string, lastLine: string) => {
