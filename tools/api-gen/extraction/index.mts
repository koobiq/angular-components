...
import { entryHandler, updateEntries, prepareMergedMetadata } from './helpers.mjs';
import { PackageMetadata, EntryCollection, ClassEntryMetadata, ModuleInfo } from '../types/index.mjs';
import { src } from '../utils.mjs';


const getMetadataFrom = (moduleName: string, packageName: string): PackageMetadata => {
    const entryPointPath = `${moduleName}/${packageName}`;
    const entryPointIndexPath = `${process.env.BASE_PATH}/${entryPointPath}/index.ts`;
...
const getModulePackagePaths = ({ moduleName, include, exclude }: ModuleInfo): string[] => {
   ...
   return src(join(process.env.BASE_PATH || '', moduleName, rule));
}

export function extractApiToJson(packages: ModuleInfo[]) {
    const modules: { [moduleName: string]: PackageMetadata[] } = packages
        .reduce((res, current) => {
