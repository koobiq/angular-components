import { ClassEntry, DocEntry, MemberEntry, MemberTags } from '../rendering/entities.mjs';
import { isClassEntry } from '../rendering/entities/categorization.mjs';
import fs from 'fs';
import ts from 'typescript';
import { ClassEntryMetadata, PackageMetadata } from '../types/types.mjs';
import { relative } from 'path';
import { isPublic } from '../manifest/helpers.mjs';
...
const metadataMembers = ({ name, memberTags }: MemberEntry) => !fProps.includes(name) &&
    !memberTags.includes(MemberTags.Inherited);
...
export function updateEntries(entries: DocEntry[], classMetadata: Record<string, ClassEntryMetadata>): DocEntry[] {
    return entries.reduce((res: DocEntry[], entry: DocEntry, _, arr) => {
        if (!isClassEntry(entry)) { res.push(entry); }

        // base class will be added to entry info if it isn't marked as docs-private and placed in scope of package
        const baseClassEntry = classMetadata[entry.name]?.baseClass && arr
            .find((curEntry) => curEntry.name === classMetadata[entry.name]?.baseClass);
        ...
        res.push({
            ...
            extendedDoc: !!baseClassEntry && isPublic(baseClassEntry) && {
...
export function entryHandler(entrySrc: string) {
    ...
    sourceFile.forEachChild((node) => {
        ...
        const expressions = (decorators || [])
            .filter((decorator: any) => decorator.expression &&
                ts.isCallExpression(decorator.expression) &&
                ts.isIdentifier((decorator.expression as ts.CallExpression).expression)
            )
            .map(({ expression }: ts.Decorator) => ts.idText(((expression as ts.CallExpression).expression as ts.Identifier)));
