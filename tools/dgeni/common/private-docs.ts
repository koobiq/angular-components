...
export function isPublicDoc(doc: ApiDoc) {
    ...
    if (isInheritanceCreatedDoc(doc)) { return false; }

    if (isEnforcedPublicDoc(doc)) { return true; }
...
function isInternalMember(memberDoc: MemberDoc): boolean {
    return INTERNAL_METHODS.includes(memberDoc.name);
}

/** Whether the given doc has a @docs-private tag set. */
function hasDocsPrivateTag(doc: ApiDoc): boolean {
 ...
 */
function isEnforcedPublicDoc(doc: ApiDoc): boolean {
