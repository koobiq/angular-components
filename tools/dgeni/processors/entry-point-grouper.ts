...
export class EntryPointGrouper implements Processor {
    ...
    $process(docs: DocCollection) {
        ...
        docs.forEach((doc) => {
            const moduleInfo = this.getModulePackageInfo(doc);

            const packageName = moduleInfo.packageName;
            const packageDisplayName = packageName === 'cdk' ? 'CDK' : 'Koobiq';

            const moduleImportPath = `@koobiq/${packageName}/${moduleInfo.entryPointName}`;
        ...
        entryPoints.forEach((entryPoint) => {
            if (entryPoint.primaryExportName !== null) { return; }

            const ngModuleExport = this.findBestPrimaryExport(entryPoint.exportedNgModules);
            ...
            const testHarnessExport = this.findBestPrimaryExport(entryPoint.testHarnesses);
     ...
     */
    private findBestPrimaryExport(docs: ApiDoc[]): ApiDoc | null {
        ...
        for (const doc of docs) {
            if (!isDeprecatedDoc(doc)) { return doc; }
        }
    ...
    private getModulePackageInfo(doc: Document): ModuleInfo {
        ...
        const foundEntryPoint = this.findMatchingEntryPoint(relativeFilePath);
        ...
        const moduleName = entryPointName.join('-');
    ...
    private findMatchingEntryPoint(relativeFilePath: string): string | null {
